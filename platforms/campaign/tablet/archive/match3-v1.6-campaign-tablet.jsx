...
// =============================================================================
// MATCH-3 GAME — CAMPAIGN MODE v1.6 — TABLET
...
const CampaignGame = ({ levelIndex, onLevelComplete, onBackToMap }) => {
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft(prev => {
        if (prev <= 1) {
          // v1.4 spec mechanic: if target not yet reached and banked moves remain,
          // consume all banked moves to add time (+5s each) — one-time per level
          if (scoreRef.current < LEVEL_TARGET && campaignBonusMovesRef.current > 0) {
            const bonusMoves = campaignBonusMovesRef.current;
            const extraSeconds = bonusMoves * TIME_EXTENSION_AMOUNT;
            setCampaignBonusMoves(0); // also clears campaignBonusMovesRef via effect
            campaignBonusMovesRef.current = 0;
            setTimeExtensions(p => [...p, {
              id: Date.now() + Math.random(),
              reason: `🏦 +${extraSeconds}s from ${bonusMoves} banked move${bonusMoves !== 1 ? 's' : ''}!`,
              createdAt: Date.now(),
            }]);
            return extraSeconds;
          }
  ...
  useEffect(() => {
    ...
    const checkTimer = setTimeout(() => {
      ...
      if (moves <= 0 && !hasReachedTarget && !usingExtraMoves && !usingBankedMoves) {
        if (bonusMovePool > 0) {
      ...
      // Using banked moves: each swap already consumed from campaignBonusMoves.
      // If campaignBonusMoves hit 0, it's a true game over.
      if (moves <= 0 && !hasReachedTarget && usingBankedMoves) {
        if (campaignBonusMoves > 0) return; // still have banked moves — game continues
        setScore(prev => prev + specialsBonus);
        setGameState('gameover');
      }
    }, 150);
    return () => clearTimeout(checkTimer);
  }, [moves, gameState, LEVEL_TARGET, isAnimating, combo, targetReached, pendingSpecials.length, grid, turnComplete, bonusRoundActive, showBonusPrompt, showExtraMovesPrompt, bonusMovePool, usingExtraMoves, IS_TIME_ATTACK, usingBankedMoves, showBankedMovesPrompt, campaignBonusMoves]);
  ...
  // Player chose "Use banked moves" — remaining campaignBonusMoves consumed one per swap
  ...
  // Player chose "End & carry 🏦 moves forward" — end level, remaining banked moves persist naturally
  const endLevelCarryBanked = () => {
    usingBankedMovesRef.current = false;
    setUsingBankedMoves(false);
    setShowBankedMovesPrompt(false);
    setGameState(targetReached || scoreRef.current >= LEVEL_TARGET ? 'won' : 'gameover');
  };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <h1 style={{ margin: '0', color: '#333', fontSize: '18px' }}>
          ...
          <span style={{ fontSize: '11px', color: '#888', marginLeft: '6px' }}>v1.6</span>
        </h1>
      ...
      {/* Banked Moves Prompt — shown when regular moves hit 0, pool empty, but banked moves remain */}
      {showBankedMovesPrompt && (
        <div style={{
          ...
          <div style={{ textAlign: 'center', color: 'white', minWidth: '160px' }}>
            ...
            <div style={{ fontSize: '14px', color: 'rgba(255,255,255,0.85)' }}>
              You have {campaignBonusMoves} banked move{campaignBonusMoves !== 1 ? 's' : ''}
            </div>
          </div>
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', justifyContent: 'center' }}>
            <button
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
              ...
              <button
                onClick={() => {
                  ...
                  setBonusMoveFlash(0); setBonusMovePool(0); setUsingExtraMoves(false); setShowExtraMovesPrompt(false);
                  setUsingBankedMoves(false); usingBankedMovesRef.current = false; setShowBankedMovesPrompt(false);
                }}
