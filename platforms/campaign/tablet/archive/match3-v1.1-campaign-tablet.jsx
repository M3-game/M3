...
// =============================================================================
// MATCH-3 GAME — CAMPAIGN MODE v1.1 — TABLET
    ...
    <div style={{
      ...
      <div style={{
        ...
        {LEVEL_CONFIGS.map((cfg, idx) => {
          ...
          const unlocked = levelNum <= storedLevel || canAdvanceToLevel(levelNum, levelStars);
          ...
          return (
            <button
              ...
              style={{
                background: unlocked ? 'rgba(255,255,255,0.95)' : 'rgba(255,255,255,0.35)',
            ...
            >
              <div style={{ fontSize: '18px', fontWeight: 'bold', color: unlocked ? '#333' : '#aaa', marginBottom: '6px' }}>
...
const CampaignGame = ({ levelIndex, onLevelComplete, onBackToMap }) => {
  ...
  }, [score, gameState, turnComplete, IS_TIME_ATTACK]);
  ...
  const attemptSwap = (row1, col1, row2, col2) => {
    ...
    setTimeout(() => {
      ...
      if (matches.length > 0) {
        if (!IS_TIME_ATTACK) setMoves(prev => prev - 1);
  ...
  const activateSpecialCombination = (row1, col1, row2, col2, type1, type2, currentGrid) => {
    ...
    const finalPoints = bonusRoundActive ? Math.floor(points * BONUS_ROUND_MULTIPLIER) : points;
    addScorePopup(effectRow, effectCol, finalPoints, message);
    setScore(prev => prev + finalPoints); setCurrentTurnScore(prev => prev + finalPoints);
    if (bonusRoundActive) setBonusRoundScore(prev => prev + finalPoints);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <h1 style={{ margin: '0', color: '#333', fontSize: '18px' }}>
          ...
          <span style={{ fontSize: '11px', color: '#888', marginLeft: '6px' }}>v1.1</span>
        </h1>
      ...
      {(gameState === 'gameover' || gameState === 'won') && (() => {
        const finalStars = gameState === 'won' ? calculateStars(score, LEVEL_TARGET) : 0;
        return (
          <div style={{ position: 'fixed', top: 0, left: 0, right: 0, minHeight: '130px', background: gameState === 'won' ? 'linear-gradient(135deg, rgba(68,255,68,0.98), rgba(40,180,40,0.98))' : 'linear-gradient(135deg, rgba(255,68,68,0.98), rgba(180,40,40,0.98))', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '20px', zIndex: 1000, boxShadow: '0 4px 20px rgba(0,0,0,0.4)', padding: '10px 20px', flexWrap: 'wrap' }}>
            <div style={{ textAlign: 'center', color: 'white' }}>
              <div style={{ fontSize: '24px', fontWeight: 'bold', marginBottom: '4px' }}>{gameState === 'won' ? '🎉 Level Complete!' : '😓 Level Failed'}</div>
              <div style={{ fontSize: '20px' }}>Score: <strong>{score}</strong></div>
              {gameState === 'won' && <div style={{ marginTop: '6px' }}><StarRating stars={finalStars} /></div>}
            </div>
            <div style={{ textAlign: 'left', color: 'white', fontSize: '13px', lineHeight: '1.5', background: 'rgba(0,0,0,0.2)', padding: '8px 12px', borderRadius: '8px' }}>
              <div>Target: {LEVEL_TARGET}</div>
              <div>Best combo: x{maxComboReached + 1}</div>
              {IS_TIME_ATTACK && <div>Time attack level</div>}
            </div>
            <div style={{ display: 'flex', gap: '10px', flexDirection: 'column', alignItems: 'center' }}>
              <button onClick={onBackToMap} style={{ padding: '10px 22px', fontSize: '15px', background: 'white', color: gameState === 'won' ? '#28b428' : '#b42828', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', boxShadow: '0 2px 8px rgba(0,0,0,0.2)' }}>
...
// =============================================================================
// ROOT COMPONENT — manages screen routing between Level Map and Game
// =============================================================================
const Match3Campaign = () => {
  ...
  const handleLevelComplete = ({ levelIndex, score, stars, won }) => {
    ...
    // Stay on game screen so end overlay is visible; back button returns to map
  };
