...
// =============================================================================
// MATCH-3 GAME — CAMPAIGN MODE v1.10 — TABLET
...
const CampaignGame = ({ levelIndex, onLevelComplete, onBackToMap }) => {
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft(prev => {
        if (prev <= 1) {
          // v1.10: convert bonus moves one at a time (+5s each); stop if target reached.
          // Previous behaviour consumed the entire pool at once unconditionally — wrong.
          if (scoreRef.current < LEVEL_TARGET && campaignBonusMovesRef.current > 0) {
            ...
            const remaining = campaignBonusMovesRef.current;
            setTimeExtensions(p => [...p, {
              id: Date.now() + Math.random(),
              reason: `🏦 +${TIME_EXTENSION_AMOUNT}s from bonus move${remaining > 0 ? ` (${remaining} left)` : ''}!`,
              createdAt: Date.now(),
            }]);
            return TIME_EXTENSION_AMOUNT;
          }
