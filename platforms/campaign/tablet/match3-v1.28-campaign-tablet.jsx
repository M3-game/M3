...
// =============================================================================
// MATCH-3 GAME — CAMPAIGN MODE v1.28 — TABLET (T-2 terminology rename)
//
// Session T-2 (2026-05-02): Bundled terminology rename. "Bonus Round" →
// "Victory Round" (the 1.5× multiplier phase) and `bankedMoves` →
// `bonusMoves` (the carry pool of earned +1 moves). Mirrors T-1's
// pattern; T-2 completes the sweep across desktop, time-attack, phone
// arcade, and campaign.
//
// Campaign-specific changes:
//   - All identifier renames per the standard inventory (state vars,
//     refs, setters, functions, the BANKED_MOVES_CAP/WARN constants).
//   - Import update: `BANKED_KEY` → `BONUS_MOVES_KEY` (core constant
//     was renamed in T-2 for code coherence; campaign as a T-2 platform
//     gets the import-line update along with the full terminology
//     rename).
...
//   `BONUS_MOVES_KEY` (imported from core) keeps its value
//   'match3_bankedMoves'. T-3 migrates this and all other `bankedMoves`
//   storage strings as one coordinated lockstep event.
//
// Stats-data layer (per Option A continuation): `endType: 'bonusRound'`
// in `recordGameResult()` and `stats.bonusRoundsTaken` counter
// preserved. T-3 migrates these along with the schema rename.
//
// No behavior changes. Pure rename.
//
// =============================================================================
// MATCH-3 GAME — CAMPAIGN MODE v1.27 — TABLET
//
...
// target, instead of ending the level directly. Same one-line fix as
// N-4: prompt button onClick switched from `endLevelCarryBanked` to
// `requestEndLevelCarryBanked` (the wrapper that already handles
// below-target confirms for the in-header button).
//
// What changed in v1.27 vs v1.26:
//   - Bonus-moves prompt's "End and carry moves forward" button now
//     routes through `requestEndLevelCarryBanked`. Wrapper's `wouldWin`
//     check produces correct behavior in both prompt variants — confirms
//     if below target; ends directly if targetReached.
//   - v1.25 wrapper comment block updated to reflect the v1.27 expansion.
//
// No other behavior changes. Single-line code change; rest is doc.
//
// =============================================================================
// MATCH-3 GAME — CAMPAIGN MODE v1.26 — TABLET
...
// v1.1: Bonus moves game-flow wiring (matches arcade v11.3)
...
//     (one-time per level, consumes all bonus moves).
...
// v1.6: Bonus moves UX — player can use 🏦 campaignBonusMoves during any level
//   When moves hit 0 and bonusPool is empty but campaignBonusMoves > 0:
//   "Use bonus moves" prompt appears (instead of gameover).
//   Choosing "Use bonus moves" lets the player keep playing; each move consumes
...
//   all remaining bonus moves to the next level.
...
// v1.8: Unified bonus/bonus moves — one pool, one counter (🏦)
...
//   a prompt offers "Use bonus moves" (keep playing, 🏦 ticks down) or
...
//   resolving. Button label changed from "Use bonus moves" to "Use bonus moves".
// v1.25: Session A — post-playtest polish batch (2026-04-21)
//   - Transition pause 3000ms → 2000ms via TRANSITION_PAUSE_MS constant.
//   - Bonus moves capped at 99 (BONUS_MOVE_CAP). Amber warning at ≥90
//     (BONUS_MOVE_WARN), red at 99. Existing moves preserved — only earning
//     past the cap is blocked. No "+1 bonus move" burst when at cap.
//   - "Are you sure?" confirm on in-header "End and carry moves forward"
//     button when below target during active bonus-moves use. Not fired from
//     the bonus-moves prompt (that's a save, not a loss).
//   - All-time best campaign run tracked via CAMPAIGN_KEYS.bestRunScore.
//     Shown on LevelSelect header subtitle and CampaignComplete card
//     (🏆 New record! badge when beaten).
//   - Timed-mode: 1.5s grace window (TIME_ATTACK_GRACE_MS) after timeLeft=0.
//     addTimeExtension can fire during grace and re-arm the timer. Fixes
//     cases where late cascade +5s extensions were lost because the card
//     popped before they could apply.
//   - Admin panel: Export / Import JSON + last-backed-up timestamp. Manual
//     safety net against iOS Safari ITP localStorage purge (7-day).
//   - Admin access from LevelSelect: long-press on "Campaign Mode" title
//     (1.5s) or ?admin=1 URL param reveals "🔧 Admin" button.
//   - Stale header version label (v1.12) → v1.25.
// v1.26: Session L (2026-04-22) — contrast fix in the inline CampaignAdmin
//   wrapper. Off-white-on-dark text (label/dim/access-hint/instruction
//   copy/disabled-button greys) lifted to #ccc floor; 10px uppercase
//   labels bumped to 11px + fontWeight 500; 11px helper text to 12px.
//   Banner version string v1.25 → v1.26. Shared core/AdminPanel.jsx
//   fix applies via import (edited in place per CLAUDE.md shared-core
//   exception).
// v1.24: Pause 3 seconds on win before transition screen
//   On gameState → 'won', handleLevelEnd is deferred by 3000ms so the player
//   sees the final grid state before the TransitionScreen appears.
//   Loss path (gameover overlay) is unchanged.
// v1.23: Fix — Campaign complete screen: stars displayed in rows of 10 (up to 4 rows)
//   All 40 stars were rendered in a single flat line, overflowing the card border.
//   New layout: Array.from rows of 10 stars each, filled left-to-right across rows.
// v1.22: Fix — bonus move prompt now fires after victory round ends
//   When bonusRoundActive && moves <= 0, code previously called setGameState('won')
//   immediately, skipping the bonus move prompt. Fix: check campaignBonusMoves first;
//   if > 0, end the victory round and show the prompt instead of resolving.
// v1.21: L4 intro — update cascade/combo bullet
// v1.20: Remove 🏦 icon — replace with "bonus moves" text throughout
//   Header counter: "🏦 N" → "Bonus moves: N". Burst notification: "+1 🏦"
//   → "+1 bonus move" (smaller text). L5 intro: icon removed (empty),
//   bullet updated to remove 🏦 header reference. LevelIntroScreen skips
//   icon div when intro.icon is falsy.
// v1.19: Fix — runTotalScore resets when player selects L1 from the map
//   handleSelectLevel now resets runTotalScore to 0 when levelIndex === 0,
//   covering the case where a player returns to the menu and starts again.
...
// v1.25 tunables (future: expose in admin panel — see docs/DEFERRED.md)
const TRANSITION_PAUSE_MS      = 2000;  // delay between win and transition screen
const BONUS_MOVE_CAP           = 99;    // max bonus moves retained
const BONUS_MOVE_WARN          = 90;    // header turns amber at/above this count
const TIME_ATTACK_GRACE_MS     = 1500;  // time-attack expiry grace window — late +5s extensions can still land
const LEVEL_SELECT_ADMIN_LONGPRESS_MS = 1500; // long-press on title to reveal admin button
...
// v1.25: all-time best full-run campaign score (after winning L8)
const loadBestRunScore = () => parseInt(localStorage.getItem(CAMPAIGN_KEYS.bestRunScore) || '0', 10);
...
// LEVEL SELECT SCREEN
// =============================================================================
const LevelSelectScreen = ({ levelStars, levelHighScores, bestRunScore, onSelectLevel, onOpenAdmin, adminVisible, onTitlePressStart, onTitlePressEnd }) => {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '24px' }}>
        <h1
          onPointerDown={onTitlePressStart}
          onPointerUp={onTitlePressEnd}
          onPointerLeave={onTitlePressEnd}
          style={{
            color: 'white', fontSize: '28px', marginBottom: '8px',
        ...
        <p style={{ color: 'rgba(255,255,255,0.8)', marginBottom: '4px', fontSize: '14px' }}>
          Total stars: {levelStars.reduce((a, b) => a + (b || 0), 0)} / {LEVEL_CONFIGS.length * 5}
        </p>
        {bestRunScore > 0 && (
          <p style={{ color: 'rgba(255,255,255,0.9)', marginTop: '2px', fontSize: '13px', fontWeight: 'bold' }}>
            🏆 Best campaign run: {bestRunScore.toLocaleString()}
          </p>
        )}
        {adminVisible && (
          <button
...
const LEVEL_INTROS = [
  ...
  // Level 4
  {
    ...
    bullets: [
      'Combinations of matches (combos) and each match that occurs after (cascades) create score multipliers that lead to big increases',
  ...
  // Level 5
  {
    ...
    heading: 'Bonus Moves',
    bullets: [
      'Every 10,000 points earns you 1 bonus move — tracked in the header as "Bonus moves"',
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {intro.icon && <div style={{ fontSize: '40px', textAlign: 'center', marginBottom: '8px' }}>{intro.icon}</div>}
...
// =============================================================================
const CampaignCompleteScreen = ({ levelStars, levelHighScores, bestRunScore, runScore, isNewRecord, onPlayAgain, onResetAll }) => {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontSize: '12px', color: '#999', marginBottom: '12px' }}>Total campaign score (best per level)</div>

        {/* v1.25: this-run total + all-time best run */}
        {(runScore > 0 || bestRunScore > 0) && (
          <div style={{
            background: isNewRecord ? 'linear-gradient(135deg, #fff9c4 0%, #ffecb3 100%)' : '#f5f3ff',
            border: isNewRecord ? '2px solid #FFD700' : '1px solid #c4b5fd',
            borderRadius: '10px', padding: '10px 16px', marginBottom: '16px',
          }}>
            {isNewRecord && (
              <div style={{ fontSize: '13px', color: '#b8860b', fontWeight: 'bold', marginBottom: '4px' }}>
                🏆 NEW CAMPAIGN BEST!
              </div>
            )}
            {runScore > 0 && (
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', color: '#333' }}>
                <span>This run</span>
                <span style={{ fontWeight: 'bold' }}>{runScore.toLocaleString()}</span>
              </div>
            )}
            {bestRunScore > 0 && !isNewRecord && (
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', color: '#666', marginTop: '4px' }}>
                <span>All-time best</span>
                <span>{bestRunScore.toLocaleString()}</span>
              </div>
            )}
          </div>
        )}
        ...
        <div style={{ fontSize: '20px', marginBottom: '20px' }}>
          {Array.from({ length: Math.ceil(maxStars / 10) }, (_, rowIdx) => {
            const start = rowIdx * 10;
            const end = Math.min(start + 10, maxStars);
            return (
              <div key={rowIdx} style={{ letterSpacing: '2px', lineHeight: '1.5' }}>
                {Array.from({ length: end - start }, (_, i) => {
                  const idx = start + i;
                  return <span key={i} style={{ color: idx < totalStars ? '#FFD700' : '#ddd' }}>★</span>;
                })}
              </div>
            );
          })}
...
const CampaignGame = ({ levelIndex, onLevelComplete, onBackToMap, highScore, bestStars, onStartOver }) => {
  ...
  // True after player chooses "Use bonus moves" — each swap consumes 1 from campaignBonusMoves
  const [usingBonusMoves, setUsingBonusMoves] = useState(false);
  // True when regular moves=0, pool=0, and campaignBonusMoves>0, shows bonus moves decision prompt
  ...
  // v1.25: 1.5s grace window after timer=0 during which +5s extensions can still re-arm the timer
  const [graceActive, setGraceActive] = useState(false);
  // v1.25: "are you sure?" confirm when in-header End-and-carry clicked while below target
  ...
  const usingBonusMovesRef = useRef(false);    // v1.6: ref mirror for use inside setTimeout callbacks
  ...
  // v1.25: Also pauses during the grace window (graceActive). A late
  //        addTimeExtension during grace can re-arm the timer by setting
  //        timeLeft > 0, at which point the grace effect clears graceActive
  //        and this effect re-runs to re-create the interval.
  useEffect(() => {
    if (!IS_TIME_ATTACK || gameState !== 'playing') return;
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft(prev => {
        if (prev <= 1) {
          ...
          // v1.25: start grace window; pendingTimeExpiry is set only after
          // grace ends without an extension landing.
          setGraceActive(true);
          return 0;
        }
        return prev - 1;
  ...
  }, [IS_TIME_ATTACK, gameState, showTimeUpPrompt, graceActive]);

  // v1.25: Grace window watcher. When the timer hits 0 and the level is about
  // to resolve, we wait TIME_ATTACK_GRACE_MS. If an addTimeExtension (from a
  // cascade's +5s special/combo/milestone) fires during that window, it sets
  // timeLeft > 0 — we cancel grace and the main timer effect resumes the
  // interval. If nothing lands by the end of the window, we fall through to
  // the normal pendingTimeExpiry resolution path.
  useEffect(() => {
    if (!graceActive) return;
    if (timeLeft > 0) {
    ...
    }, TIME_ATTACK_GRACE_MS);
    return () => clearTimeout(graceTimeout);
  }, [graceActive, timeLeft]);
  ...
  // v1.25: pool capped at BONUS_MOVE_CAP. If the player is at or would exceed
  // the cap, earned moves past the cap are silently discarded (no "+1 bonus
  // move" burst). The threshold ref still advances so we don't re-award them
  // once the player spends back down below the cap.
  useEffect(() => {
    ...
    if (threshold > 0 && threshold > bonusMoveThresholdRef.current) {
      ...
      const roomLeft   = Math.max(0, BONUS_MOVE_CAP - campaignBonusMovesRef.current);
      const applied    = Math.min(newMoves, roomLeft);
      if (applied > 0) {
        setCampaignBonusMoves(prev => Math.min(BONUS_MOVE_CAP, prev + applied));
        campaignBonusMovesRef.current = Math.min(BONUS_MOVE_CAP, campaignBonusMovesRef.current + applied);
        if (showBonusPrompt) bonusMoveFlashPendingRef.current += applied;
        else setBonusMoveFlash(prev => prev + applied);
      }
    }
  ...
  useEffect(() => {
    ...
    const checkTimer = setTimeout(() => {
      ...
      if (bonusRoundActive && moves <= 0) {
        // v1.22: offer bonus move prompt before resolving, same as non-bonus-round path.
      ...
      // moves = 0, not in victory round: offer bonus moves prompt if pool > 0.
      ...
      if (moves <= 0 && !bonusRoundActive && !usingBonusMoves) {
        if (campaignBonusMoves > 0) {
          setShowBonusMovesPrompt(true);
          return;
        }
  ...
  }, [moves, gameState, LEVEL_TARGET, isAnimating, combo, targetReached, pendingSpecials.length, grid, turnComplete, bonusRoundActive, showBonusPrompt, IS_TIME_ATTACK, usingBonusMoves, showBonusMovesPrompt, campaignBonusMoves]);

  // Victory round handlers
  ...
  // v1.25: Wrapper for the in-header "End and carry moves forward" button.
  // If the player is below target during active bonus-moves use, show a
  // confirm first — clicking End here means losing a level they could still
  // win.
  // v1.27 (Session N-5, 2026-04-29): the bonus-moves prompt's End button now
  // also routes through this wrapper. Original v1.25 wording excluded the
  // prompt button as "a save" — but user feedback (collected via tablet
  // v11.13 / N-4) was that the carry-forward decision is itself worth
  // confirming. The wrapper's `wouldWin` check produces correct behavior
  // for both prompt variants — confirms below target; just-ends if target
  // already reached.
  const requestEndLevelCarryBanked = () => {
    const { bonus: specialsBonus } = calculateUnusedSpecialsBonus(grid);
    const wouldWin = targetReached || scoreRef.current + specialsBonus >= LEVEL_TARGET;
    if (!wouldWin) {
  ...
  const attemptSwap = (row1, col1, row2, col2) => {
    ...
    if (tile1Special && tile2Special) {
      if (usingBonusMovesRef.current) setCampaignBonusMoves(prev => Math.max(0, prev - 1));
  ...
  useEffect(() => {
    if ((gameState === 'won' || gameState === 'gameover') && !levelEndFiredRef.current) {
      ...
      if (gameState === 'won') {
        setTimeout(() => handleLevelEnd(true), TRANSITION_PAUSE_MS);
      } else {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-around', fontSize: '15px', fontWeight: 'bold', color: '#555' }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              {bonusMoveFlash > 0 && (
                <span key={bonusMoveFlash} style={{
                  ...
                  fontSize: '14px', fontWeight: '700', color: '#00C853',
                  textShadow: '0 0 8px #00C853', pointerEvents: 'none',
          ...
          <div title="Bonus moves — carry forward to future levels" style={{ position: 'relative' }}>
            Bonus moves:{' '}
            <span style={{
              color: campaignBonusMoves >= BONUS_MOVE_CAP ? '#d32f2f'
                   : campaignBonusMoves >= BONUS_MOVE_WARN ? '#e65100'
                   : usingBonusMoves ? '#e65100' : '#667eea',
              fontWeight: 'bold',
            }}>
              {campaignBonusMoves}
              {campaignBonusMoves >= BONUS_MOVE_WARN && (
                <span style={{ fontSize: '10px', color: campaignBonusMoves >= BONUS_MOVE_CAP ? '#d32f2f' : '#e65100', marginLeft: '4px' }}>
                  / {BONUS_MOVE_CAP}
                </span>
              )}
            </span>
            {campaignBonusMoves >= BONUS_MOVE_WARN && (
              <div style={{
                ...
                color: campaignBonusMoves >= BONUS_MOVE_CAP ? '#d32f2f' : '#e65100',
                marginTop: '2px', lineHeight: '1.2',
              }}>
                {campaignBonusMoves >= BONUS_MOVE_CAP
                  ? 'Cap reached — no additional moves can be earned'
                  : `Nearing cap — max ${BONUS_MOVE_CAP}`}
              </div>
            )}
          </div>
        ...
        {bonusRoundActive && (
          ...
          }}>🌟 VICTORY ROUND — {BONUS_ROUND_MULTIPLIER}x ALL POINTS! 🌟</div>
        )}
      ...
      {/* v1.25: "Are you sure?" confirm when in-header End-and-carry clicked while below target.
          Only fires from the in-header button (active bonus-moves use) — not from the bonus-moves prompt. */}
      {showEndConfirm && (
        <div style={{
          position: 'fixed', top: 0, left: 0, right: 0, bottom: 0,
          background: 'rgba(0,0,0,0.75)',
          ...
          <div style={{
            ...
            <div style={{ fontSize: '14px', color: 'rgba(255,255,255,0.9)', lineHeight: 1.5, marginBottom: '20px' }}>
              You're below target ({scoreRef.current.toLocaleString()} / {LEVEL_TARGET.toLocaleString()})
              with {campaignBonusMoves} bonus move{campaignBonusMoves !== 1 ? 's' : ''} still available.
              <br />
              Ending now will fail this level. Your bonus moves carry forward either way.
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <button
        ...
        return (
          <div style={{ position: 'fixed', top: 0, left: 0, right: 0, minHeight: '100px', background: 'linear-gradient(135deg, rgba(255,215,0,0.98), rgba(255,165,0,0.98))', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '20px', zIndex: 1000, boxShadow: '0 4px 20px rgba(0,0,0,0.4)', padding: '10px 20px', flexWrap: 'wrap' }}>
            <div style={{ textAlign: 'center', color: '#333' }}>
              ...
              <div style={{ fontSize: '13px', color: '#555' }}>Enter Victory Round?</div>
            </div>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={startBonusRound} style={{ padding: '10px 18px', fontSize: '14px', background: '#333', color: '#FFD700', border: '2px solid #333', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold' }}>🌟 VICTORY ROUND</button>
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', marginBottom: '12px' }}>
              ...
              <button
                onClick={() => {
                  ...
                  setBonusMoveFlash(0);
                  setUsingBonusMoves(false); usingBonusMovesRef.current = false; setShowBonusMovesPrompt(false); setShowTimeUpPrompt(false); setPendingTimeExpiry(false); setGraceActive(false); setShowEndConfirm(false);
                }}
...
// =============================================================================
// CAMPAIGN ADMIN WRAPPER (v1.25) — backup / restore for campaign state.
// Rendered at root level so it's accessible from LevelSelect (not just
// mid-game). Writes a consolidated JSON of all CAMPAIGN_KEYS values; restore
// accepts the same shape and writes back via localStorage.
// Safety net against iOS Safari ITP 7-day purge of localStorage on
// m3-game.github.io. See docs/DEFERRED.md for PWA follow-up.
// =============================================================================
const CampaignAdminWrapper = ({ onClose, bestRunScore, onDataRestored }) => {
  const [importText, setImportText] = useState('');
  const [status, setStatus]         = useState(null); // {text, color}
  const [confirmRestore, setConfirmRestore] = useState(false);
  const [showGameStats, setShowGameStats]   = useState(false);

  const lastBackup = localStorage.getItem(CAMPAIGN_KEYS.lastBackup);

  const buildExport = () => {
    const data = { version: 1, exportedAt: new Date().toISOString(), keys: {} };
    Object.entries(CAMPAIGN_KEYS).forEach(([name, lsKey]) => {
      data.keys[name] = localStorage.getItem(lsKey);
    });
    return JSON.stringify(data, null, 2);
  };

  const flashStatus = (text, color = '#81c784') => {
  ...
  const markBackup = () => {
    ...
    try { localStorage.setItem(CAMPAIGN_KEYS.lastBackup, now); } catch {}
  };
  ...
  const handleCopyToClipboard = async () => {
    ...
    try {
      ...
      ta.select();
      try { document.execCommand('copy'); markBackup(); flashStatus('✓ Copied to clipboard'); }
      catch { flashStatus('⚠ Copy failed — use Download instead', '#f87171'); }
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `m3-campaign-backup-${Date.now()}.json`;
    a.click();
  ...
  const handleRestore = () => {
    ...
    try {
      ...
      if (!parsed || !parsed.keys) throw new Error('missing keys');
      Object.entries(parsed.keys).forEach(([name, value]) => {
        const lsKey = CAMPAIGN_KEYS[name];
        if (!lsKey) return;
      ...
      setImportText('');
      flashStatus('✓ Restored — refreshing state');
      onDataRestored && onDataRestored();
    } catch (err) {
      flashStatus('⚠ Invalid JSON — check format', '#f87171');
      setConfirmRestore(false);
    }
  };
  ...
  const S = {
    ...
    textarea:{ width: '100%', minHeight: '120px', background: '#0e0e0e', color: '#ddd', border: '1px solid #333', borderRadius: '6px', padding: '10px', fontFamily: 'monospace', fontSize: '11px', boxSizing: 'border-box', resize: 'vertical' },
  };
  ...
  return (
    <div style={S.overlay}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
        <div>
          ...
          <div style={{ fontSize: '12px', color: '#ccc' }}>
            Long-press "Campaign Mode" title or use <span style={{ color: '#ccc' }}>?admin=1</span>
          </div>
        </div>
        <button onClick={onClose} style={{ ...S.btn, background: '#2a2a2a', color: '#ccc', fontSize: '17px', lineHeight: 1, padding: '4px 11px' }}>×</button>
      </div>

      {/* ── Best run / campaign snapshot ─────────────────────────────────── */}
      <div style={S.card}>
        <span style={S.label}>Campaign Snapshot</span>
        <div style={S.row}>
          <span style={S.dim}>Best full-run score</span>
          <span style={S.val}>{bestRunScore > 0 ? bestRunScore.toLocaleString() : '—'}</span>
        </div>
        <div style={S.row}>
          <span style={S.dim}>Highest level reached</span>
          <span style={S.val}>{localStorage.getItem(CAMPAIGN_KEYS.level) ?? '1'}</span>
        </div>
        <div style={S.row}>
          <span style={S.dim}>Bonus moves pool</span>
          <span style={S.val}>{localStorage.getItem(CAMPAIGN_KEYS.bonusMoves) ?? '0'}</span>
        </div>
        <div style={S.row}>
          <span style={S.dim}>Last backup</span>
          <span style={S.val}>
            {lastBackup ? new Date(lastBackup).toLocaleString() : <span style={{ color: '#e57373' }}>never</span>}
          </span>
        </div>
      </div>
      ...
      {/* ── Backup ──────────────────────────────────────────────────────── */}
      <div style={S.card}>
        <span style={S.label}>Backup</span>
        <div style={{ fontSize: '12px', color: '#ccc', lineHeight: 1.5, marginBottom: '10px' }}>
          iOS Safari may purge localStorage after 7 days of inactivity. Save a backup you can paste back later.
        </div>
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          <button onClick={handleCopyToClipboard} style={{ ...S.btn, background: '#1b3a1a', color: '#81c784' }}>📋 Copy to clipboard</button>
          <button onClick={handleDownload}        style={{ ...S.btn, background: '#0d1b3e', color: '#90caf9' }}>📥 Download JSON</button>
        </div>
      </div>

      {/* ── Restore ─────────────────────────────────────────────────────── */}
      <div style={S.card}>
        <span style={S.label}>Restore</span>
        <div style={{ fontSize: '12px', color: '#ccc', lineHeight: 1.5, marginBottom: '10px' }}>
          Paste a previously-saved JSON backup. Overwrites current campaign state.
        </div>
        <textarea
          value={importText}
          onChange={e => setImportText(e.target.value)}
          placeholder='{"version":1,"exportedAt":"...","keys":{...}}'
          style={S.textarea}
        />
        <div style={{ marginTop: '10px' }}>
          <button
            ...
            style={{
              ...S.btn,
              background: confirmRestore ? '#4a1010' : (importText.trim() ? '#3a2a0d' : '#1e1e1e'),
              color: confirmRestore ? '#f87171' : (importText.trim() ? '#ffcc80' : '#ccc'),
              cursor: importText.trim() ? 'pointer' : 'not-allowed',
            }}
          >
            {confirmRestore ? '⚠ Confirm restore?' : '♻ Restore from JSON'}
          </button>
        </div>
      </div>

      {status && (
        <div style={{ ...S.card, textAlign: 'center', color: status.color }}>{status.text}</div>
      )}
      ...
      {showGameStats && (
        <AdminPanel onClose={() => setShowGameStats(false)} />
      )}
    </div>
  );
};
...
const Match3Campaign = () => {
  ...
  // v1.25: all-time best full-run score + new-record flag for the CampaignComplete screen
  const [bestRunScore, setBestRunScore] = useState(loadBestRunScore);
  const [lastRunNewRecord, setLastRunNewRecord] = useState(false);
  // v1.25: admin access from LevelSelect (?admin=1 URL param or long-press on title)
  const [adminEnabled, setAdminEnabled] = useState(
    () => new URLSearchParams(window.location.search).get('admin') === '1'
         || sessionStorage.getItem('m3_admin') === '1'
  );
  const [showRootAdmin, setShowRootAdmin] = useState(false);
  const titlePressTimerRef = useRef(null);

  const handleTitlePressStart = () => {
    titlePressTimerRef.current = setTimeout(() => {
      setAdminEnabled(true);
      try { sessionStorage.setItem('m3_admin', '1'); } catch {}
    }, LEVEL_SELECT_ADMIN_LONGPRESS_MS);
  };
  const handleTitlePressEnd = () => clearTimeout(titlePressTimerRef.current);

  const handleSelectLevel = (levelIndex) => {
    if (levelIndex === 0) setRunTotalScore(0);
  ...
  const handleLevelComplete = ({ levelIndex, score, stars, won, bonusMovesEarned, bonusMovePoolTotal, movesUsed, bestCombo, specialsCreated }) => {
    ...
    if (won) {
      ...
      // v1.25: on L8 win, check for new all-time best run
      const isFinalLevel = levelIndex === LEVEL_CONFIGS.length - 1;
      if (isFinalLevel && newRunTotal > bestRunScore) {
        setBestRunScore(newRunTotal);
        setLastRunNewRecord(true);
        try { localStorage.setItem(CAMPAIGN_KEYS.bestRunScore, newRunTotal.toString()); } catch {}
      } else if (isFinalLevel) {
        setLastRunNewRecord(false);
      }
  ...
  // v1.25: admin panel rendered at root level so it's accessible from LevelSelect too.
  // Reuses the same AdminPanel component; CampaignGame renders its own instance while in-game.
  const adminOverlay = showRootAdmin && (
    <CampaignAdminWrapper
      onClose={() => setShowRootAdmin(false)}
      bestRunScore={bestRunScore}
      onDataRestored={() => {
        // Re-read all campaign state from localStorage after an import
        setLevelStars(loadCampaignStars());
        setLevelHighScores(loadCampaignHighScores());
        setBestRunScore(loadBestRunScore());
      }}
    />
  );
  ...
  if (screen === 'levelSelect') {
    return <>
      <LevelSelectScreen
        ...
        onOpenAdmin={() => setShowRootAdmin(true)}
        adminVisible={adminEnabled}
        onTitlePressStart={handleTitlePressStart}
        onTitlePressEnd={handleTitlePressEnd}
      />
      {adminOverlay}
    </>;
  }
