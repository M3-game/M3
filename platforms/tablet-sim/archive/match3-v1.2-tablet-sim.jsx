...
// =============================================================================
// MATCH-3 GAME v1.2 — TABLET SIM (headless simulation sibling)
//
...
// with a 1-ply bot heuristic, without any React state, for balance testing.
//
...
// =============================================================================
// HEADLESS SIMULATION — Session E-1a
// =============================================================================
// Pure-function reimplementation of the tablet v11.11 game mechanics plus a
// 1-ply bot heuristic. Expose on window.runSimGame() for DevTools
// verification. No React state; callable anytime.
//
// Simplifications vs. real game (documented limitations):
//   - Match base scoring = tiles * 10 * cascadeMultiplier. Real game has
//     subtle 4+/5+/L-shape bonuses folded into the base here.
//   - Specials activation: flat points (line/cross/bomb 750, supernova
//     2000, hypernova 5000) × cascade multiplier. Real game has tuned
//     combo scores; sim sums individual values.
//   - Hypernova's "random 50% of remaining regular tiles" is NOT simulated;
//     treated as 5×5 + row + col only. Underestimates hypernova clear count.
//   - Specials-swap combo fires via synthetic 1-tile pseudo-groups at each
//     special's position (resolveMatches then activates them).
//   - No bonus round, no time-attack, no banked-move persistence.
// =============================================================================
const _SIM = (() => {
  ...
  // Find 3+ runs (horizontal + vertical). Returns array of { tiles, type, length, direction }.
  ...
  // Activation zone for a special at (row, col).
  ...
  // Resolve one match-round: clear matched tiles, activate specials in the
  // cleared area, create new specials from 4+ groups, return point total.
  function resolveMatches(grid, groups, cascadeDepth) {
    ...
    // Activate any specials caught in the cleared area; expand the zone.
    let activationPts = 0;
    ...
    // Create new specials from 4+ match groups (skip 1-length pseudo-groups).
    const toCreate = [];
    groups.forEach(g => {
  ...
  // Try a swap. Returns { valid, swappedGrid, groups, bothSpecial, tile1, tile2 }.
  // If both tiles are specials and no natural matches result, synthesize
  // 1-tile pseudo-groups so resolveMatches activates both specials.
  ...
  function runGame(opts = {}) {
    // v1.2 — E-1b: canonical opts shape is { bot, target, moves, botParams }.
    // Legacy keys (tileCount, levelTarget) still honored so window.runSimGame()
    // from pre-v1.2 continues to work. `bot` is echoed back in the result so
    // the E-1b batch runner can include it in JSON metadata without a
    // separate parameter; `botParams` is reserved for E-2a (Monte Carlo).
    ...
    while (moves > 0 || bonusMoves > 0) {
      ...
      const best = pickBestSwap(grid, score, levelTarget, effectiveMoves);
  ...
  return {
    ...
    trySwap, enumerateValidSwaps, scoreSwap, pickBestSwap, runGame,
...
// =============================================================================
const SIM_VERSION = 'v1.2-tablet-sim';
...
const SimBatchRunner = () => {
  ...
  const [target, setTarget] = useState(() => {
    const stored = sessionStorage.getItem(SIM_TARGET_KEY);
    const n = stored ? parseInt(stored, 10) : NaN;
    return Number.isFinite(n) && n > 0 ? n : SIM_DEFAULT_TARGET;
  });
  const [moves, setMoves] = useState(() => {
    const stored = sessionStorage.getItem(SIM_MOVES_KEY);
    const n = stored ? parseInt(stored, 10) : NaN;
    return Number.isFinite(n) && n > 0 ? n : SIM_DEFAULT_MOVES;
  });
  ...
  const runBatch = async (size) => {
    ...
    for (let i = 0; i < size; i++) {
      ...
      const result = _SIM.runGame({ bot, target, moves });
      games.push(result);
