...
//
// The hazard was reported in reward mode rather than here, but the code path
// is the same: the cap applies when bonus moves are EARNED (one per 10,000
// points), so playing here with more than 99 stored would have cut the total
// to 99 and saved that back for arcade and verses to read. Fixed before it was
// hit rather than after.
//
// BANKED_MOVES_CAP 99 → 999, BANKED_MOVES_WARN 90 → 900. Nothing else changed.
//
// =============================================================================
// MATCH-3 GAME v1.5 - TABLET SIM (T-3a stats-blob writes)
//
...
//
// =============================================================================
// MATCH-3 GAME v1.4 - TABLET SIM (T-2 importer-only update)
//
...
// in lockstep with all platform constants. Local state var still
// `bankedMoves` (tablet-sim is a sim tool; full terminology rename
// deferred).
//
// =============================================================================
...
import {
  ...
  BONUS_MOVE_INTERVAL,
  SUPERNOVA_MIN_TILES, HYPERNOVA_MIN_TILES,
  hasValidMoves, calculateUnusedSpecialsBonus,
  _SIM,
} from './simCore.js';

// =============================================================================
// MATCH-3 GAME v1.3 — TABLET SIM (headless simulation sibling)
//
// Sibling platform forked from platforms/tablet/match3-v11.11-tablet.jsx.
// Independent versioning (v1.0 start). Purpose: run games programmatically
// with a 1-ply or Monte Carlo bot, without any React state, for balance
// testing.
//
// v1.3 — Session E-2a: Monte Carlo bot + Web Worker pool.
//   The v1.2 `_SIM` namespace plus its shared constants and pure helpers
//   (`hasValidMoves`, `findMatchesSimple`, `calculateUnusedSpecialsBonus`)
//   moved into `./simCore.js` so both the main thread and Web Workers
//   (`./sim-worker.js`) can import the same implementation. `_SIM` itself
//   now holds a new Monte Carlo bot (`mcPickBestSwap`) and `runGame`
//   dispatches on `opts.bot` between `'heuristic-1-ply'` (default) and
//   `'monte-carlo'` (which consumes `opts.botParams = { n, depthCap,
//   rolloutStrategy }`).
//
//   Admin batch runner gains an MC Parameters sub-section (N default 30 /
//   range 10–200, depth-cap default 5 / range 3–20, rollout strategy
//   1-ply-heuristic or random default 1-ply, workers default
//   navigator.hardwareConcurrency−1). All four persist to sessionStorage.
//   Results summary line shows bot name + key params. Clipboard JSON
//   metadata adds `botParams` when bot === 'monte-carlo'.
//
//   Runtime architecture: for the 1-ply bot, games still run synchronously
//   on the main thread with `setTimeout(0)` yields between games (fast
//   enough; ~12s for 1000 games). For the MC bot, games are dispatched
//   across a pool of Web Workers (each running a full game per message);
//   pool is spun up on Run and terminated on Cancel or batch completion.
//
// v1.2 — Session E-1b: admin UI batch runner + stats + histogram.
//   Adds a "Simulation Batch Runner" admin card in the tablet-sim admin
//   wrapper. Bot dropdown (1-Ply Heuristic only — MC slot reserved for
//   E-2a), admin-tunable Target score + Moves per game inputs (default
//   6000 / 20, sessionStorage-persisted), preset batch-size buttons
//   [10][50][100][200][1000], async orchestration via setTimeout(0)
//   yield per game, live progress text + Cancel button during runs.
//   Results panel: aggregate stats block (outcomes, score distribution,
//   moves, cascades, specials, bonus moves, runtime), vertical-bar
//   score histogram with target line + median/p10/p90 ticks, collapsible
//   per-game detail table, Copy-JSON button emitting Option C shape
//   (metadata + aggregates + raw games). _SIM.runGame signature extended
//   to {bot, target, moves, botParams} — final shape set here so E-2a's
//   Monte Carlo addition slots in without retrofit. Return shape adds
//   `bonusMovesEarned` accumulator (remaining field was always 0 post-
//   v1.1 because bot consumes everything; earned is the useful counter
//   for the "% games earning ≥1 bonus move" stat).
//
// v1.1 — Session E-1a2: bot now plays until moves exhausted.
//   Previously the game loop exited as soon as score ≥ levelTarget, which
//   meant the bot stopped playing after hitting target and therefore
//   never crossed the 10k bonus-move threshold across 50 games of console
//   testing. The 0-bonus-moves finding was an artifact of the stop-at-
//   target setting, not a real "bot can't reach it" signal. Fixed by
//   dropping the `score < levelTarget` clause from the game-loop while
//   condition — games now run until both moves and bonusMoves pools are
//   empty (or the board becomes stuck). The `won` flag in the returned
//   result still reflects whether target was reached at any point;
//   finalScore captures the full play-through.
//
// v1.0 — Session E-1a: scaffold + headless game loop + 1-ply heuristic.
//   The original tablet React component is preserved (so visiting
//   tablet-sim.html still renders a playable board — useful for
//   debugging). Added alongside it is a pure-function namespace `_SIM`
//   implementing init / match-detect / specials / cascade / scoring /
//   refill / game-loop, plus a phase-aware 1-ply heuristic. Exposed as
//   `window.runSimGame()` for DevTools verification.
//
// E-1b (future): admin UI wrapping _SIM — preset buttons for batch runs
//   (10 / 50 / 100 / 200 / 1000), progress indicator, win-rate + score-
//   distribution stats + vertical-bar histogram, clipboard-copy JSON
//   (aggregates + per-game raw). Scoped 2026-04-23.
//
// Sim scope notes:
//   - Specials: created on 4+ matches, activated on clear, combos
//     (special × special) supported. Hypernova's "half of remaining
//     regular tiles" branch approximated as 5×5 + row + col only.
//   - Bonus moves earned per 10k points; heuristic considers this in
//     the desperate phase.
//   - Banked moves NOT persisted across sim games (reset each run).
//   - Bonus round SKIPPED — game ends when both moves and bonusMoves
//     hit 0 (or the board stucks). Target is a win marker, not a stop
//     signal (v1.1 change).
//   - Time-attack mode not simulated (not part of tablet arcade).
//
// Calibration caveat (from DEFERRED / DESIGN): 1-ply bot is lower-bound
// casual play. Skilled humans set up 5–8 move cascade chains the bot
// can't see. Use sim output for RELATIVE difficulty comparisons and
// balance floor checks; never for absolute target tuning.
//
// Original tablet v11.11 header comment block preserved below.
// =============================================================================
//
...
// v11.2-tablet: Stats + admin panel + banked moves foundation
...
//   Banked moves: persistent counter + on-screen display (game-flow wiring TBD)
// v11.3-tablet: Banked moves game-flow wiring
...
//   play (per 10k pts) go directly into bankedMoves (persistent across games)
//   instead of a separate in-game bonusMovePool.
//   Prompt fires whenever moves=0 and bankedMoves>0 — win OR fail (fixes the
//   old !hasReachedTarget gate that caused the prompt to never appear on a win).
//   Prompt also fires after the bonus round ends if bankedMoves>0.
//   "Use bonus moves": sets usingBankedMoves=true; each valid swap draws 1 from
//   bankedMoves (moves counter stays at 0).
//   "End and carry moves forward": game ends, bankedMoves persist naturally.
...
//
//   1) Bonus-move cap bumped 25 → 99 (BANKED_MOVES_CAP). Warning threshold
//      bumped 20 → 90 (BANKED_MOVES_WARN). Header messaging when nearing
...
//      (that's a save, not a loss). Click "Yes, end…" → endLevelCarryBanked.
...
// v1.3 — E-2a: ROWS, COLS, TILE_TYPES, MIN_MOVES, MAX_MOVES, BASE_TARGET,
// TARGET_VARIANCE, BONUS_MOVE_INTERVAL, SUPERNOVA_MIN_TILES,
// HYPERNOVA_MIN_TILES now imported from ./simCore.js (shared with sim
// worker + bot logic). React-side-only constants still declared here.

// React-side rendering constants
const TILE_SIZE = 50;
const TILE_GAP = 4;
...
// points here would cut a >99 total down to 99 and save it back for every
// other platform to read. Fixed alongside rewardmode v1.5, 2026-08-22.
const BANKED_MOVES_CAP  = 999; // hard cap on stored moves
const BANKED_MOVES_WARN = 900; // counter turns amber at/above this; red at cap
...
// v1.3 — E-2a: hasValidMoves, findMatchesSimple, calculateUnusedSpecialsBonus
// moved to ./simCore.js (imported at top). They were formerly module-scope
// in v1.2 and called by both _SIM and the React game; now single-sourced.

// =============================================================================
// v1.3 — E-2a: _SIM namespace now lives in ./simCore.js. Imported at top of
// this file. Contains the full headless game simulation (init, match
// detection, specials, cascade, scoring) plus two bots: 1-ply heuristic
// and Monte Carlo. The same _SIM is imported by ./sim-worker.js for the
// Web Worker pool. See simCore.js header for simplification notes vs. the
// real React game.
// =============================================================================

if (typeof window !== 'undefined') {
  window.runSimGame = _SIM.runGame;
  window._SIM = _SIM;
}
// =============================================================================

// =============================================================================
// v1.2 — E-1b: SIMULATION BATCH RUNNER
// =============================================================================
// Aggregate-statistics helpers (pure). Called once per batch after the
// async run loop finishes. Percentile uses nearest-rank (simple + matches
// small-N intuition); batches are 10–1000 games, not large enough for
// interpolation to matter.
// =============================================================================
const SIM_VERSION = 'v1.3-tablet-sim';
const SIM_TARGET_KEY = 'm3_sim_target';
const SIM_MOVES_KEY = 'm3_sim_moves';
const SIM_MC_N_KEY = 'm3_sim_mc_n';
const SIM_MC_DEPTH_KEY = 'm3_sim_mc_depth';
const SIM_MC_ROLLOUT_KEY = 'm3_sim_mc_rollout';
const SIM_MC_WORKERS_KEY = 'm3_sim_mc_workers';
const SIM_PRESETS = [10, 50, 100, 200, 1000];
const SIM_DEFAULT_TARGET = 6000;
const SIM_DEFAULT_MOVES = 20;

// v1.3 — E-2a: Monte Carlo defaults. Revisit in E-2b after tuning runs.
const SIM_MC_DEFAULT_N = 30;
const SIM_MC_DEFAULT_DEPTH = 5;
const SIM_MC_DEFAULT_ROLLOUT = 'heuristic-1-ply';
const SIM_MC_N_MIN = 10;
const SIM_MC_N_MAX = 200;
const SIM_MC_DEPTH_MIN = 3;
const SIM_MC_DEPTH_MAX = 20;
const SIM_MC_ROLLOUT_OPTIONS = [
  { value: 'heuristic-1-ply', label: '1-Ply Heuristic' },
  { value: 'random',          label: 'Random' },
];
const SIM_HARDWARE_CONCURRENCY =
  (typeof navigator !== 'undefined' && navigator.hardwareConcurrency) ? navigator.hardwareConcurrency : 4;
const SIM_MC_DEFAULT_WORKERS = Math.max(1, SIM_HARDWARE_CONCURRENCY - 1);

const simPercentile = (sortedArr, p) => {
  if (sortedArr.length === 0) return 0;
  const idx = Math.min(sortedArr.length - 1, Math.max(0, Math.ceil((p / 100) * sortedArr.length) - 1));
  return sortedArr[idx];
};

const computeSimAggregates = (games, target) => {
  const n = games.length;
  if (n === 0) return null;
  const wins = games.filter(g => g.outcome === 'win');
  const losses = games.filter(g => g.outcome === 'loss');
  const stucks = games.filter(g => g.outcome === 'stuck');
  const scores = games.map(g => g.finalScore).slice().sort((a, b) => a - b);
  const movesToWin = wins.map(g => g.movesUsed);
  const movesInLoss = losses.map(g => g.movesUsed);
  const avg = (arr) => arr.length === 0 ? 0 : arr.reduce((s, x) => s + x, 0) / arr.length;
  ...
  return {
    ...
    stuckRate: stucks.length / n,
    meanScore: avg(scores),
    medianScore: simPercentile(scores, 50),
    p10Score: simPercentile(scores, 10),
    p90Score: simPercentile(scores, 90),
    minScore: scores[0],
    maxScore: scores[scores.length - 1],
    avgMovesToWin: avg(movesToWin),
    avgMovesInLoss: avg(movesInLoss),
    avgMaxCombo: avg(games.map(g => g.maxCombo)),
    bestMaxCombo: Math.max(0, ...games.map(g => g.maxCombo)),
    avgSpecialsCreated: avg(games.map(g => g.specialsCreated)),
    pctEarnedBonus: games.filter(g => g.bonusMovesEarned > 0).length / n,
    maxBonusEarned: Math.max(0, ...games.map(g => g.bonusMovesEarned)),
    totalRuntimeMs: sum(games.map(g => g.runtimeMs)),
    avgGameMs: avg(games.map(g => g.runtimeMs)),
  };
};

const bucketizeScores = (games, target) => {
  if (games.length === 0) return { bucketSize: 500, buckets: [] };
  const scores = games.map(g => g.finalScore);
  const min = Math.min(...scores);
  const max = Math.max(...scores);
  const range = max - min;
  const bucketSize = range >= 10000 ? 1000 : 500;
  const bucketStart = Math.floor(min / bucketSize) * bucketSize;
  const bucketEnd = Math.ceil((max + 1) / bucketSize) * bucketSize;
  const bucketCount = Math.max(1, (bucketEnd - bucketStart) / bucketSize);
  const buckets = [];
  for (let i = 0; i < bucketCount; i++) {
    const lo = bucketStart + i * bucketSize;
    const hi = lo + bucketSize;
    buckets.push({ lo, hi, count: 0, isWinSide: lo >= target });
  }
  for (const g of games) {
    const i = Math.min(buckets.length - 1, Math.floor((g.finalScore - bucketStart) / bucketSize));
    buckets[i].count++;
  }
  return { bucketSize, bucketStart, bucketEnd, buckets };
};
...
const formatInt = (x) => Math.round(x).toLocaleString();
const formatScore = (x) => Math.round(x).toLocaleString();

// Score-histogram column: renders one vertical bar. Kept inline as a
// function component so the target-line overlay (absolute-positioned over
// the bar row) can layer on top without a container-query dance.
const SimHistogramColumn = ({ bucket, maxCount, target }) => {
  const pctHeight = maxCount === 0 ? 0 : (bucket.count / maxCount) * 100;
  const isTargetBucket = target >= bucket.lo && target < bucket.hi;
  const fill = bucket.isWinSide ? '#4fc3f7' : '#e57373';
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: 0 }}>
      <div style={{ fontSize: '10px', color: '#ccc', height: '12px', lineHeight: '12px' }}>
        {bucket.count > 0 ? bucket.count : ''}
      </div>
      <div style={{ width: '100%', height: '100px', display: 'flex', alignItems: 'flex-end', padding: '0 1px', boxSizing: 'border-box' }}>
        <div style={{ width: '100%', height: `${pctHeight}%`, background: fill, borderRadius: '2px 2px 0 0', minHeight: bucket.count > 0 ? '2px' : '0' }} />
      </div>
      <div style={{ fontSize: '9px', color: isTargetBucket ? '#ffd54f' : '#999', marginTop: '3px', whiteSpace: 'nowrap' }}>
        {(bucket.lo / 1000).toFixed(1) + 'k'}
      </div>
    </div>
  );
};

const readIntSetting = (key, fallback, min, max) => {
...
const SimBatchRunner = () => {
  const [bot, setBot] = useState('heuristic-1-ply');
  const [target, setTarget] = useState(() => readIntSetting(SIM_TARGET_KEY, SIM_DEFAULT_TARGET, 1));
  const [moves, setMoves] = useState(() => readIntSetting(SIM_MOVES_KEY, SIM_DEFAULT_MOVES, 1));
  const [mcN, setMcN] = useState(() => readIntSetting(SIM_MC_N_KEY, SIM_MC_DEFAULT_N, SIM_MC_N_MIN, SIM_MC_N_MAX));
  const [mcDepth, setMcDepth] = useState(() => readIntSetting(SIM_MC_DEPTH_KEY, SIM_MC_DEFAULT_DEPTH, SIM_MC_DEPTH_MIN, SIM_MC_DEPTH_MAX));
  const [mcRollout, setMcRollout] = useState(() => {
    const stored = sessionStorage.getItem(SIM_MC_ROLLOUT_KEY);
    return SIM_MC_ROLLOUT_OPTIONS.some(o => o.value === stored) ? stored : SIM_MC_DEFAULT_ROLLOUT;
  });
  const [mcWorkers, setMcWorkers] = useState(() =>
    readIntSetting(SIM_MC_WORKERS_KEY, SIM_MC_DEFAULT_WORKERS, 1, SIM_HARDWARE_CONCURRENCY)
  );
  ...
  const mcFinishRef = useRef(null);

  useEffect(() => { sessionStorage.setItem(SIM_TARGET_KEY, String(target)); }, [target]);
  useEffect(() => { sessionStorage.setItem(SIM_MOVES_KEY, String(moves)); }, [moves]);
  useEffect(() => { sessionStorage.setItem(SIM_MC_N_KEY, String(mcN)); }, [mcN]);
  useEffect(() => { sessionStorage.setItem(SIM_MC_DEPTH_KEY, String(mcDepth)); }, [mcDepth]);
  useEffect(() => { sessionStorage.setItem(SIM_MC_ROLLOUT_KEY, mcRollout); }, [mcRollout]);
  useEffect(() => { sessionStorage.setItem(SIM_MC_WORKERS_KEY, String(mcWorkers)); }, [mcWorkers]);

  // 1-Ply Heuristic: fast enough in-thread (~12ms/game); sync setTimeout(0) loop.
  const run1PlyBatch = async (size) => {
    const games = [];
    for (let i = 0; i < size; i++) {
      if (cancelRef.current) break;
      const result = _SIM.runGame({ bot: 'heuristic-1-ply', target, moves });
      games.push(result);
      setProgress({ done: i + 1, total: size });
      // eslint-disable-next-line no-await-in-loop
      await new Promise(resolve => setTimeout(resolve, 0));
    }
    return { games, botParams: null };
  };

  // Monte Carlo: Web Worker pool. Each worker runs one game at a time,
  // posts result, then receives the next dispatched game until the batch
  // completes. Cancel terminates all workers and force-resolves the
  // pending promise (terminate() fires no event, so we need an out-of-band
  // finish signal via mcFinishRef).
  const runMcBatch = async (size) => {
    const poolSize = Math.min(mcWorkers, size);
    const botParams = { n: mcN, depthCap: mcDepth, rolloutStrategy: mcRollout, workers: mcWorkers };
    const games = [];
    let dispatched = 0;
    let completed = 0;
    ...
    await new Promise((resolve) => {
      ...
      const finish = () => {
        if (resolved) return;
        resolved = true;
        for (const w of workersRef.current) { try { w.terminate(); } catch (_) { /* ignore */ } }
        workersRef.current = [];
        mcFinishRef.current = null;
        resolve();
      };
      mcFinishRef.current = finish;

      const dispatchTo = (worker) => {
        if (cancelRef.current || dispatched >= size) return;
        const id = dispatched++;
        worker.postMessage({ type: 'run', id, bot: 'monte-carlo', target, moves, botParams });
      };
      ...
      for (let i = 0; i < poolSize; i++) {
        ...
        worker.addEventListener('message', (ev) => {
          ...
          const msg = ev.data;
          if (msg?.type === 'result') {
            games.push(msg.result);
          } else if (msg?.type === 'error') {
            console.error('[sim-worker]', msg.message);
          }
          completed++;
          setProgress({ done: completed, total: size });
          if (completed >= size) finish();
          else dispatchTo(worker);
        });
        worker.addEventListener('error', (err) => {
          console.error('[sim-worker runtime]', err.message || err);
        });
        workers.push(worker);
      }
      workersRef.current = workers;
      workers.forEach(w => dispatchTo(w));
    });

    return { games, botParams };
  };
  ...
  const runBatch = async (size) => {
    ...
    const { games, botParams } = bot === 'monte-carlo'
      ? await runMcBatch(size)
      : await run1PlyBatch(size);

    if (!cancelRef.current) {
      const aggregates = computeSimAggregates(games, target);
      const metadata = {
      ...
      if (botParams) metadata.botParams = botParams;
      setLastBatch({ metadata, aggregates, games });
      setTableExpanded(size <= 100);
    }
    setIsRunning(false);
  ...
  const cancel = () => {
    ...
    if (mcFinishRef.current) mcFinishRef.current();
  };
  ...
  const copyJson = async () => {
    ...
    try {
      ...
      console.warn('[sim] clipboard write failed', err);
    }
  };
  ...
  const S = {
    ...
    row: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 0', fontSize: '13px', flexWrap: 'wrap', gap: '8px' },
    input: { background: '#0f0f0f', color: '#e0e0e0', border: '1px solid #333', borderRadius: '4px', padding: '6px 10px', fontSize: '13px', fontFamily: 'monospace', width: '100px' },
    select: { background: '#0f0f0f', color: '#e0e0e0', border: '1px solid #333', borderRadius: '4px', padding: '6px 10px', fontSize: '13px', fontFamily: 'monospace' },
    btn: { padding: '8px 16px', borderRadius: '6px', border: 'none', cursor: 'pointer', fontSize: '12px', fontFamily: 'monospace', fontWeight: 'bold' },
    presetBtn: (isActive, isDisabled) => ({
      padding: '6px 14px',
      borderRadius: '6px',
      border: isActive ? '2px solid #4fc3f7' : '1px solid #333',
      background: isActive ? '#0d1b3e' : (isDisabled ? '#151515' : '#1e1e1e'),
      color: isActive ? '#4fc3f7' : (isDisabled ? '#555' : '#ccc'),
    ...
    statRow: { display: 'flex', justifyContent: 'space-between', padding: '4px 0', fontSize: '12px', borderBottom: '1px dotted #2a2a2a' },
    statKey: { color: '#ccc' },
    statVal: { color: '#e0e0e0', fontFamily: 'monospace' },
    groupLabel: { color: '#ffd54f', fontSize: '10px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '1px', marginTop: '10px', marginBottom: '4px' },
  };

  const agg = lastBatch?.aggregates;
  const hist = lastBatch ? bucketizeScores(lastBatch.games, target) : null;
  const maxBucketCount = hist ? Math.max(1, ...hist.buckets.map(b => b.count)) : 1;

  // Target line x-position within the histogram bars row (percentage across all buckets).
  let targetLinePct = null;
  if (hist && hist.buckets.length > 0) {
    const totalSpan = hist.bucketEnd - hist.bucketStart;
    targetLinePct = Math.max(0, Math.min(100, ((target - hist.bucketStart) / totalSpan) * 100));
  }

  return (
    <div style={S.card}>
      <span style={S.label}>🤖 Simulation Batch Runner</span>

      <div style={S.row}>
        <span style={{ color: '#ccc' }}>Bot</span>
        <select
          value={bot}
          ...
          style={S.select}
        >
          <option value="heuristic-1-ply">1-Ply Heuristic</option>
          <option value="monte-carlo">Monte Carlo</option>
        </select>
      </div>

      {bot === 'monte-carlo' && (
        <div style={{ background: '#141414', border: '1px solid #2a2a2a', borderRadius: '6px', padding: '10px 12px', margin: '4px 0 8px' }}>
          <div style={{ color: '#ffd54f', fontSize: '10px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '6px' }}>
            MC Parameters
          </div>
          <div style={S.row}>
            <span style={{ color: '#ccc' }}>N <span style={{ color: '#888', fontSize: '11px' }}>(playouts / swap, {SIM_MC_N_MIN}–{SIM_MC_N_MAX})</span></span>
            <input
              type="number"
              value={mcN}
              onChange={(e) => setMcN(Math.max(SIM_MC_N_MIN, Math.min(SIM_MC_N_MAX, parseInt(e.target.value, 10) || SIM_MC_DEFAULT_N)))}
              disabled={isRunning}
              style={S.input}
              min={SIM_MC_N_MIN}
              max={SIM_MC_N_MAX}
          ...
          <div style={S.row}>
            <span style={{ color: '#ccc' }}>Depth-cap <span style={{ color: '#888', fontSize: '11px' }}>(moves / rollout, {SIM_MC_DEPTH_MIN}–{SIM_MC_DEPTH_MAX})</span></span>
            <input
              type="number"
              value={mcDepth}
              onChange={(e) => setMcDepth(Math.max(SIM_MC_DEPTH_MIN, Math.min(SIM_MC_DEPTH_MAX, parseInt(e.target.value, 10) || SIM_MC_DEFAULT_DEPTH)))}
              disabled={isRunning}
              style={S.input}
              min={SIM_MC_DEPTH_MIN}
              max={SIM_MC_DEPTH_MAX}
              step={1}
            />
          </div>
          <div style={S.row}>
            <span style={{ color: '#ccc' }}>Rollout strategy</span>
            <select
              value={mcRollout}
              onChange={(e) => setMcRollout(e.target.value)}
              disabled={isRunning}
              style={S.select}
            >
              {SIM_MC_ROLLOUT_OPTIONS.map(o => (
                <option key={o.value} value={o.value}>{o.label}</option>
              ))}
            </select>
          </div>
          <div style={S.row}>
            <span style={{ color: '#ccc' }}>Workers <span style={{ color: '#888', fontSize: '11px' }}>(1–{SIM_HARDWARE_CONCURRENCY})</span></span>
            <input
              type="number"
              value={mcWorkers}
              onChange={(e) => setMcWorkers(Math.max(1, Math.min(SIM_HARDWARE_CONCURRENCY, parseInt(e.target.value, 10) || SIM_MC_DEFAULT_WORKERS)))}
              disabled={isRunning}
              style={S.input}
              min={1}
              max={SIM_HARDWARE_CONCURRENCY}
              step={1}
            />
          </div>
        </div>
      ...
      <div style={S.row}>
        <span style={{ color: '#ccc' }}>Target score</span>
        <input
          type="number"
          value={target}
          onChange={(e) => setTarget(Math.max(1, parseInt(e.target.value, 10) || SIM_DEFAULT_TARGET))}
          disabled={isRunning}
          style={S.input}
          min={1}
          step={100}
      ...
      <div style={S.row}>
        <span style={{ color: '#ccc' }}>Moves per game</span>
        <input
          type="number"
          value={moves}
          onChange={(e) => setMoves(Math.max(1, parseInt(e.target.value, 10) || SIM_DEFAULT_MOVES))}
          disabled={isRunning}
          style={S.input}
          min={1}
          step={1}
      ...
      <div style={{ ...S.row, marginTop: '6px' }}>
        <span style={{ color: '#ccc' }}>Batch size</span>
        <div>
          {SIM_PRESETS.map(n => (
            <button
              key={n}
              style={S.presetBtn(activePreset === n, isRunning)}
              onClick={() => runBatch(n)}
              disabled={isRunning}
            >
              {n}
      ...
      {isRunning && (
        <div style={{ ...S.row, marginTop: '4px', background: '#0d1b3e', border: '1px solid #1e3a5f', borderRadius: '4px', padding: '8px 12px' }}>
      ...
      {agg && !isRunning && (
        <>
          <div style={{ background: '#0d1b3e', border: '1px solid #1e3a5f', borderRadius: '4px', padding: '8px 12px', marginTop: '10px', marginBottom: '10px' }}>
            <div style={{ color: '#90caf9', fontSize: '12px', fontFamily: 'monospace' }}>
              {lastBatch.metadata.bot === 'monte-carlo'
                ? `Monte Carlo — N=${lastBatch.metadata.botParams?.n}, depth ${lastBatch.metadata.botParams?.depthCap}, ${SIM_MC_ROLLOUT_OPTIONS.find(o => o.value === lastBatch.metadata.botParams?.rolloutStrategy)?.label || lastBatch.metadata.botParams?.rolloutStrategy} rollouts, ${lastBatch.metadata.botParams?.workers} workers`
                : '1-Ply Heuristic'}
            </div>
            <div style={{ color: '#888', fontSize: '11px', fontFamily: 'monospace', marginTop: '2px' }}>
              {lastBatch.metadata.n} games · target {lastBatch.metadata.target.toLocaleString()} · {lastBatch.metadata.moves} moves · win rate {formatPct(agg.winRate)}
            </div>
          </div>

          <div style={S.groupLabel}>Outcomes · n = {agg.n}</div>
          <div style={S.statRow}><span style={S.statKey}>Win rate</span><span style={S.statVal}>{formatPct(agg.winRate)}</span></div>
          <div style={S.statRow}><span style={S.statKey}>Loss rate</span><span style={S.statVal}>{formatPct(agg.lossRate)}</span></div>
          <div style={S.statRow}><span style={S.statKey}>Stuck rate</span><span style={S.statVal}>{formatPct(agg.stuckRate)}</span></div>

          <div style={S.groupLabel}>Score</div>
          <div style={S.statRow}><span style={S.statKey}>Mean</span><span style={S.statVal}>{formatScore(agg.meanScore)}</span></div>
          <div style={S.statRow}><span style={S.statKey}>Median (p50)</span><span style={S.statVal}>{formatScore(agg.medianScore)}</span></div>
          <div style={S.statRow}><span style={S.statKey}>p10 · p90</span><span style={S.statVal}>{formatScore(agg.p10Score)} · {formatScore(agg.p90Score)}</span></div>
          <div style={S.statRow}><span style={S.statKey}>Range</span><span style={S.statVal}>{formatScore(agg.minScore)} → {formatScore(agg.maxScore)}</span></div>

          <div style={S.groupLabel}>Moves</div>
          <div style={S.statRow}><span style={S.statKey}>Avg moves to win (winners)</span><span style={S.statVal}>{agg.avgMovesToWin.toFixed(1)}</span></div>
          <div style={S.statRow}><span style={S.statKey}>Avg moves used (losers)</span><span style={S.statVal}>{agg.avgMovesInLoss.toFixed(1)}</span></div>

          <div style={S.groupLabel}>Cascades</div>
          <div style={S.statRow}><span style={S.statKey}>Avg maxCombo / game</span><span style={S.statVal}>{agg.avgMaxCombo.toFixed(2)}</span></div>
          <div style={S.statRow}><span style={S.statKey}>Best single-game maxCombo</span><span style={S.statVal}>{agg.bestMaxCombo}</span></div>

          <div style={S.groupLabel}>Specials</div>
          <div style={S.statRow}><span style={S.statKey}>Avg specials created / game</span><span style={S.statVal}>{agg.avgSpecialsCreated.toFixed(2)}</span></div>

          <div style={S.groupLabel}>Bonus moves</div>
          <div style={S.statRow}><span style={S.statKey}>% games earning ≥1</span><span style={S.statVal}>{formatPct(agg.pctEarnedBonus)}</span></div>
          <div style={S.statRow}><span style={S.statKey}>Max single-game earned</span><span style={S.statVal}>{agg.maxBonusEarned}</span></div>

          <div style={S.groupLabel}>Runtime</div>
          <div style={S.statRow}><span style={S.statKey}>Total batch</span><span style={S.statVal}>{formatMs(agg.totalRuntimeMs)}</span></div>
          <div style={S.statRow}><span style={S.statKey}>Avg / game</span><span style={S.statVal}>{formatMs(agg.avgGameMs)}</span></div>

          {hist && hist.buckets.length > 0 && (
            <>
              <div style={S.groupLabel}>Score histogram · {hist.bucketSize}-pt buckets · target line dashed</div>
              <div style={{ position: 'relative', padding: '6px 0 2px' }}>
                <div style={{ display: 'flex', alignItems: 'flex-end', gap: '0', padding: '0 2px' }}>
                  {hist.buckets.map((b, i) => (
                    <SimHistogramColumn key={i} bucket={b} maxCount={maxBucketCount} target={target} />
                  ))}
                </div>
                {targetLinePct !== null && (
                  <div style={{
                    position: 'absolute',
                    left: `calc(2px + ${targetLinePct}% * (100% - 4px) / 100%)`,
                    top: '12px',
                    bottom: '18px',
                    width: 0,
                    borderLeft: '2px dashed #ffd54f',
                    pointerEvents: 'none',
                  }} />
                )}
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-around', fontSize: '9px', color: '#888', padding: '0 2px', marginTop: '2px' }}>
                <span>p10 {formatScore(agg.p10Score)}</span>
                <span>median {formatScore(agg.medianScore)}</span>
                <span>p90 {formatScore(agg.p90Score)}</span>
              </div>
            </>
          )}

          <div style={{ ...S.groupLabel, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span>Per-game detail ({lastBatch.games.length})</span>
            <button
              style={{ ...S.btn, background: '#2a2a2a', color: '#ccc', padding: '4px 10px', fontSize: '11px' }}
          ...
          {tableExpanded && (
            <div style={{ maxHeight: '240px', overflowY: 'auto', border: '1px solid #2a2a2a', borderRadius: '4px', background: '#0f0f0f' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px', fontFamily: 'monospace', color: '#ccc' }}>
                <thead style={{ position: 'sticky', top: 0, background: '#1a1a1a' }}>
                  <tr>
                    <th style={{ textAlign: 'left', padding: '6px 8px', borderBottom: '1px solid #2a2a2a' }}>#</th>
                    <th style={{ textAlign: 'left', padding: '6px 8px', borderBottom: '1px solid #2a2a2a' }}>Outcome</th>
                    <th style={{ textAlign: 'right', padding: '6px 8px', borderBottom: '1px solid #2a2a2a' }}>Score</th>
                    <th style={{ textAlign: 'right', padding: '6px 8px', borderBottom: '1px solid #2a2a2a' }}>Moves</th>
                    <th style={{ textAlign: 'right', padding: '6px 8px', borderBottom: '1px solid #2a2a2a' }}>maxCombo</th>
                    <th style={{ textAlign: 'right', padding: '6px 8px', borderBottom: '1px solid #2a2a2a' }}>Specials</th>
                    <th style={{ textAlign: 'right', padding: '6px 8px', borderBottom: '1px solid #2a2a2a' }}>Bonus</th>
                    <th style={{ textAlign: 'right', padding: '6px 8px', borderBottom: '1px solid #2a2a2a' }}>ms</th>
                  </tr>
                </thead>
                <tbody>
                  {lastBatch.games.map((g, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid #1e1e1e' }}>
                      <td style={{ padding: '4px 8px', color: '#888' }}>{i + 1}</td>
                      <td style={{ padding: '4px 8px', color: g.outcome === 'win' ? '#81c784' : (g.outcome === 'stuck' ? '#ffb74d' : '#e57373') }}>{g.outcome}</td>
                      <td style={{ padding: '4px 8px', textAlign: 'right' }}>{formatScore(g.finalScore)}</td>
                      <td style={{ padding: '4px 8px', textAlign: 'right' }}>{g.movesUsed}</td>
                      <td style={{ padding: '4px 8px', textAlign: 'right' }}>{g.maxCombo}</td>
                      <td style={{ padding: '4px 8px', textAlign: 'right' }}>{g.specialsCreated}</td>
                      <td style={{ padding: '4px 8px', textAlign: 'right' }}>{g.bonusMovesEarned}</td>
                      <td style={{ padding: '4px 8px', textAlign: 'right', color: '#888' }}>{formatInt(g.runtimeMs)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div style={{ marginTop: '10px', display: 'flex', justifyContent: 'flex-end' }}>
            <button
              style={{ ...S.btn, background: copyFlash ? '#1b3a1a' : '#0d1b3e', color: copyFlash ? '#81c784' : '#90caf9' }}
              onClick={copyJson}
            >
...
const Match3Sim = () => {
  ...
  // v11.2: Banked moves — persisted across games
  const [bankedMoves, setBankedMoves] = useState(() => parseInt(localStorage.getItem(BONUS_MOVES_KEY) || '0', 10));

  // v11.6: True after player chooses "Use bonus moves" — each swap draws from bankedMoves
  const [usingBankedMoves, setUsingBankedMoves] = useState(false);
  // v11.6: True when moves=0 and bankedMoves>0, shows the bonus moves decision prompt
  ...
  const usingBankedMovesRef = useRef(false); // v11.6: ref mirror for attemptSwap callbacks
  ...
  // v11.2: Persist banked moves whenever they change
  useEffect(() => {
    localStorage.setItem(BONUS_MOVES_KEY, bankedMoves.toString());
  }, [bankedMoves]);
  ...
  useEffect(() => {
    ...
    if (showBonusPrompt) return; // Don't check while bonus prompt is showing
    if (showBankedMovesPrompt) return; // v11.6: Don't check while bonus moves prompt is showing
    ...
    const checkTimer = setTimeout(() => {
      ...
      // v8.10 / v11.6: Bonus round ended — check bankedMoves before resolving
      ...
      // v11.6: moves = 0, not in bonus round — check bankedMoves before resolving (win OR fail)
  ...
  }, [moves, gameState, levelTarget, highScore, isAnimating, combo, targetReached, pendingSpecials.length, grid, turnComplete, bonusRoundActive, showBonusPrompt, showBankedMovesPrompt, bankedMoves, usingBankedMoves]);
  ...
  useEffect(() => {
    ...
    if (threshold > 0 && threshold > bonusMoveThresholdRef.current) {
      ...
      // v11.6: Earned bonus moves go directly into bankedMoves (persistent across games)
  ...
  // v11.6: Player chose "Use bonus moves" — each subsequent swap draws from bankedMoves
  ...
  // v11.6: Player chose "End and carry moves forward" — end game, bankedMoves persist naturally
  ...
  // win. The bonus-moves prompt's End button (in showBankedMovesPrompt)
  // is unchanged because that's a save after an already-failed turn.
  // Mirrors campaign v1.25.
  const requestEndLevelCarryBanked = () => {
    const { bonus: specialsBonus } = calculateUnusedSpecialsBonus(grid);
  ...
  const attemptSwap = (row1, col1, row2, col2) => {
    ...
    if (tile1Special && tile2Special) {
      if (usingBankedMovesRef.current) setBankedMoves(prev => Math.max(0, prev - 1));
  ...
  const processMatches = (currentGrid, matchGroups, lShapeMatches, currentCombo, generation = 0, connectedGroups = [], swapPosition = null) => {
    ...
    matchGroups.forEach(group => {
      ...
      const final = bonusRoundActive ? Math.floor(pointsForThisGroup * BONUS_ROUND_MULTIPLIER) : pointsForThisGroup;
      recordScoringEvent({
        type: generation === 0 ? 'match' : 'cascade-match',
        description,
        points: final,
        multiplier: multiplier * (bonusRoundActive ? BONUS_ROUND_MULTIPLIER : 1),
    ...
    if (lShapeMatches && lShapeMatches.length > 0) {
      ...
      const lShapeFinal = bonusRoundActive ? Math.floor(lShapeTotal * BONUS_ROUND_MULTIPLIER) : lShapeTotal;
      recordScoringEvent({
        type: 'l-shape-bonus',
        description: `L-shape bonus (×${lShapeMatches.length})`,
        points: lShapeFinal,
        multiplier: bonusRoundActive ? BONUS_ROUND_MULTIPLIER : 1,
  ...
  const activateSpecialCombination = (row1, col1, row2, col2, type1, type2, currentGrid, additionalGroups = [], swapPosition = null) => {
    ...
    const comboBaseFinal = bonusRoundActive ? Math.floor(points * BONUS_ROUND_MULTIPLIER) : points;
    recordScoringEvent({
      type: 'special-combo',
    ...
    if (cascadePoints > 0) {
      const cascadeFinal = bonusRoundActive ? Math.floor(cascadePoints * BONUS_ROUND_MULTIPLIER) : cascadePoints;
      recordScoringEvent({
        type: 'combo-cascade',
  ...
  const removeMatches = (currentGrid, matchGroups, lShapeMatches, generation, connectedGroups = [], swapPosition = null) => {
    ...
    const processSpecialWithCascade = (special, depth, sourceSpecial = null, staggerIndex = 0) => {
      ...
      if (result.points > 0) {
        const finalPointsForEvent = bonusRoundActive ? Math.floor(multipliedPoints * BONUS_ROUND_MULTIPLIER) : multipliedPoints;
        ...
        recordScoringEvent({
          ...
          multiplier: cascadeMultiplier * (bonusRoundActive ? BONUS_ROUND_MULTIPLIER : 1),
  ...
  const restartGame = () => {
    ...
    bonusMoveFlashPendingRef.current = 0;
    setBonusMoveFlash(0);
    // v11.6: Reset bonus moves flow (bankedMoves persists — not reset here)
    usingBankedMovesRef.current = false;
    setUsingBankedMoves(false);
    setShowBankedMovesPrompt(false);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <h1 style={{ margin: '0', color: '#333', fontSize: '20px', textShadow: '2px 2px 4px rgba(0,0,0,0.1)' }}>
          🤖 Match-3 Sim <span style={{ fontSize: '12px', color: '#ccc' }}>v1.6 (base: tablet v11.16)</span>
        </h1>
        <div style={{
          display: 'flex',
          ...
          {bankedMoves > 0 && (
            <div title="Bonus moves carry forward to future games" style={{ position: 'relative' }}>
              Bonus moves:{' '}
              <span style={{
                color: bankedMoves >= BANKED_MOVES_CAP  ? '#d32f2f' :
          ...
          {/* v11.3: "End level" button — visible while using extra moves
              v11.9: routes through requestEndLevelCarryBanked so an
              are-you-sure confirm fires if the player is below target. */}
          {usingBankedMoves && (
            <button
              onClick={requestEndLevelCarryBanked}
              style={{
                padding: '4px 12px',
                fontSize: '13px',
                background: '#667eea',
      ...
      {/* v11.6: Bonus Moves Prompt — fires when moves=0 and bankedMoves>0, win OR fail */}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ fontSize: '14px', color: 'rgba(255,255,255,0.9)', lineHeight: 1.5, marginBottom: '20px' }}>
              ...
              with {bankedMoves} bonus move{bankedMoves !== 1 ? 's' : ''} still available.
              <br />
      ...
      {showAdmin && (
        <TabletAdminWrapper
          ...
          constants={{
            ...
            MIN_MOVES, MAX_MOVES,
            BONUS_MOVE_INTERVAL,
            BANKED_MOVES_CAP, BANKED_MOVES_WARN,
          }}
        />
      )}
