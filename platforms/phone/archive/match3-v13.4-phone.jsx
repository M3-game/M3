...
// =============================================================================
// MATCH-3 GAME v13.4 - PHONE (JSX) (T-2 terminology rename)
//
// Session T-2 (2026-05-02): Bundled terminology rename. "Bonus Round" →
// "Victory Round" (the 1.5× multiplier phase) and `bankedMoves` →
// `bonusMoves` (the carry pool of earned +1 moves). T-2 completes the
// sweep on phone arcade — was deferred from T-1 because the cross-
// platform key share with phone-verses required coordinated handling
// (T-1 renamed phone-verses but left the value-side alone; T-3 will
// migrate the value across both files in one event).
//
// Phone-arcade-specific changes:
...
//   `BONUS_MOVES_KEY` value 'match3_phone418_bankedMoves' kept (matches
//   phone-verses' `PHONE_BONUS_MOVES_KEY` value verbatim — they share
//   one localStorage key for the bonus-moves pool, with verses → arcade
//   carry-out depending on the value match). T-3 migrates this and all
//   other `bankedMoves` storage strings as one coordinated lockstep
//   event. Phone arcade and phone-verses MUST migrate together — this
//   is captured as a hard sync requirement in DEFERRED.md
//   "Cross-platform terminology + naming sweep".
//
...
const BONUS_MOVES_KEY = 'match3_phone418_bankedMoves';
...
const RUN_CURRENT_KEY = 'match3_phone418_currentRun';
const RUN_LONGEST_KEY = 'match3_phone418_longestRun';
...
const Match3Game = () => {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem('m3_arcade_carry_from_verses_phone418');
      if (!raw) return;
      ...
      if (amount > 0) {
        ...
        localStorage.removeItem('m3_arcade_carry_from_verses_phone418');
      ...
      try { localStorage.removeItem('m3_arcade_carry_from_verses_phone418'); } catch {}
    }
  }, []);
      ...
      >
        ...
        >
          ...
          <h1 style={{ margin: '0', color: '#333', fontSize: '15px', fontWeight: 'bold' }}>
            M3 <span style={{ fontSize: '9px', color: '#999', fontWeight: 'normal' }}>v13.4</span>
          </h1>
