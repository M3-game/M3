...
//   - "Banked" / "bonus round" terminology — phone arcade is a deferred
...
//   - All "banked" / "bonus round" terminology — phone arcade is
...
// adds bankedMoves into PHONE418_BANKED_MOVES_KEY and writes the
// receipt key, then navigates to phone.html.
//
// Banker-side bonus-moves pool: PHONE418_BANKED_MOVES_KEY is
// `match3_phone418_bankedMoves` (already this file's BANKED_MOVES_KEY
// constant), so the carry pool feeds directly into phone arcade's
// existing banked-moves storage on the next mount.
//
// =============================================================================
...
// "banked" naming (BANKED_MOVES_KEY, bankedMoves, etc.) intentionally
...
// v12.0: Unified bonus/banked moves — persistent 🏦 pool across games
//   Bonus moves earned during play (per 10k pts) go into a persistent bankedMoves
//   counter (localStorage) instead of regular moves. When regular moves hit 0 and
//   bankedMoves > 0, a prompt offers "Use banked moves" (keep playing, 🏦 ticks
//   down one per swap, regular moves stays at 0) or "End and carry moves forward"
//   (game ends as a loss, all remaining 🏦 carries to next game). Banked moves
...
//   - BANKED_MOVES_CAP = 99 hard cap on bonus moves earned.
//     Header counter turns amber at ≥ BANKED_MOVES_WARN (90), red at cap.
...
// v12.0: localStorage key for banked moves (persistent across games)
const BANKED_MOVES_KEY = 'match3_phone418_bankedMoves';

// v12.4: Bonus-move cap + warn threshold (from tablet v11.9 / campaign v1.25).
const BANKED_MOVES_CAP  = 99; // hard cap on stored moves
const BANKED_MOVES_WARN = 90; // counter turns amber at/above this; red at cap
...
const Match3Game = () => {
  ...
  // v12.0: Banked moves — persistent across games
  ...
  const usingBankedMovesRef = useRef(false);
  ...
  // v12.0: Persist banked moves and keep ref in sync
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (threshold > 0 && threshold > bonusMoveThresholdRef.current) {
      ...
      // v12.4: Clamp to BANKED_MOVES_CAP — moves earned past the cap are dropped silently.
      setBankedMoves(prev => Math.min(prev + newMoves, BANKED_MOVES_CAP));
      bankedMovesRef.current = Math.min(bankedMovesRef.current + newMoves, BANKED_MOVES_CAP);
      if (showBonusPrompt) {
  ...
  // v12.0: Added banked moves check — offer prompt before true game over
  useEffect(() => {
    ...
    if (showBankedMovesPrompt) return; // v12.0: Don't check while banked prompt is showing
    ...
    const checkTimer = setTimeout(() => {
      ...
      // v8.10 / v12.2: Bonus round ended — check bankedMoves before resolving
      ...
      // v12.2: moves = 0, not in bonus round — check bankedMoves before resolving (win OR fail)
      if (moves <= 0 && !bonusRoundActive) {
        if (bankedMoves > 0 && !usingBankedMoves) {
          setShowBankedMovesPrompt(true);
          return;
        }
        if (usingBankedMoves && bankedMoves > 0) return; // still have bonus moves — game continues
  ...
  }, [moves, gameState, levelTarget, highScore, isAnimating, combo, targetReached, pendingSpecials.length, grid, turnComplete, bonusRoundActive, showBonusPrompt, showBankedMovesPrompt, usingBankedMoves, bankedMoves]);
  ...
  // v12.0: Player chose "Use banked moves" — swaps consume from bankedMoves, regular moves stays at 0
  ...
  // v12.2: Player chose "End and carry moves forward" — end game, bankedMoves persist naturally
  ...
  // win. The bonus-moves prompt's End button (in showBankedMovesPrompt) is
  ...
  const attemptSwap = (row1, col1, row2, col2) => {
    ...
    if (tile1Special && tile2Special) {
      // v12.0: When using banked moves, consume from banked pool; regular moves stays at 0
      if (usingBankedMovesRef.current) {
  ...
  const restartGame = () => {
    ...
    bonusMoveFlashPendingRef.current = 0;
    setBonusMoveFlash(0);
    // v12.0: Reset banked move UI state (bankedMoves itself persists — do not reset it)
    setUsingBankedMoves(false);
    usingBankedMovesRef.current = false;
    setShowBankedMovesPrompt(false);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          <h1 style={{ margin: '0', color: '#333', fontSize: '15px', fontWeight: 'bold' }}>
            M3 <span style={{ fontSize: '9px', color: '#999', fontWeight: 'normal' }}>v13.3.1</span>
          </h1>
        ...
        {bankedMoves > 0 && (
          ...
          >
            ...
            <span
              style={{
                color:
                  bankedMoves >= BANKED_MOVES_CAP ? '#d32f2f' :
                  bankedMoves >= BANKED_MOVES_WARN ? '#e65100' :
                  usingBankedMoves ? '#e65100' : '#667eea',
                fontWeight: 'bold'
              }}
            >
              {bankedMoves}
              {bankedMoves >= BANKED_MOVES_WARN && (
                <span
                  style={{
                    fontSize: '10px',
                    color: bankedMoves >= BANKED_MOVES_CAP ? '#d32f2f' : '#e65100',
                    marginLeft: '4px'
                  }}
                >
                  {' '}/ {BANKED_MOVES_CAP}
            ...
            {bankedMoves >= BANKED_MOVES_WARN && (
              ...
              >
                {bankedMoves >= BANKED_MOVES_CAP
                  ? 'Cap reached — no additional moves can be earned'
                  : `Nearing cap — max ${BANKED_MOVES_CAP}`}
              </div>
            )}
          </div>
        )}
        ...
        {/* End-and-carry button (visible during banked-moves use) */}
        {usingBankedMoves && (
          <button
        ...
        >
          <div style={{ textAlign: 'center', color: 'white', minWidth: '160px' }}>
            ...
            <div style={{ fontSize: '14px', color: 'rgba(255,255,255,0.85)' }}>
              {targetReached
                ? `Keep playing with ${bankedMoves} bonus move${bankedMoves !== 1 ? 's' : ''}?`
                : `You have ${bankedMoves} bonus move${bankedMoves !== 1 ? 's' : ''}`}
            </div>
          </div>
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.92)', lineHeight: 1.4, marginBottom: '14px' }}>
              You're below target ({scoreRef.current.toLocaleString()} / {levelTarget.toLocaleString()}) with {bankedMoves} bonus move{bankedMoves !== 1 ? 's' : ''} still available. Your bonus moves carry forward either way.
            </div>
            <button
      ...
      {/* Bonus round prompt (target reached, moves remaining) */}
      {showBonusPrompt && (() => {
        ...
        return (
          ...
          >
            <div style={{ textAlign: 'center', color: '#333', minWidth: '160px' }}>
              ...
              <div style={{ fontSize: '14px', color: '#555' }}>Enter Bonus Round?</div>
            </div>
            <div
              style={{
                textAlign: 'center',
      ...
      {(gameState === 'gameover' || gameState === 'won') && (() => {
        ...
        return (
          ...
          >
            ...
            >
              {bonusRoundScore > 0 && <div>🌟 Bonus Round: +{bonusRoundScore}</div>}
