...
// =============================================================================
// MATCH-3 GAME v1.7 - PHONE VERSES (in-session ship batch)
//
...
//
// In-game header label bumps to "Verses v1.7".
//
...
// v1.7: biblical-order slug list driven by each game's `book` + `chapter`
      ...
      <div style={{
        ...
        <h1 style={{ margin: '0', color: '#333', fontSize: '20px', textShadow: '2px 2px 4px rgba(0,0,0,0.1)' }}>
          {VERSES_MODE && versesGame ? (
            ...
            <>Verses <span style={{ fontSize: '12px', color: '#ccc' }}>v1.7</span></>
          )}
        </h1>
                ...
                buttons.push({
                  ...
                  onClick: () => {
                    ...
                    try {
                      ...
                      localStorage.setItem('m3_arcade_carry_from_verses_phone', JSON.stringify({
                        amount: carryAmount,
...
const VersesPicker = ({ games, slugs, progress, onPick, secretUnlock, onSecretPressStart, onSecretPressEnd }) => {
  return (
    <div style={{
      ...
      {/* v1.7 (#2): long-press 1.5 s on this header to toggle secret-
      ...
      <div style={{
        ...
        {slugs.map(slug => {
          ...
          // v1.7: long titles (>20 chars) shrink to 16px on the card.
...
// v1.7 (#2): secret-unlock localStorage key — read at mount, written on
...
const Match3Verses = () => {
  ...
  // v1.7 (#2): secret-unlock state. ON → every locked level in every
