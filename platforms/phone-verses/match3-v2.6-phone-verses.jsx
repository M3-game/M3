...
// =============================================================================
// MATCH-3 GAME v2.6 — PHONE VERSES (level-select back button overlapped the title)
//
// Player-reported 2026-08-09 with a screenshot of Isaiah 52:13–53:12: on the
// LEVEL-SELECT screen (the per-level cards for a multi-level passage), the
// "← Back to passage selections" button overlapped the passage title.
//
// Cause — a static layout collision, unrelated to the scroll/status-bar work
// in v2.4 and v2.5. The button is absolutely positioned at top:20px and is
// 32px tall (14px text + 12px vertical padding + 4px border), so it occupies
// the vertical band 20–52px. The title is a normal in-flow element, so it
// starts at the container's 40px top padding — 12px inside the button's band.
// The container's top padding was never raised to account for a button that
// had been taken out of the normal flow.
//
...
// screen, and the title is centred — so every title crosses the button's
// column. Isaiah only made it obvious because at 36px Georgia its capitals
// and digits are tall enough to visibly cross the button's lower border.
//
// FIX: container top padding 40px → 68px (the button's 52px lower edge plus
// ~16px of breathing room).
//
// ALSO IN THIS VERSION — preventive safe-area padding on BOTH select screens
// (user-approved 2026-08-09 after the overlap fix was scoped). The passage-
// select and level-select screens carried the same gap the game screen had
// before v2.4: viewport-fit=cover in phone-verses.html runs the page under
// the iOS status bar, and when Safari's bottom toolbar collapses to its pill
// the page shifts upward. Neither screen reserved
// env(safe-area-inset-top), so their top-left controls — the back button
// here, the Tutorial pill on the passage-select screen — could end up behind
// the clock and battery. NOTE this is preventive: it has not been observed,
// and the reported screenshot was taken with Safari's toolbar expanded, the
// state in which the page is NOT shifted up.
//
// Both absolutely-positioned controls needed the inset applied to themselves.
// Raising a container's padding moves in-flow content only — an absolutely
// positioned element is placed from the container's edge and does not move.
// Both wrappers also go 100vh → 100dvh, matching the game screen.
//
// The passage-select screen's own 40px top padding is UNCHANGED: its "Verses"
// heading is short and sits well to the right of the Tutorial pill, so there
// is no collision there to fix — only the inset was added.
//
// TERMINOLOGY: "select", not "picker" (user direction 2026-08-09). The code
// identifier `VersesPicker` is unchanged — renaming it is deferred item 7.
//
// No gameplay, scoring, board-logic, or content change.
//
// =============================================================================
// MATCH-3 GAME v2.5 — PHONE VERSES (vertical layout: balance, fit, and short phones)
//
// Follow-on to the v2.4 header-clipping fix. v2.4 stopped the header being
// cut off on the reporting player's phone; this makes the vertical layout
// correct on phones we can't test, and balances the space on the ones we can.
//
// MEASUREMENTS THIS IS BUILT ON. Every band of a real 428×926pt screenshot
// was measured and scaled to CSS pixels. They sum to 927pt against a real
// screen height of 926pt — that agreement is why the numbers below are
// treated as measured rather than estimated:
//   space above header 29 · header card 94 · text bar (2 lines) 67 ·
//   board container 456 (40pt tiles) · instructions panel 72 ·
//   gaps between elements 45 · dead space below 86 · usable height 848
//
// 1) VERTICAL SPACE REBALANCED. Padding was 8px top / 60px bottom. Centring
//    happens INSIDE the padding, so space above the content is paddingTop +
//    half the leftover slack and space below is paddingBottom + the same
//    half — the 52px difference was the whole reason the game sat high with
//    a band of dead space beneath it (measured 29pt above vs 86pt below).
//    Both are now 25px plus their respective safe-area inset, so the ~117pt
//    of genuinely spare height on this phone splits evenly (~58pt each end).
//    This does not remove the empty space — with the board size fixed, that
//    space has to live somewhere — it balances it. User chose the even split
//    over spreading the slack between the elements (2026-08-09).
//
// 2) OVERFLOW CAN NO LONGER EAT THE HEADER. justifyContent 'center' →
//    'safe center'. Plain centring splits any overflow evenly above and
//    below, and the half above the top of the screen cannot be scrolled to
//    — the same class of failure v2.4 fixed, which would return on any
//    phone too short for the layout. `safe` falls back to top-alignment
//    when content overflows, pushing it to the bottom instead. Browsers
//    without support drop the declaration and land on flex-start, which is
//    the safe direction anyway.
//
// 3) THE INSTRUCTIONS PANEL NOW YIELDS ON SHORT PHONES — BY MEASUREMENT,
//    NOT BY DEVICE TABLE. New `hideInstructions` state: a useLayoutEffect
//    compares the page's rendered height against visualViewport.height
//    after every render, and if the layout genuinely does not fit AND the
//    player has made two moves, the panel goes. On a phone with room to
//    spare it never fires and nothing changes — which matters, because
//    hiding it there would trade a useful panel for a bigger band of empty
//    background (user's point, 2026-08-09). visualViewport rather than
//    window.innerHeight because innerHeight reports the toolbars-hidden
//    height on iOS — the very mismatch behind the v2.4 bug.
//
//    Why two moves: the panel and the growing passage text bar compete for
//    the same space, and the panel (~118pt with its gap) is nearly three
//    times the text bar's total growth (~42pt). So the TALLEST moment of a
//    round is its beginning, not its end, and retiring the panel at move 2
//    means the two never coexist at full size. On a screen too short even
//    for that, the panel simply overflows off the bottom for those first
//    two moves — accepted by the user as the least-bad clipping, since it
//    is the least important element and is about to vanish anyway.
//
// 4) TILE SIZE IS NOW HEIGHT-AWARE. It came from viewport WIDTH only, so a
//    narrow phone got smaller tiles but a SHORT phone got the same tall
//    stack. Now it takes whichever of the width and height budgets is
//    smaller. Priority order agreed with the user: keep the board as large
//    as the width allows, give up the instructions panel next, and shrink
//    tiles only as a last resort — the board is the game. See
//    VERSES_CHROME_PX for why this one value is an estimate and why that is
//    harmless (on every phone bigger than the SE class the width budget
//    still binds, so tile size is unchanged).
//
// EXPECTED EFFECT BY DEVICE. On a 428×926 phone: nothing changes except the
// rebalanced padding — panel stays up all round, tiles stay at 40. iPhone
// 15-class: unchanged tiles, panel likely stays. iPhone 13 mini: unchanged
// tiles, panel retires at move 2. iPhone SE-class: smaller tiles AND the
// panel retires. Only the first of those is verifiable on hardware we have;
// the rest follow from the measured numbers above.
//
// The title-vs-"See passage" crowding and the level-select back-button /
// title overlap remain SEPARATE deferred items and are untouched here.
// No gameplay, scoring, board-logic, or content change.
//
// =============================================================================
// MATCH-3 GAME v2.4 — PHONE VERSES (in-game header clipped at the top — fix)
//
// Bug: on iPhone the top of the white in-game header card was intermittently cut
// off — the card's rounded top corner and part of the "See passage" pill sat
// above the visible area, with the iOS status bar drawn over the title. It came
// and went within a single session with unchanged content, and tapping /
// dismissing Safari's address bar did not restore it.
//
// Cause — the page always had a small amount of scroll room it should never have
// had, and the code made that scroll unrecoverable:
//   1) NO CSS RESET ANYWHERE IN THE PROJECT. No stylesheet file exists and no
//      entry file imports one, so <body> kept the browser default 8px margin,
//      top and bottom. With the page wrapper at minHeight 100vh, the document
//      was permanently 100vh + 16px — taller than the screen on every load,
//      regardless of content.
//   2) 100vh ON iOS SAFARI IS THE *LARGE* VIEWPORT HEIGHT — the height the page
//      would have if the browser toolbars were hidden. Whenever the toolbar is
//      showing, the visible area is shorter than that, adding a further ~60–90px
//      of scroll room on top of (1).
//   3) touchAction: 'none' on the wrapper (there so swipes reach the board, not
//      the page) means that once the page IS scrolled, a drag cannot scroll it
//      back. Nothing in the file resets scrollTop — there are no scrollTo /
//      scrollIntoView / scrollTop calls anywhere.
//   4) viewport-fit=cover + apple-mobile-web-app-status-bar-style:
//      black-translucent (both in phone-verses.html) tell Safari to run the page
//      under the status bar and notch, but no env(safe-area-inset-top) padding
//      was ever added to compensate — a constant ~47–59px overlap.
// The intermittency was simply whether the page happened to be sitting at
// scroll 0. (1)+(2) created the room, (3) made it one-way, (4) put the status
// bar on top of whatever was left.
//
// Fix — three changes, in lockstep with phone-verses-sandbox v1.14:
//   a) BODY MARGIN ZEROED. `<style>html,body{margin:0;padding:0}</style>` added
//      to phone-verses.html (and the sandbox's own HTML). Scoped to these two
//      pages so no other platform's layout shifts. Removes the phantom 16px.
//   b) SAFE-AREA TOP PADDING on the wrapper:
//      calc(env(safe-area-inset-top, 0px) + 8px). Fixes the status-bar overlap.
//      Written as padding longhands so the value can't be clobbered by the
//      shorthand.
//   c) minHeight 100vh → 100dvh. The dynamic viewport unit tracks the height
//      actually visible on iOS instead of the toolbars-hidden height, removing
//      the larger source of scroll room. (dvh: Safari 15.4+, Chrome 108+,
//      Firefox 101+. On a browser without it the declaration is dropped and the
//      purple wrapper sizes to its content — cosmetic, not a break.)
//
// justifyContent: 'center' is deliberately LEFT ALONE. Top-anchoring would make
// clipping structurally impossible, but it also moves the game to the top of the
// screen on every device where it already fits — a visible change to every
// session. Fixing the actual cause first (user direction 2026-08-09).
//
// No gameplay, scoring, board, or content change. The title-vs-"See passage"
// crowding and the level-select back-button/title overlap are SEPARATE issues,
// deliberately not touched here.
//
// =============================================================================
// MATCH-3 GAME v2.3 — PHONE VERSES (title nudge tweak for medium-length titles)
//
// Small visual tweak: the medium title tier (15–20 chars — Isaiah, Habakkuk,
// Genesis 50, 2 Chronicles) read too far right / off-center with the full 48px
// right-nudge added in v2.2. Eased that tier's nudge back to 28px (~40% of the
// way back toward center, per user direction). The very-long tier (> 20 chars,
// "1 Thessalonians 4:13–18") keeps 48px — it needs the full nudge to clear the
// "See passage" button. Font tiers unchanged. In-game header label bumps to
// "Verses v2.3".
//
// =============================================================================
// MATCH-3 GAME v2.2 — PHONE VERSES (port: bonus moves + arcade free-replay + see passage)
//
// Port of three tablet-verses features (v2.2/v2.3/v2.4) to phone-verses in one
// bump, structurally identical to tablet-verses (arcade destination is
// phone.html; the passage peek uses the phone single-column layout):
//   1) USE BONUS MOVES IN VERSES (tablet-verses v2.2). Bonus moves (earned +
//      shown, previously unspendable in verses) are now usable. LOSS (0 moves,
//      below target): the arcade "Use bonus moves / End and carry" banner drops
//      down over the still-visible board instead of jumping to the passage
//      screen. WIN (at/above target): the passage screen gains a "Use bonus
//      moves" button that returns to the board with the 1.5× victory round
//      still on. Decision 4: a victory round already active on a WIN is kept;
//      on a LOSS, crossing target during bonus moves completes the level
//      (stars/unlock) but does NOT switch the 1.5× on (and its toast is
//      suppressed). Touch points: end-of-round effect (banner-vs-passage),
//      3 input-freeze guards (unfreeze during active bonus play),
//      endLevelCarryBonus (VERSES branch → passage screen), resumeBonusFromPassage,
//      passage-screen button matrix (win-path button), target→victory-round
//      effect (loss-path guard).
//   2) ARCADE ON FREE-REPLAY (tablet-verses v2.3). "Arcade mode" button now
//      also shows on the free-replay end screen (any level replay once the
//      passage is fully completed, win or fail).
//   3) "SEE PASSAGE" BUTTON (tablet-verses v2.4). Top-left during play —
//      opens the CURRENT LEVEL's full passage (memorization peek),
//      single-column phone layout, Close returns to the board. PHONE-SPECIFIC:
//      the in-game "← Back" button was removed to free this spot (header was
//      too tight for both); mid-game back is rarely needed and the end screen
//      still returns to the picker. Tablet keeps its Back button.
//      FUTURE OPTION (noted for later): make the rolling passage text bar
//      itself tappable/expandable to show more lines — a zero-chrome
//      alternative or addition if more passage visibility is wanted.
// No verses gameplay/scoring formula change. In-game header label bumps to
// "Verses v2.2".
//
// =============================================================================
// MATCH-3 GAME v2.1 — PHONE VERSES (bug fix: first-line reveal + end freeze)
//
// Bug fix — two visible symptoms, one root cause. On the start-of-round modal
// the "Begin game" handler (and the "Play again" restart path) reset the reveal
// tracker `prevMovesRef` to null, but the accompanying setMoves(versesLevel.moves)
// was a no-op whenever moves already equaled that value (the default 1× case —
// every game). With moves unchanged, the reveal effect never re-ran to restore
// the tracker, so the FIRST move's line-reveal was skipped, leaving every reveal
// one line behind for the whole round. The two symptoms:
//   (1) after the first move the second line never appeared;
//   (2) at moves=0 the final line was never marked revealed, so the end-of-round
//       passage modal never opened and the game froze with no end screen.
// Fix: set prevMovesRef.current = versesLevel.moves (the move count at play
// start) instead of null, so the first move is detected. Two touch points: the
// Begin handler + the restart path. No other gameplay/scoring change.
// In-game header label bumps to "Verses v2.1".
//
// =============================================================================
// MATCH-3 GAME v2.0 — PHONE VERSES (tutorial port: 8 shared panels + V1/V2)
//
// The portable tutorial (core/Tutorial.jsx) lands on Phone Verses — the second
// tutorial port (tablet-verses v2.0 was first). A small opt-in "Tutorial" link
// sits at the top-left of the passage-selection screen; it opens the shared
// modal. Phone Verses shows the same 10 panels as tablet-verses: the 8 shared
// match panels + two verses-only panels (V1 verse-reveal with Genesis 1:1, V2
// target/moves static still). No new panel work — the panels are shared and
// platform-agnostic.
//
// Phone fit: the tutorial modal has a fixed natural size (the demo board is a
// fixed-pixel canvas) that would overflow a phone viewport, so core/Tutorial.jsx
// gained a responsive scale (measures the box, scales it to fit; scale 1 on
// wide screens, so tablet is unchanged). This file only imports Tutorial, adds
// the picker link + modal, and carries the 10-panel `sections` list. Major bump
// (1.11 → 2.0): the tutorial is a significant new screen landing whole, matching
// tablet-verses v2.0. No verses gameplay/scoring change.
// In-game header label bumps to "Verses v2.0". See
// docs/verses/tutorial-storyboard.md "Build plan — tablet-verses port".
//
// =============================================================================
// MATCH-3 GAME v1.11 — PHONE VERSES (play-through options: drill + repeat)
//
// June-20 update item (b), ported from tablet-verses v1.17 (same feature,
// adapted to phone's existing ×300/×500 target split):
//   1) "Play N times" drill — 1×/2×/3×/4× selector on the start-of-round
//      modal. N>1 plays the level as one CONTINUOUS run: passage loops
//      (revealedChunkIndex wraps mod totalChunks), one running score,
//      move budget N × totalChunks − 1 (chunk 0 free only on pass 1).
//      "Pass X of N" header indicator. Threaded via selectedPlays →
//      versesFlattenLevel(game, levelIndex, plays). Resets to 1× per
//      level (wrapper remounts on slug:levelIndex). Selector offers only
//      counts whose total ≤ 60 moves; if only 1× fits, none shows.
//   2) "Repeat level" button on the multi-level non-final first-pass WIN
//      end screen — quick replay at the rep count just played.
//   Scoring: versesTargetForMoves extends phone's split upward and
//   applies to ALL play (Option A): <17 →×300, 17–24 →×500, 25–29 →×600,
//   then +×100 per +5 moves (no ceiling yet — verses sims, TODO #9).
// In-game header label bumps to "Verses v1.11".
//
// =============================================================================
...
// v2.5: `safe center` tells the browser to centre the game vertically but
// fall back to top-alignment if the content is taller than the screen, so
// the top can never be pushed off where it cannot be scrolled to. Older
// browsers don't know the `safe` keyword and would DROP the whole
// declaration — which would silently leave the game top-aligned even when
// there is plenty of room. Feature-detecting instead means those browsers
// keep exactly today's plain centring, and only browsers that understand
// the keyword get the overflow protection.
const SAFE_CENTER = (
  typeof CSS !== 'undefined' && CSS.supports && CSS.supports('justify-content', 'safe center')
) ? 'safe center' : 'center';
...
// v2.5: HEIGHT BUDGET. Until now the tile size came from the viewport
// WIDTH only, so a narrow phone got smaller tiles but a SHORT phone got
// the same tall stack of header + text bar + board + instructions. On an
// iPhone SE-class screen (667pt tall) that stack does not fit, and
// because the page centres its content the overflow above the top is
// unreachable — the header would be clipped permanently rather than
// intermittently. So the tile size now takes whichever of the two
// budgets is smaller.
//
// VERSES_CHROME_PX is everything on screen that is NOT the board's tiles
// and inter-tile gaps, measured off a real device (see the v2.5 comment
// block at the top of this file for the measurements) and rounded UP for
// narrower phones, where the same text wraps onto more lines:
//   wrapper padding (25 top + 25 bottom)              50
//   header card                                      ~100
//   header → text bar gap                              14
//   passage text bar at its tallest (4 lines)        ~135
//   text bar → board gap                               10
//   board container's own padding + border             38
//                                                   -----
//                                                    ~347  → 350
// The instructions panel is deliberately NOT counted. Per the agreed
// priority order, a tight screen hides that panel (see `hideInstructions`
// in the component) BEFORE shrinking the board — the board is the game,
// and it should be the last thing to give.
//
// This is the one place in the v2.5 work that uses an estimate rather
// than a measurement, and that is deliberate: tile size is locked at
// module load, before anything has rendered, and making it respond to a
// real measurement would mean either resizing the board after first
// paint (a visible jump) or turning TILE_SIZE into runtime state, which
// every canvas draw call in this file reads. Being conservative here
// costs nothing on phones where the width budget already binds — which
// is every phone larger than the SE class.
const VERSES_CHROME_PX = 350;
const TILE_SIZE = (function () {
  ...
  if (!VERSES_MODE) {
    return Math.min(MAX_TILE_PX, Math.max(MIN_TILE_PX, tileBudget));
  }
  const heightBudget = Math.floor(
    (window.innerHeight - VERSES_CHROME_PX - (ROWS - 1) * TILE_GAP) / ROWS
  );
  return Math.min(
    MAX_TILE_PX,
    Math.max(MIN_TILE_PX, Math.min(tileBudget, heightBudget))
  );
})();
...
// to a safe state.
// v1.11: Tiered target multiplier (item b). Extends the existing
// ×300 (<17) / ×500 (≥17) split upward so a longer level — or a
// multi-play drill — gets a steeper target (more moves make big combos
// easier to line up). Applies to ALL play (Option A).
//   < 17 moves : × 300 (TARGET_PER_MOVE_SHORT)
//   17–24      : × 500 (TARGET_PER_MOVE_LONG)
//   25–29      : × 600, then +× 100 every additional 5 moves.
// No multiplier ceiling yet — revisit with verses sims (TODO #9).
function versesTargetForMoves(moves) {
  if (moves < TARGET_PER_MOVE_THRESHOLD) return moves * TARGET_PER_MOVE_SHORT;
  if (moves < 25) return moves * TARGET_PER_MOVE_LONG;
  const perMove = 600 + Math.floor((moves - 25) / 5) * 100;
  return moves * perMove;
}

function versesFlattenLevel(game, levelIndex = 0, plays = 1) {
  if (!game) return null;
  let level;
  ...
  // v1.11: drill support (item b). `plays` (1–4) repeats the level
  // continuously — chunk 0 free only on the first pass; each repeat pass
  // costs a move per chunk. Total moves = plays × totalChunks − 1
  // (11-chunk level: 10 at 1×, 21 at 2×, 43 at 4×). At plays = 1 this
  // reduces to the original totalChunks − 1.
  const reps = Math.max(1, Math.min(4, plays | 0));
  const moves = Math.max(1, reps * totalChunks - 1);
  // Tiered target on the (possibly drilled) move count. A single play
  // still honors an explicit per-level targetScore override; a drill
  // always uses the formula on the full drill length.
  const target = (reps === 1 && level.targetScore != null)
    ? level.targetScore
    : versesTargetForMoves(moves);
  return { level, flatChunks, totalChunks, moves, target, reps };
}
...
const VersesGame = ({
  ...
  // v1.11: selectedPlays — drill repetition count (1–4) chosen on the
  // start-of-round modal. Threads into versesFlattenLevel so moves/target
  // reflect the drill length. Resets to 1 automatically on level change
  // (the wrapper keys on slug:levelIndex, so this component remounts).
  const [selectedPlays, setSelectedPlays] = useState(1);
  const versesLevel = useMemo(() => versesFlattenLevel(versesGame, levelIndex, selectedPlays), [versesGame, levelIndex, selectedPlays]);
  ...
  // v2.2: showLevelPassage — on-demand peek at the CURRENT LEVEL's full
  // passage, opened by the top-left "See passage" button during play.
  // Reuses the same Georgia layout as the start/end passage modal; a single
  // Close returns to the board. (The whole-game text remains the separate
  // "See entire <title>" popup; this one is just the current level.)
  const [showLevelPassage, setShowLevelPassage] = useState(false);
  ...
  // v2.5: pageRef + hideInstructions — the "does it actually fit?" check.
  //
  // The instructions panel is static help text: valuable on a player's
  // first round, dead weight on their twentieth, and the only thing on
  // screen that isn't live game state. On a phone with room to spare it
  // stays up for the whole round — hiding it there would just trade a
  // useful panel for a bigger band of empty background, which is worse.
  // On a phone that CANNOT fit the layout it is the right thing to give
  // up, because the alternative is shrinking the board.
  //
  // Rather than guess which phones are which from a table of screen
  // sizes, the game measures itself: `pageRef` is the outer page wrapper,
  // and the effect below compares its rendered height against the height
  // actually visible. That removes device guesswork entirely and adapts
  // to phones neither the code nor its author has seen.
  //
  // Timing is deliberate too. The panel goes away after the player's
  // SECOND move, not immediately — by then they have seen it, made two
  // matches, and the passage text bar has started to grow into the space
  // the panel frees. Before that the panel simply overflows off the
  // bottom on a very short screen, which is accepted: it is the least
  // important element and it is about to disappear anyway.
  ...
  // v2.5: DOES THE LAYOUT ACTUALLY FIT? — the measurement behind
  // `hideInstructions` (declared with pageRef above).
  //
  // Runs after every render, deliberately without a dependency list. The
  // page's height changes for several unrelated reasons — a line of the
  // passage is revealed, the bonus-moves row appears once bonus moves are
  // earned, the phone is rotated, a long title wraps — and enumerating
  // them would be a list to get wrong. Reading one layout property after
  // each render is cheap, and the guard below means the state can only
  // ever flip false → true once per round, so there is no render loop.
  //
  // Why visualViewport rather than window.innerHeight: on iOS Safari
  // innerHeight reports the LARGE viewport (the height the page would
  // have with the browser toolbars hidden), which is exactly the mismatch
  // that caused the v2.4 header-clipping bug. visualViewport.height is
  // what the player can actually see. Pinch-zoom would also move that
  // number, but the page sets user-scalable=no, so it cannot.
  //
  // The two-move condition uses movesUsedRef rather than
  // `versesLevel.moves - moves` because it is incremented per successful
  // swap and is not confused by bonus moves, which leave the regular move
  // count pinned at 0.
  useLayoutEffect(() => {
    if (!VERSES_MODE || hideInstructions) return;
    const el = pageRef.current;
    if (!el) return;
    const visible = (window.visualViewport && window.visualViewport.height) || window.innerHeight;
    // 2px tolerance — sub-pixel layout rounding should not count as overflow.
    const overflows = el.offsetHeight > visible + 2;
    if (overflows && movesUsedRef.current >= 2) {
      setHideInstructions(true);
    }
  });

  // v2.5: re-run the fit check when the visible height changes on its own —
  // rotation, or Safari's toolbar expanding/collapsing. Without this a
  // player who rotates into a shorter viewport mid-round would keep the
  // panel until their next move happened to trigger a render.
  useEffect(() => {
    ...
    const recheck = () => {
      ...
      if (!el) return;
      const visible = (window.visualViewport && window.visualViewport.height) || window.innerHeight;
      if (el.offsetHeight > visible + 2 && movesUsedRef.current >= 2) {
        setHideInstructions(true);
      }
    };
    ...
    return () => {
      ...
      if (window.visualViewport) window.visualViewport.removeEventListener('resize', recheck);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    // v2.2 (decision 4): the target-hit toast reads "✓ Target reached — 1.5×!"
    // and the victory round applies the 1.5× multiplier — both only make sense
    // when the target is crossed during REGULAR play. If it's crossed while the
    // player is spending bonus moves (a loss-path rescue), suppress BOTH: the
    // level still completes (targetReached drives the gold ✓ header + stars/
    // unlock) but scoring stays 1× and no misleading 1.5× toast fires. A
    // victory round already active before bonus play (win path) is untouched
    // here — targetReached didn't change, so this effect doesn't re-run.
    if (usingBonusMoves) return;
  ...
  useEffect(() => {
    ...
    if (prevMovesRef.current !== null && moves < prevMovesRef.current) {
      setRevealedChunkIndex(prev => {
        const next = prev + 1;
        const total = versesLevel.totalChunks;
        // v1.11: in a drill (reps > 1) the passage loops — wrap to chunk
        // 0 instead of capping. End-of-round is gated on moves === 0, not
        // the reveal index, so looping is safe. reps === 1 keeps the cap.
        if (versesLevel.reps > 1) return next % total;
        const cap = total - 1;
  ...
  useEffect(() => {
    ...
    // v2.2: bonus-moves flow slots in here, before the passage screen.
    // Banner is already up (offer pending) — wait for the player's choice.
    if (showBonusMovesPrompt) return;
    // Still spending bonus moves with wallet remaining — keep playing;
    // moves stays 0 (bonus swaps draw from the wallet, not moves), so the
    // passage stays fully revealed and no new lines appear.
    if (usingBonusMoves && bonusMoves > 0) return;
    // LOSS-path offer (decision 1): 0 moves, below target, wallet non-empty,
    // not yet using them → drop the arcade banner instead of ending. The
    // board (with the fully-revealed final line) stays visible behind it.
    if (!targetReached && bonusMoves > 0 && !usingBonusMoves) {
      setShowBonusMovesPrompt(true);
      return;
    }
    // Otherwise end the round → passage screen. Covers: a WIN, a LOSS with
    // no wallet, and the moment bonus moves are just exhausted.
    const t = setTimeout(() => {
      if (usingBonusMoves) { usingBonusMovesRef.current = false; setUsingBonusMoves(false); }
    ...
    return () => clearTimeout(t);
  }, [moves, revealedChunkIndex, showPassageModal, gameState, isAnimating, combo, pendingSpecials.length, targetReached, onLevelComplete, levelIndex, score, levelTarget, showBonusMovesPrompt, usingBonusMoves, bonusMoves]);
  ...
  // v2.2: WIN-path resume. The passage screen carries a "Use bonus moves"
  // button (when the wallet is non-zero); clicking it closes the passage
  // screen and returns to the board to keep playing on bonus moves. The
  // victory round (if active on the win) is intentionally left on, so the
  // extra swaps score at 1.5× (decision 4). Ending later re-opens the
  // passage screen via endLevelCarryBonus's verses branch.
  const resumeBonusFromPassage = () => {
  ...
  const endLevelCarryBonus = () => {
    ...
    setScore(prev => prev + specialsBonus);
    // v2.2: In VERSES_MODE, ending bonus-move play opens the passage screen
    // (verses' end-of-round) instead of the arcade win/lose gameState the
    // passage screen replaces. Completion (stars/unlock) is recorded here
    // when the target was reached — the V-2 end-of-round effect that
    // normally calls onLevelComplete is bypassed because we set
    // showPassageModal directly.
    if (VERSES_MODE) {
      if (wonLevel) {
        if (!targetReached) setTargetReached(true);
        if (onLevelComplete) onLevelComplete(levelIndex, scoreRef.current + specialsBonus, levelTarget);
      }
      setShowPassageModal(true);
      return;
    }
  ...
  const handleCanvasClick = (e) => {
    ...
    if (VERSES_MODE && ((moves <= 0 && !(usingBonusMoves && bonusMoves > 0)) || showStartModal || showPassageModal)) return; // V-2: freeze input while a passage modal is open or during the 2.5s pre-modal hold. v2.2: but allow play while actively spending bonus moves (moves===0 but wallet remaining).
  ...
  const restartGame = () => {
    ...
    _pendingSpecialDrop.kind = null;
    // v2.5: bring the instructions panel back for the new round. The
    // component remounts on a LEVEL change (keyed on slug:levelIndex), so
    // that path resets this automatically — but a restart on the SAME level
    // reuses the mounted component, so it has to be cleared here alongside
    // movesUsedRef, which is what the panel's two-move timing reads.
    ...
    if (VERSES_MODE) {
      ...
      // v2.1: match prevMovesRef to the move count at play start (not null) so
      // the first move's reveal isn't skipped. setMoves above is a no-op when
      // moves already equals versesLevel.moves, so null would persist into play.
      prevMovesRef.current = versesLevel.moves;
    }
  ...
  return (
    <div ref={pageRef} style={{
      ...
      alignItems: 'center',
      // v2.5: 'center' → 'safe center'. The `safe` keyword means "centre
      // it, but if the content is TALLER than the screen, fall back to
      // aligning from the top instead". Plain `center` splits any
      // overflow evenly above and below — and the half that goes above
      // the top of the screen cannot be scrolled to, so the header is
      // simply lost. That is the same failure v2.4 fixed for a different
      // reason, and it would return on any phone too short to fit the
      // layout. With `safe`, a short phone pushes its overflow to the
      // BOTTOM, where the least important content already sits.
      // Browsers without support drop the declaration and fall back to
      // flex-start, which is the safe direction anyway.
      justifyContent: SAFE_CENTER,
      // v2.4: 100vh → 100dvh. On iOS Safari 100vh is the *large* viewport
      // height (as if the toolbars were hidden), so it exceeds the visible
      // area whenever the toolbar is showing — that gap became scroll room,
      // and touchAction:'none' below meant the player could not scroll back
      // once the page drifted, leaving the header's top cut off. The dynamic
      // unit tracks what's actually visible.
      minHeight: '100dvh',
      background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)',
      fontFamily: 'Arial, sans-serif',
      // v2.4: phone-verses.html sets viewport-fit=cover + black-translucent
      // status bar, which run the page under the status bar/notch; nothing
      // compensated for it, so the status bar sat on the title. Longhands
      // (not the shorthand) so paddingTop can't be clobbered by property
      // order.
      //
      // v2.5: TOP AND BOTTOM MADE SYMMETRIC (was 8px top / 60px bottom).
      // Centring happens INSIDE the padding, so space above the content is
      // paddingTop + half the leftover slack, and space below is
      // paddingBottom + the same half. The old 52px difference between the
      // two was the entire reason the game sat high with a band of dead
      // space underneath it — measured at 29pt above vs 86pt below. Equal
      // padding splits the leftover evenly (~58pt at each end on a
      // 428×926pt phone). The safe-area insets stay on both edges: the top
      // one is 0 in normal Safari browsing but non-zero when the game is
      // launched from the Home Screen, and the bottom one keeps content off
      // the home indicator in that same case.
      paddingTop: 'calc(env(safe-area-inset-top, 0px) + 25px)',
      paddingRight: '0',
      paddingBottom: 'calc(env(safe-area-inset-bottom, 0px) + 25px)',
      paddingLeft: '0',
      touchAction: 'none',
      userSelect: 'none'
    }}>
      ...
      <div style={{
        ...
        {/* v2.2: the in-game "← Back" button was REMOVED on phone (user
            direction 2026-07-04) to free the top-left header spot for "See
            passage" — phone header real estate is tight (Back · title ·
            dark/light toggle left no room). Mid-game back is rarely needed;
            to leave a level, finish it (moves are limited) and use the end
            screen's "Back to passage selections". `onBack` is still used by
            the start-of-round modal's Back button. Tablet-verses keeps its
            in-game Back button (it has the room). */}
        {/* v2.2: "See passage" — top-left (the old Back button's spot).
            Opens the current level's full passage as an on-demand peek
            during play (memorization aid). Hidden while the start/end
            passage modals are up (they already show the passage). Small
            pill in the verses accent color. */}
        {VERSES_MODE && versesLevel && !showStartModal && !showPassageModal && (
          <button
            onClick={() => setShowLevelPassage(true)}
            style={{
              position: 'absolute',
              top: '8px',
              left: '12px',
            ...
            title="See this level's full passage"
          >
            See passage
          </button>
        )}
        ...
        {/* V-3b: title + (optional) inline "Level N of M" indicator
            ...
            work. */}
        {/* v2.2: on phone, long verses titles get (a) a smaller font and
            (b) a right-nudge so the centered title + "N/M" don't run under the
            top-left "See passage" button (there's free space on the right, so
            shift into it rather than squeeze the left). Char-length tiers:
              > 20 (e.g. "1 Thessalonians 4:13–18", 23) → 16px, 48px nudge
              15–20 (e.g. "Isaiah 52:13–53:12" 18, "Habakkuk 3:17–19" 16) → 18px,
                     28px nudge (v2.3: eased back left from 48px — the full nudge
                     read too far right / off-center for these medium titles)
              ≤ 14 (e.g. "Psalm 46", "Titus 2:11–13") → 20px, centered, no nudge */}
        <h1 style={{
          margin: '0',
          color: '#333',
          fontSize: (VERSES_MODE && versesGame && versesGame.title && versesGame.title.length > 20) ? '16px'
                  : (VERSES_MODE && versesGame && versesGame.title && versesGame.title.length > 14) ? '18px'
                  : '20px',
          paddingLeft: (VERSES_MODE && versesGame && versesGame.title && versesGame.title.length > 20) ? '48px'
                     : (VERSES_MODE && versesGame && versesGame.title && versesGame.title.length > 14) ? '28px'
                     : '0',
            ...
            <>
              ...
              {isMultiLevel && Array.isArray(versesGame.levels) && (
                <span style={{
                  ...
                  {/* v2.2: shortened from "· Level N of M" to a compact "N/M"
                      so a long multi-level title (e.g. "Isaiah 52…") doesn't
                      widen the centered header into the top-left "See passage"
                      button on phone. */}
                  {levelIndex + 1}/{versesGame.levels.length}
                </span>
              )}
              {/* v1.11: drill progress — shown for any level played 2–4×. */}
              {versesLevel && versesLevel.reps > 1 && (
                <span style={{
                  fontSize: '13px',
                  color: '#777',
                  fontFamily: 'Georgia, serif',
                  ...
                  · Pass {Math.min(versesLevel.reps, Math.floor((versesLevel.moves - moves) / versesLevel.totalChunks) + 1)} of {versesLevel.reps}
                </span>
              )}
            </>
          ) : (
            <>Verses <span style={{ fontSize: '12px', color: '#ccc' }}>v2.6</span></>
          )}
        </h1>
      ...
      {/* v2.2: current-level passage peek — opened by the top-left "See
          passage" button during play. Same single-column Georgia layout as
          the phone start/end passage modal, rendering only the current
          level's chunks; a single Close returns to the board. The
          full-screen overlay (zIndex 2000) intercepts pointer events, so the
          board is naturally frozen while it's open. */}
      {VERSES_MODE && versesLevel && showLevelPassage && (
        <div style={{
          position: 'fixed',
          top: 0, left: 0, right: 0, bottom: 0,
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              <button
                onClick={() => setShowLevelPassage(false)}
                style={{
                  padding: '12px 28px',
                  fontSize: '16px',
                  fontFamily: 'Georgia, serif',
        ...
        <div style={{
          ...
          <div style={{
            ...
            {(() => {
              ...
              if (showStartModal) {
                // Start-of-round modal — drill selector (v1.11) + Back +
                // Begin (game / level).
                const beginLabel = isMultiLevel ? 'Begin level' : 'Begin game';
                const totalChunks = versesLevel.totalChunks;
                // v1.11: offer only rep counts whose continuous-drill move
                // total (N × chunks − 1) stays ≤ the 60-move ceiling.
                // Unchoosable counts are omitted; if only 1× fits, no
                // selector shows.
                const playOptions = [1, 2, 3, 4].filter(n => n * totalChunks - 1 <= 60);
                return (
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '14px' }}>
                    {playOptions.length > 1 && (
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px' }}>
                        <div style={{ fontFamily: 'Georgia, serif', fontSize: '14px', color: '#ccc' }}>
                          Play this level:
                        </div>
                        <div style={{ display: 'flex', gap: '8px' }}>
                          {playOptions.map(n => {
                            const active = selectedPlays === n;
                            return (
                              <button
                                key={n}
                                onClick={() => setSelectedPlays(n)}
                                style={{
                                  padding: '8px 16px',
                                  fontFamily: 'Georgia, serif',
                                  fontSize: '15px',
                    ...
                    <div style={{ display: 'flex', justifyContent: 'center', gap: '12px' }}>
                      ...
                      <button
                        onClick={() => {
                          // v1.11: seed the round from the (possibly
                          // drilled) versesLevel so the chosen rep count
                          // takes effect.
                          if (VERSES_MODE && versesLevel) {
                            setScore(0);
                            setMoves(versesLevel.moves);
                            ...
                            setRevealedChunkIndex(0);
                            setTargetReached(false);
                            // v2.1: see restart path — match tracker to the move
                            // count at play start so the first reveal fires.
                            prevMovesRef.current = versesLevel.moves;
                          }
              ...
              if (!isMultiLevel) {
                ...
                // v2.2: Arcade mode is offered on every free-replay screen (win
                // OR fail) — free-replay means the whole passage is already
                // fully completed, so an arcade handoff is always valid here,
                // matching single-level parity. Shared-wallet handoff: bonus
                // moves already live in PHONE_BONUS_MOVES_KEY (the pool phone
                // arcade reads at mount), so there's nothing to transfer — just navigate.
                buttons.push({
                  kind: 'secondary',
                  label: 'Arcade mode',
                  onClick: () => {
                ...
                // Repeat level (v1.11) + Back to level selections.
                // "Repeat level" replays this level via restartGame (at
                // the same rep count just played). Completion / stars /
                // best live in the wrapper and update only on a win, so
                // they never regress; Next level stays available.
                buttons.push({ kind: 'primary', label: 'Next level →', onClick: () => onAdvanceLevel && onAdvanceLevel() });
                buttons.push({ kind: 'secondary', label: 'Repeat level', onClick: () => restartGame() });
                buttons.push({ kind: 'secondary', label: 'Back to level selections', onClick: onBackToLevelSelect });
              } else {
                // Multi-level fail (any position) first-pass — Retry +
                // Back to level selections.
              ...
              // v2.2: WIN-path bonus-moves button (decision 1). Only when the
              // player won this round and has bonus moves to spend. Slotted
              // right after the primary action so it reads as an opt-in extra,
              // not the main next step. On a LOSS the offer already came via
              // the banner before this screen, so no button is added there.
              if (won && bonusMoves > 0) {
                buttons.splice(1, 0, { kind: 'secondary', label: 'Use bonus moves', onClick: resumeBonusFromPassage });
              }
      ...
      {/* Instructions.
          v2.5: hidden once the layout has been measured as not fitting AND
          the player has made two moves — see `hideInstructions` and the fit
          effect above. On any phone with room to spare this is always false
...
const VersesPicker = ({ games, slugs, progress, onPick, onOpenTutorial, secretUnlock, onSecretPressStart, onSecretPressEnd }) => {
  return (
    <div style={{
      display: 'flex',
      flexDirection: 'column',
      alignItems: 'center',
      // v2.6: 100vh → 100dvh, matching the game screen. On iOS Safari 100vh
      // is the height the page would have with the browser toolbars hidden,
      // which is taller than what is actually visible while a toolbar shows.
      minHeight: '100dvh',
      background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)',
      fontFamily: 'Arial, sans-serif',
      // v2.6: top padding gains the safe-area inset. This page is set to use
      // the whole screen INCLUDING the strip behind the iOS status bar (the
      // clock / signal / battery row) — see viewport-fit=cover in the page's
      // HTML. When Safari's bottom toolbar shrinks to its small pill, Safari
      // gives the page more room and everything shifts upward, which is what
      // put the clock on top of the in-game title before v2.4. Reserving the
      // inset keeps this screen's content out from under that strip. The 40px
      // is unchanged — there is no collision to fix on this screen (the
      // "Verses" heading is short and sits well right of the Tutorial pill).
      ...
      {/* v2.0: opt-in Tutorial link — small pill, top-left, mirrors the tablet
          arcade header button. Light-on-purple so it reads on this background. */}
      {onOpenTutorial && (
        <button
          onClick={onOpenTutorial}
          style={{
            position: 'absolute',
            // v2.6: + the safe-area inset. An absolutely positioned element is
            // placed from the container's edge, so the container's padding
            // does NOT move it — it needs the inset itself or it can end up
            // behind the status bar when the page shifts up.
...
const VersesLevelSelect = ({ game, gameProgress, completedLevels, fullCompleted, onPickLevel, onBack, onShowFullPassage, secretUnlock, onSecretPressStart, onSecretPressEnd }) => {
  ...
  return (
    <div style={{
      ...
      flexDirection: 'column',
      alignItems: 'center',
      // v2.6: 100vh → 100dvh, matching the game screen (see the passage-select
      // screen above for why).
      minHeight: '100dvh',
      background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)',
      fontFamily: 'Arial, sans-serif',
      // v2.6: TOP PADDING RAISED 40px → 68px, PLUS the safe-area inset.
      //
      // The bug (player-reported 2026-08-09, screenshot of Isaiah
      // 52:13–53:12): the "← Back to passage selections" button overlapped
      // the passage title. Cause is a static layout collision, not a scroll
      // or status-bar problem — the button is absolutely positioned at
      // top:20px and is 32px tall (14px text + 12px vertical padding + 4px
      // border), so it occupies the band 20–52px, while the title is a
      // normal in-flow element starting at this container's 40px top
      // padding — 12px inside the button's band. The padding was never
      // raised to account for a button taken out of flow.
      //
      // It affected EVERY passage, not only long ones: the button is ~212px
      // wide from x=20, so its right edge lands just past the middle of the
      // screen, and the title is centred. Isaiah simply made it obvious
      // because at 36px Georgia its capitals and digits are tall enough to
      // visibly cross the button's lower border.
      //
      // 68px = the button's 52px lower edge + ~16px of breathing room. The
      // safe-area inset is added for the same reason as on the passage-select
      // screen above.
      ...
      <button
        ...
        style={{
          position: 'absolute',
          // v2.6: + the safe-area inset. Absolute positioning is measured from
          // the container's edge, so raising the container's padding above
          // moved the TITLE but not this button — it needs the inset itself.
...
// v2.0: the tutorial panel order for Verses — the 8 shared match panels, then
// the two verses-only panels (V1 reveal, V2 target/moves). Panel content lives
// in core/Tutorial.jsx.
const VERSES_TUTORIAL_SECTIONS = [
  'basic-match', 'line-special', 'bomb', 'cross', 'supernova', 'hypernova',
  'multipliers', 'fusion', 'verse-reveal', 'target-moves',
];

const Match3Verses = () => {
  const [activeSlug, setActiveSlug] = useState(null);
  // v2.0: opt-in tutorial modal (shared core/Tutorial.jsx). Opened from the
  // top-left "Tutorial" link on the passage-selection screen; never auto-opens.
  ...
  if (!activeSlug) {
    return (
      <>
        ...
        {showTutorial && (
          <Tutorial
            sections={VERSES_TUTORIAL_SECTIONS}
