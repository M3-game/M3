...
// uses a phone-scoped local constant `PHONE418_BONUS_MOVES_KEY`
...
// =============================================================================
// MATCH-3 GAME v1.4 - PHONE 418PX VERSES (MEMORIZE MODE)
//
...
//
// Phone-418-verses-specific changes:
...
//     constant `PHONE418_BANKED_MOVES_KEY` → `PHONE418_BONUS_MOVES_KEY`).
...
//   The constant `PHONE418_BONUS_MOVES_KEY`'s value is intentionally
//   KEPT as 'match3_phone418_bankedMoves' (NOT renamed to
//   '..._bonusMoves'). Phone-418 arcade (deferred T-1 platform) still
...
//   - Bonus-moves carry into phone-418 arcade reads/writes
//     'match3_phone418_bankedMoves' (phone-418 arcade's pool key) via
//     a new local PHONE418_BONUS_MOVES_KEY constant — replaces the
//     tablet `BANKED_KEY` import for these runtime writes. (T-1: the
//     constant name modernized to PHONE418_BONUS_MOVES_KEY but the
//     storage-string VALUE stays 'match3_phone418_bankedMoves' to
//     preserve the cross-platform contract with phone-418 arcade —
...
// N-2 (phone-418-verses v1.0): phone-scoped bonus-moves storage key.
// Replaces the imported tablet `BANKED_KEY` for runtime reads/writes —
// matches phone-418 arcade's existing key (defined in
// platforms/phone-418/match3-v13.x-418px-phone.jsx as BANKED_MOVES_KEY).
...
// Storage-string VALUE intentionally KEPT as 'match3_phone418_bankedMoves'.
// Phone-418 arcade (deferred T-1 platform) still reads this exact key
// for its own pool, and the verses → arcade carry-out flow at line ~5491
// writes to this shared key so arcade can pick up the merged total on
// entry. Renaming the value would diverge verses' write target from
// arcade's read target, breaking the carry-out. The string MUST be
// renamed in lockstep with phone-418 arcade — captured as a hard
// requirement in DEFERRED.md "Cross-platform terminology sweep" entry.
const PHONE418_BONUS_MOVES_KEY = 'match3_phone418_bankedMoves';
...
const VersesGame = ({
  ...
  const restartGame = () => {
    ...
    if (VERSES_MODE) {
      ...
      setBonusMoves(0);
      try { localStorage.setItem(PHONE418_BONUS_MOVES_KEY, '0'); } catch {}
    }
            ...
            {(() => {
              ...
              if (!isMultiLevel) {
                ...
                buttons.push({
                  ...
                  label: 'Arcade mode',
                  onClick: () => {
                    // N-2: arcade-mode handoff. Roll bonusMoves into
                    // phone-418 arcade's PHONE418_BONUS_MOVES_KEY (the
                    ...
                    try {
                      ...
                      const existing = parseInt(localStorage.getItem(PHONE418_BONUS_MOVES_KEY) || '0', 10);
                      localStorage.setItem(PHONE418_BONUS_MOVES_KEY, (existing + carryAmount).toString());
...
const Match3Verses = () => {
  ...
  // and skip PHONE418_BONUS_MOVES_KEY persistence — memorize-mode
  // bonus moves are session-scoped (zero on return to picker). In
  // !VERSES_MODE (arcade fallback), hydrate from
  // PHONE418_BONUS_MOVES_KEY at mount and persist via the effect
  ...
  const [bonusMoves, setBonusMoves] = useState(() => (
    VERSES_MODE ? 0 : parseInt(localStorage.getItem(PHONE418_BONUS_MOVES_KEY) || '0', 10)
  ));
  useEffect(() => {
    if (VERSES_MODE) return;
    localStorage.setItem(PHONE418_BONUS_MOVES_KEY, bonusMoves.toString());
  }, [bonusMoves]);
