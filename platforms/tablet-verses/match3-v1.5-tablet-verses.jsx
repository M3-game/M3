...
//
// Content:
//   - New `games/psalm-91/game.js` — 4 levels, NKJV, 49 chunks total
...
const VersesGame = ({
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(t);
  }, [moves, revealedChunkIndex, showPassageModal, gameState, isAnimating, combo, pendingSpecials.length, targetReached, onLevelComplete, levelIndex]);
          ...
          <div style={{
            ...
            {(() => {
              ...
              if (!isMultiLevel) {
                ...
                // Back to level selections + Arcade mode (inactive in V-3b).
                buttons.push({ kind: 'primary', label: 'Play again', onClick: () => onWrapperRestartGame && onWrapperRestartGame() });
                buttons.push({ kind: 'secondary', label: 'Back to level selections', onClick: onBackToLevelSelect });
                buttons.push({ kind: 'inactive', label: 'Arcade mode', caption: 'Available in v1.6' });
              } else if (won) {
...
//     (free-replay mode).
const VersesLevelSelect = ({ game, completedLevels, fullCompleted, onPickLevel, onBack }) => {
...
const Match3Verses = () => {
  ...
  // and skip BANKED_KEY persistence — memorize-mode bonus moves are
  // session-scoped (zero on return to picker). In !VERSES_MODE
  // (arcade fallback), hydrate from BANKED_KEY at mount and persist
  ...
  // V-3b: pick handler from the picker. Single-level → straight to
  // play (level 0). Multi-level → level-select first.
  ...
  // V-3b: level-completion handler called from VersesGame when the
  // player wins a level (target hit, round resolves). Adds the
  // index to completedLevels; flips fullCompleted if the set now
  // covers every level.
  ...
  // V-3b: render branch resolution.
  if (!activeSlug) {
    return <VersesPicker games={versesGameRegistry} onPick={handlePick} />;
  }
