...
// picker. The outer wrapper passes `key={slug}` so each pick triggers
// a fresh mount (and, in V-3b, level-to-level transitions will too).
const VersesGame = ({ slug, onBack }) => {
  // V-3a: derive active game + flattened level from the slug prop.
  // Memoized on slug; identity is stable for the lifetime of this
  // mount since the outer wrapper keys on slug.
  const versesGame = useMemo(() => (slug ? versesGameRegistry[slug] || null : null), [slug]);
  const versesLevel = useMemo(() => versesFlattenLevel(versesGame), [versesGame]);
  ...
  // V-3a: in VERSES_MODE, init to 0 instead of reading localStorage.
  // Spec: bonus-moves pool zeroes on any return to main picker, and
  // remounts via the slug-keyed wrapper start fresh. Pairs with the
  // VERSES_MODE short-circuit on the persist-to-localStorage effect
  // below so a memorize round never writes to BANKED_KEY.
  const [bankedMoves, setBankedMoves] = useState(() => (
    VERSES_MODE ? 0 : parseInt(localStorage.getItem(BANKED_KEY) || '0', 10)
  ));
  ...
  // V-3a: in VERSES_MODE, skip the write — memorize-mode bonus moves
  // are session-scoped (zero on return to picker), so they should
  // never bleed into the shared BANKED_KEY that arcade mode reads.
  useEffect(() => {
      ...
      <div style={{
        ...
        <h1 style={{ margin: '0', color: '#333', fontSize: '20px', textShadow: '2px 2px 4px rgba(0,0,0,0.1)' }}>
          {VERSES_MODE && versesGame
            ? versesGame.title
            : <>Verses <span style={{ fontSize: '12px', color: '#ccc' }}>v1.3</span></>}
        </h1>
        <div style={{
          display: 'flex',
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: '12px' }}>
              {/* V-3a: Back button on the start-of-round modal alongside
                  Begin game. The modal covers the in-game header (so the
                  header's ← Back is unreachable here). End-of-round modal
                  doesn't need a Back button — the post-round buttons
                  shipped in V-2 already handle their own routing. */}
              {showStartModal && onBack && (
                <button
                  onClick={onBack}
                  style={{
                    padding: '12px 24px',
              ...
              <button
                onClick={() => {
                  if (showStartModal) {
                    // Start-of-round: just close the modal; game is
                    // already in fresh-state from mount or restartGame.
                    setShowStartModal(false);
                  } else {
                    // End-of-round: restart the round. restartGame
                    // re-opens showStartModal as part of its reset.
                    restartGame();
                  }
                }}
                style={{
              ...
              >
                {showStartModal ? 'Begin game' : 'Play again'}
              </button>
            </div>
          </div>
        </div>
...
const VersesPicker = ({ games, onPick }) => {
        ...
        {slugs.length === 0 && (
          <div style={{
            ...
            {' '}<code style={{ background: 'rgba(255,255,255,0.15)', padding: '2px 6px', borderRadius: '4px' }}>
              platforms/tablet-verses/games/&lt;slug&gt;/
            </code>{' '}
            to add one.
          </div>
        )}
        {slugs.map(slug => {
          ...
          return (
            ...
            >
              ...
              {game.translation && (
                <div style={{ fontSize: '13px', color: '#888', marginBottom: isMultiLevel ? '4px' : 0 }}>
                  {game.translation}
                </div>
              )}
              {isMultiLevel && (
...
// V-3a: Outer wrapper. Manages `activeSlug` state. Null = picker.
// `key={slug}` on VersesGame forces a fresh mount on each pick (and
// will handle level-to-level remounts in V-3b without changes here).
const Match3Verses = () => {
  const [activeSlug, setActiveSlug] = useState(null);
  if (!activeSlug) {
    return <VersesPicker games={versesGameRegistry} onPick={setActiveSlug} />;
  }
  return (
    <VersesGame
      key={activeSlug}
