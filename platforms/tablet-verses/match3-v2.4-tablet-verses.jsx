...
// =============================================================================
// MATCH-3 GAME v2.4 - TABLET VERSES ("See passage" — current-level peek)
//
// New feature: a "See passage" button (top-left, just below the Back button,
// small accent-colored pill) available during play. It opens a popup showing
// the CURRENT LEVEL's full passage (versesLevel.flatChunks) in the same Georgia
// two-column layout as the start/end passage modals, with a single Close.
// Memorization aid — mid-level you otherwise only see the rolling window
// (current line + the two before it). Current level only, per user direction:
// the whole-game text is already reachable via "See entire <title>" (final-win
// / level-select) and the end-of-round screen. Hidden while the start/end
// passage modals are up (they already show the passage). The full-screen
// overlay freezes board input while open. New state: showLevelPassage.
// In-game header label bumps to "Verses v2.4".
//
// =============================================================================
// MATCH-3 GAME v2.3 - TABLET VERSES (Arcade mode on free-replay end screens)
//
// Small parity fix: the "Arcade mode" button now also appears on the
// free-replay end-of-round screen. "Free-replay" = the whole multi-level
// passage has already been fully completed (fullCompleted), so every level is
// unlocked and replaying any of them is a revisit with nothing left to unlock.
// Previously arcade showed on single-level ends and the first-pass final-level
// win, but NOT on free-replay screens — so once a passage was 100%'d, arcade
// disappeared on all replays (including replaying the final level). Now it's
// offered on every free-replay screen, win OR fail (the passage is already
// won either way, per user direction 2026-07-04). No change to the other
// branches: first-pass non-final wins and first-pass fails still show no
// arcade button. One button push in the isFreeReplay branch, reusing the
// existing shared-wallet navigation to tablet.html.
// In-game header label bumps to "Verses v2.3".
//
// =============================================================================
// MATCH-3 GAME v2.2 - TABLET VERSES (use bonus moves in verses)
//
// New feature: bonus moves (earned + shown in the header, previously
// unspendable inside verses) can now be USED in a verses game. Reuses the
// arcade bonus-move machinery (the drop-down banner + the in-header "End and
// carry moves forward" button + the below-target confirm), wired into the
// verses end-of-round flow. Two paths (user-scoped 2026-07-04):
//   LOSS (0 moves, below target): instead of jumping to the passage screen,
//     the "Out of moves! Use bonus moves / End and carry" banner drops down
//     over the still-visible board. "Use bonus moves" resumes play (swaps draw
//     from the shared wallet; the passage stays fully revealed); the in-header
//     "End and carry" button (with below-target confirm) ends the round and
//     opens the passage screen.
//   WIN (0 moves, at/above target): the passage screen appears as usual, now
//     with an added "Use bonus moves" button (only when the wallet is non-zero).
//     Clicking it returns to the board with the 1.5× victory round still on;
//     ending re-opens the passage screen.
// Scoring rule (decision 4): a victory round already active on a WIN is kept
// (fat novas pop at 1.5×). On a LOSS, crossing the target during bonus moves
// completes the level (stars/unlock) but does NOT switch the 1.5× on — it
// wasn't a victory earned in regular play. Touch points: the verses end-of-
// round effect (banner-vs-passage decision), the 3 input-freeze guards
// (unfreeze during active bonus play), the target->victory-round effect
// (loss-path guard), endLevelCarryBonus (verses branch opens the passage
// screen, not the arcade win/lose gameState), a resumeBonusFromPassage
// handler, and the passage-screen button matrix (win-path button).
// In-game header label bumps to "Verses v2.2".
//
// =============================================================================
// MATCH-3 GAME v2.1 - TABLET VERSES (bug fix: first-line reveal + end freeze)
//
...
//
// =============================================================================
// MATCH-3 GAME v2.0 - TABLET VERSES (tutorial port: 8 shared panels + V1/V2)
//
// The portable tutorial (core/Tutorial.jsx) lands on Verses. A small opt-in
// "Tutorial" link sits at the top-left of the passage-selection screen (mirrors
// the tablet arcade header button); it opens the shared modal. Verses shows the
// 8 shared match panels PLUS two verses-only panels that run after them:
//   V1 (verse-reveal) — an animated match board with a rolling text bar; the
//      first line of Genesis 1:1 is pre-visible and each match reveals the next
//      line (as close to the real verses game as possible).
//   V2 (target-moves) — a static labeled still highlighting the target score +
//      moves-remaining, with conceptual wording (no exact formula).
// Both new panels live in core/Tutorial.jsx (edited in place, inherited here);
// this file only imports Tutorial, adds the picker link + modal, and carries the
// 10-panel `sections` list. Major bump (1.17 → 2.0): the tutorial is a
// significant new screen landing whole. No verses gameplay/scoring change.
// In-game header label bumps to "Verses v2.0". See
// docs/verses/tutorial-storyboard.md "Build plan — tablet-verses port".
//
// =============================================================================
// MATCH-3 GAME v1.17 - TABLET VERSES (play-through options: drill + repeat)
//
// June-20 update item (b): two related play-through features.
//   1) "Play N times" drill — a 1×/2×/3×/4× selector on the start-of-round
//      modal (showStartModal). Picking N>1 makes the level play as one
//      CONTINUOUS run: the passage loops (revealedChunkIndex wraps mod
//      totalChunks), one running score, one continuous move budget of
//      N × totalChunks − 1 (chunk 0 free only on the first pass; each
//      repeat pass costs a move for every chunk). A "Pass X of N"
//      indicator shows in the header. Threaded via selectedPlays →
//      versesFlattenLevel(game, levelIndex, plays). Resets to 1× per
//      level (the wrapper remounts on slug:levelIndex).
//   2) "Repeat level" button — added to the multi-level non-final
//      first-pass WIN end screen (the one screen that lacked a quick
//      replay); replays the level via restartGame at the rep count just
//      played. Completion/stars/best never regress; "Next level" stays.
//   Ceiling: the selector offers only rep counts whose drill total stays
//   ≤ 60 moves; unchoosable counts are omitted (if only 1× fits, no
//   selector shows).
//   Scoring: a new tiered target multiplier (versesTargetForMoves)
//   replaces the uniform × 300 and applies to ALL play (Option A):
//   <17 →×300, 17–24 →×500, 25–29 →×600, then +×100 per +5 moves
//   (no ceiling yet — revisit with verses sims, TODO #9). A single play
//   still honors an explicit per-level targetScore override.
// Tablet-verses-first; phone-verses + sandbox port to follow.
// In-game header label bumps to "Verses v1.17".
//
// =============================================================================
// MATCH-3 GAME v1.16 - TABLET VERSES (board 10 → 12 rows, match Tablet Arcade)
//
// June-20 update item (c): tablet-verses board height restored to 12 rows
// (was 10 since V-4) so the playing grid matches Tablet Arcade exactly
// (12 rows × 10 cols at 50px tiles). COLS, tile size, and the whole
// match-3 engine were already in parity; board height was the only
// un-intended difference. The ~108px-taller canvas is accepted — user
// confirmed there's room on the target tablet (to be verified by
// playtest). Scoring formula unchanged (moves × 300); a taller board
// makes targets slightly easier, judged by playtest before any retune.
// Decision (2026-06-22): item (c) supersedes verses TODO #13 — tablet
// and phone verses are intentionally different games, so tablet-verses
// parity targets Tablet Arcade, not phone-verses' sandbox mechanics.
// Only functional change vs v1.15: the ROWS constant.
// In-game header label bumps to "Verses v1.16".
//
// =============================================================================
// MATCH-3 GAME v1.15 - TABLET VERSES (persistent shared bonus-move wallet)
//
// Bonus moves are now a single persistent wallet shared with Tablet
// Arcade (both read/write the core BONUS_MOVES_KEY). Changes:
//   - Pool hydrates from BONUS_MOVES_KEY at mount and persists via the
//     effect in BOTH modes (was: forced to 0 + non-persisted under
//     VERSES_MODE).
...
//     only changes on earn/spend — never auto-zeroed.
//   - "Arcade mode" handoff simplified: no add-to-key, no carry-receipt
//     banner (retired) — moves already live in the shared pool, so it
//     just navigates to tablet.html.
//   - "Arcade mode" button added to single-level passages' end screen.
//   - Bonus-move cap 99 → 999, near-cap warning 90 → 900 (matches the
//     same bump in Tablet Arcade v11.17, so the shared pool isn't
//     clamped back to 99 when the player earns in arcade).
// In lockstep with phone-verses v1.10, phone-verses-sandbox v1.9,
// phone arcade v13.6, tablet arcade v11.17.
//
// In-game header label bumps to "Verses v1.15".
//
// =============================================================================
// MATCH-3 GAME v1.14 - TABLET VERSES (content add — Psalms 46, 97, 139)
//
...
// in lockstep with phone-verses v1.9 + phone-verses-sandbox v1.8.
//
// In-game header label bumps to "Verses v1.14".
//
// =============================================================================
// MATCH-3 GAME v1.13 - TABLET VERSES (in-session ship batch)
//
// Bundles three independent changes shipped in the same session:
//
...
//
// In-game header label bumps to "Verses v1.13".
//
// No engine, gameplay, or scoring changes.
//
// =============================================================================
// MATCH-3 GAME v1.12 - TABLET VERSES (T-3a stats-blob writes)
//
...
// change. T-3b (still pending) handles storage-value renames +
// lockstep on phone keys.
//
// =============================================================================
...
// Game Constants
// V-4: VERSES_MODE board shrank 12 → 10 rows to free vertical space for
// the text bar and tighter laptop-viewport fit.
// v1.16 (item c, 2026-06-22): restored to 12 rows so the verses board
// matches Tablet Arcade exactly (12 × 10 @ 50px). The text bar still
// renders above the taller board; fit on the target tablet confirmed by
// the user, to be verified by playtest. COLS unchanged at 10.
const ROWS = 12;
const COLS = 10;
const TILE_SIZE = 50;
const TILE_GAP = 4;
...
// v1.13: biblical-order slug list driven by each game's `book` + `chapter`
...
// to a safe state.
// v1.17: Tiered target multiplier (replaces the old uniform
// moves × TARGET_PER_MOVE_DEFAULT). A longer level — or a multi-play
// drill, whose move count is larger — needs a steeper target, because
// more moves make big combos easier to line up. Applies to ALL play
// (Option A), so a drill's bigger move count naturally lands in a
// higher tier.
//   < 17 moves : × 300  (TARGET_PER_MOVE_DEFAULT)
//   17–24      : × 500
//   25–29      : × 600, then +× 100 every additional 5 moves.
// No ceiling on the multiplier yet — revisit once the verses simulation
// modes exist (verses TODO #9) rather than guess a cap now.
function versesTargetForMoves(moves) {
  if (moves < 17) return moves * TARGET_PER_MOVE_DEFAULT;
  if (moves < 25) return moves * 500;
...
function versesFlattenLevel(game, levelIndex = 0, plays = 1) {
  ...
  // v1.17: drill support. `plays` (1–4) repeats the level continuously —
  // chunk 0 is free only on the first pass; each repeat pass costs a move
  // for every chunk. Total moves = plays × totalChunks − 1
  // (11-chunk level: 10 moves at 1×, 21 at 2×, 43 at 4×). At plays = 1
  // this reduces to the original totalChunks − 1.
...
// v1.15: cap 99 → 999, warn 90 → 900 — persistent shared wallet, user
// wants headroom. Must match Tablet Arcade v11.17 (shared pool) or the
// arcade earn-clamp would slam the pool back to 99.
...
const VersesGame = ({
  ...
  // v1.17: selectedPlays — drill repetition count (1–4) chosen on the
  ...
  // v2.4: showLevelPassage — on-demand peek at the CURRENT LEVEL's full
  ...
  useEffect(() => {
    ...
    if (prevMovesRef.current !== null && moves < prevMovesRef.current) {
      setRevealedChunkIndex(prev => {
        ...
        // v1.17: in a drill (reps > 1) the passage loops — wrap back to
        // chunk 0 instead of capping, so each pass re-reveals from the
        // top. The end-of-round trigger is gated on moves === 0, not on
        // the reveal index, so looping here is safe. At reps === 1 keep
        // the original cap behavior.
  ...
  const restartGame = () => {
    ...
    // doesn't register as a decrement. v1.15: the bonus-moves pool is
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* v2.4: "See passage" — top-left, on the same row as the Back
            button and just to its right (below overlapped the score).
            Opens the current level's full passage as an on-demand peek
            during play (memorization aid). Hidden while the start/end
            passage modals are up (they already show the passage). Styled
            as a small pill in the verses accent color. */}
        ...
        <h1 style={{ margin: '0', color: '#333', fontSize: '20px', textShadow: '2px 2px 4px rgba(0,0,0,0.1)' }}>
          {VERSES_MODE && versesGame ? (
            <>
              ...
              {/* v1.17: drill progress — shown for any level played 2–4×. */}
            ...
            <>Verses <span style={{ fontSize: '12px', color: '#ccc' }}>v2.4</span></>
          )}
        </h1>
        <div style={{
          display: 'flex',
      ...
      {/* v2.4: current-level passage peek — opened by the top-left "See
          passage" button during play. Same Georgia two-column layout as the
          start/end passage modal, rendering only the current level's chunks;
          a single Close returns to the board. The full-screen overlay
          (zIndex 2000) intercepts pointer events, so the board is naturally
          frozen while it's open. */}
        ...
        <div style={{
          ...
          <div style={{
            ...
            {(() => {
              ...
              if (showStartModal) {
                // Start-of-round modal — drill selector (v1.17) + Back +
                // Begin (game / level).
                const beginLabel = isMultiLevel ? 'Begin level' : 'Begin game';
                const totalChunks = versesLevel.totalChunks;
                // v1.17: offer only the repetition counts whose
                // continuous-drill move total (N × chunks − 1) stays
                // at/under the 60-move ceiling. Unchoosable counts are
                // omitted entirely; if only 1× fits, no selector shows.
                ...
                return (
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '14px' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'center', gap: '12px' }}>
                      ...
                      <button
                        onClick={() => {
                          // v1.17: seed the round from the (possibly
                          // drilled) versesLevel so the chosen rep count
                          // takes effect — the mount initializer ran at
                          // 1× before the player picked.
                          if (VERSES_MODE && versesLevel) {
                            setScore(0);
                            setMoves(versesLevel.moves);
                            setLevelTarget(versesLevel.target);
              ...
              if (!isMultiLevel) {
                ...
                // Back to passage selections + Arcade mode (v1.15 —
                ...
                // v2.3: Arcade mode is offered on every free-replay screen (win
                ...
                // moves already live in BONUS_MOVES_KEY (the pool tablet arcade
                // reads at mount), so there's nothing to transfer — just navigate.
                buttons.push({
                  kind: 'secondary',
                  label: 'Arcade mode',
                  onClick: () => {
                ...
                buttons.push({
                  ...
                  label: 'Arcade mode',
                  onClick: () => {
                    // v1.15: shared-wallet handoff. Bonus moves already
                    // live in BONUS_MOVES_KEY (the same pool tablet
                ...
                // Repeat level (v1.17) + Back to level selections.
                // "Repeat level" replays this level via restartGame (at
                // the same rep count just played). Completion / stars /
                // best score live in the wrapper and update only on a
                // win, so they never regress on a repeat; Next level
                // stays available since this level is already unlocked.
...
const VersesPicker = ({ games, slugs, progress, onPick, onOpenTutorial, secretUnlock, onSecretPressStart, onSecretPressEnd }) => {
  return (
    <div style={{
      ...
      {/* v1.13 (#2): long-press 1.5 s on this header to toggle secret-
      ...
      <div style={{
        ...
        {slugs.map(slug => {
          ...
          // v1.13: long titles (>20 chars) shrink to 16px on the card.
...
// v1.13 (#2): secret-unlock localStorage key — read at mount, written on
...
const Match3Verses = () => {
  ...
  // v1.13 (#2): secret-unlock state. ON → every locked level in every
  ...
  // v1.15: persistent shared wallet. Hydrate from BONUS_MOVES_KEY at
  // mount and persist on every change — in BOTH modes. Same key Tablet
  // Arcade reads/writes, so the pool is one shared wallet across Verses
  // and Arcade (earn anywhere, spend anywhere, survives browser close +
  // the round trip to arcade and back).
  ...
  const handlePick = (slug) => {
    ...
    // v1.15: bonusMoves NOT reset — persistent shared wallet.
    ...
    <>
      <VersesGame
        ...
        onRestartGame={() => {
          ...
          // completedLevels (player stays in free-replay). v1.15: the
