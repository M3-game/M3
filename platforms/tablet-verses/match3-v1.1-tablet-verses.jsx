...
// =============================================================================
// MATCH-3 GAME v1.1 - TABLET VERSES (MEMORIZE MODE)
//
...
//
// Changes vs. v1.0:
//   - Content model: games/<slug>/game.js discovered via
//     import.meta.glob. V-2 hardcodes boot to games/titus-2-11-13/.
...
const CHUNK_REVEAL_BEAT_MS   = 200;   // pause after turn-settle before reveal fires
...
const versesGameModules = import.meta.glob('./games/*/game.js', { eager: true });
const versesGameRegistry = (() => {
  const out = {};
  for (const path in versesGameModules) {
    // path shape: "./games/<slug>/game.js"
    const match = path.match(/^\.\/games\/([^/]+)\/game\.js$/);
    if (!match) continue;
    const slug = match[1];
...
const VERSES_BOOT_SLUG = 'titus-2-11-13';
const versesActiveGame = versesGameRegistry[VERSES_BOOT_SLUG] || null;

// V-2: Flatten a single-level game to { level, flatChunks, totalChunks,
// moves, target }. For V-2 the runtime takes one level at a time — for
// single-level shapes we wrap the top-level `verses` as levels[0] so
// V-3 can extend without refactoring the game loop.
function versesFlattenLevel(game) {
  if (!game) return null;
  const level = game.levels && game.levels.length
    ? game.levels[0]
    : { title: game.title, verses: game.verses, targetScore: game.targetScore };
...
const versesActiveLevel = versesFlattenLevel(versesActiveGame);
...
const Match3Verses = () => {
  ...
  const [moves, setMoves] = useState(() => (
    VERSES_MODE && versesActiveLevel
      ? versesActiveLevel.moves
  ...
  const [levelTarget, setLevelTarget] = useState(() => {
    if (VERSES_MODE && versesActiveLevel) return versesActiveLevel.target;
  ...
  // pendingRevealRef: set true when a successful swap decrements moves;
  // cleared after the post-settle reveal fires.
  ...
  // verses-mode handle — derived at module load, stable.
  const versesGame = versesActiveGame;
  const versesLevel = versesActiveLevel;
  ...
  // V-2: Successful-swap detector. Each real move (match → moves
  // decrements) arms pendingRevealRef so the post-settle effect below
  // fires exactly one reveal per turn. Watching `moves` (rather than
  // splicing into the two setMoves call sites) keeps the swap handlers
  // untouched. Non-match swaps don't decrement moves, so they don't
  // arm a reveal; restartGame resets prevMovesRef so the bump from 0
  // back to initial isn't misread as a decrement.
  useEffect(() => {
    if (!VERSES_MODE) return;
  ...
  // V-2: Post-settle chunk reveal. Runs on the same gating conditions
  // as the arcade game-end effect (turnComplete + no combo + no
  // pending specials + not animating). When a reveal is armed, wait
  // CHUNK_REVEAL_BEAT_MS so the settled board reads before the text
  // bar shifts.
  useEffect(() => {
    ...
    }, CHUNK_REVEAL_BEAT_MS);
    return () => clearTimeout(t);
  }, [turnComplete, isAnimating, combo, pendingSpecials.length, gameState, moves]);
  ...
  useEffect(() => {
    ...
    // Also wait for the board to be quiet — no in-flight cascade or
    // pending reveal — so the modal doesn't cover an active animation.
    if (isAnimating || combo > 0 || pendingSpecials.length > 0) return;
    if (pendingRevealRef.current) return;
    const t = setTimeout(() => setShowPassageModal(true), PASSAGE_HOLD_MS);
    return () => clearTimeout(t);
  }, [moves, revealedChunkIndex, showPassageModal, gameState, isAnimating, combo, pendingSpecials.length]);
  ...
  const handleCanvasClick = (e) => {
    ...
    if (VERSES_MODE && moves <= 0) return; // V-2: freeze input during 2.5s pre-modal hold
  ...
  const restartGame = () => {
    ...
    if (VERSES_MODE) {
      ...
      setBankedMoves(0);
      try { localStorage.setItem(BANKED_KEY, '0'); } catch {}
    }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h1 style={{ margin: '0', color: '#333', fontSize: '20px', textShadow: '2px 2px 4px rgba(0,0,0,0.1)', fontFamily: VERSES_MODE ? 'Georgia, serif' : 'inherit' }}>
          {VERSES_MODE && versesGame
            ? versesGame.title
            : <>Verses <span style={{ fontSize: '12px', color: '#ccc' }}>v1.1</span></>}
        </h1>
        <div style={{
          display: 'flex',
          ...
          {/* V-2: In VERSES_MODE, target display flips gold + ✓ once
              targetReached is true and stays that way through
              end-of-round. Paired with the one-shot Target-Reached toast. */}
      ...
      {/* V-2: Rolling 3-chunk text bar. Current chunk + 2 prior. Georgia
          serif; two-column grid (reference flush-left, content indented).
          Reference renders only on chunks that begin a new verse (the
          rest of that verse's chunks leave the reference column blank).
          Current chunk: 22px, fontWeight 500, full contrast. Prior two:
          15px, dimmed #888 (on the light card background). Newest row
          fades/slides in over CHUNK_REVEAL_FADE_MS keyed on chunk
          index, so re-renders of stable rows don't re-animate.
          min-height prevents the canvas from shifting during reveal. */}
          ...
          <div style={{
            ...
            {rows.map(row => {
              ...
              return (
                ...
                >
                  <span style={{ textAlign: 'left', fontStyle: 'italic', whiteSpace: 'nowrap' }}>
      ...
      {/* V-2: Passage-reveal modal — fires at end-of-round after the
          final chunk has been revealed and a PASSAGE_HOLD_MS hold.
          Dimmed overlay (no blur). Shows translation label + full
          passage in the same Georgia two-column layout as the rolling
          text bar (at full contrast, no dimming), and a single Play
          Again button. No auto-dismiss — player clicks to restart. */}
      {VERSES_MODE && versesLevel && showPassageModal && (
        <div style={{
          position: 'fixed',
          top: 0, left: 0, right: 0, bottom: 0,
      ...
      <div style={{
        ...
        {VERSES_MODE ? (
          <>
            <strong>Match 3+ tiles to reveal each chunk.</strong>{' '}
            Line / bomb / cross specials reward bigger matches.{' '}
            Hit the target to earn a ✓ — keep playing through all chunks to see the full passage.
          </>
        ) : (
          <>
