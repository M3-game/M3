...
// =============================================================================
// MATCH-3 GAME v1.16 — PHONE VERSES SANDBOX (level-select back button overlapped title)
//
// Lockstep port of phone-verses v2.6; see that file's v2.6 block for the full
// reasoning. Reported on phone-verses, but this file's level-select block was
// character-for-character identical, so it carried the same bug.
//
// The "← Back to passage selections" button overlapped the passage title on
// the level-select screen. Static layout collision: the button is absolutely
// positioned at top:20px and 32px tall, occupying the band 20–52px, while the
// title starts at the container's 40px top padding — 12px inside it. Affected
// every passage, since the button's right edge passes the middle of the
// screen and the title is centred. FIX: top padding 40px → 68px.
//
// ALSO — preventive safe-area padding on both select screens (user-approved
// 2026-08-09): viewport-fit=cover runs the page under the iOS status bar and
// the page shifts up when Safari's toolbar collapses, so the back button
// could land behind the clock and battery. Preventive, not observed. The
// inset is applied to the button ITSELF as well as the container, because an
// absolutely positioned element is placed from the container's edge and does
// not move when the container's padding changes. Both wrappers also go 100vh
// → 100dvh.
//
// SANDBOX DIFFERENCE FROM phone-verses: the passage-select screen here has no
// top-left control — the Tutorial pill was never ported to the sandbox — so
// that screen gets the inset on its container only, and there is no second
// control to adjust.
//
...
//
// No gameplay, scoring, board-logic, or content change. In-game header label
// bumps to "VERSES-SANDBOX v1.16".
//
// =============================================================================
// MATCH-3 GAME v1.15 — PHONE VERSES SANDBOX (vertical layout: balance, fit, short phones)
//
// Lockstep port of phone-verses v2.5; see that file's v2.5 block for the full
// reasoning and the device-by-device measurements. Follow-on to the v1.14
// header-clipping fix: v1.14 stopped the header being cut off on the
// reporting player's phone, this makes the vertical layout correct on phones
// we can't test and balances the space on the ones we can. Four changes:
//
// 1) VERTICAL SPACE REBALANCED. Wrapper padding was 8px top / 60px bottom.
//    Centring happens INSIDE the padding, so that 52px difference was the
//    whole reason the game sat high with dead space beneath it (measured
//    29pt above vs 86pt below on a 428×926pt phone). Both are now 25px plus
//    their safe-area inset, so the ~117pt of spare height splits evenly
//    (~58pt each end). This balances the empty space rather than removing
//    it — with the board size fixed, it has to live somewhere. User chose
//    the even split over spreading the slack between elements (2026-08-09).
//
// 2) OVERFLOW CAN NO LONGER EAT THE HEADER. justifyContent 'center' →
//    'safe center', which falls back to top-alignment when content
//    overflows instead of splitting it evenly and losing the half above the
//    screen — the same class of failure v1.14 fixed, which would otherwise
//    return on any phone too short for the layout.
//
// 3) THE INSTRUCTIONS PANEL YIELDS ON SHORT PHONES — BY MEASUREMENT, NOT BY
//    DEVICE TABLE. New `hideInstructions` state: a useLayoutEffect compares
//    the page's rendered height against visualViewport.height after every
//    render, and if the layout genuinely does not fit AND the player has
//    made two moves, the panel goes. On a phone with room to spare it never
//    fires — hiding it there would trade a useful panel for a bigger band
//    of empty background (user's point, 2026-08-09). Two moves because the
//    panel (~118pt with its gap) is nearly three times the passage text
//    bar's total growth (~42pt), making the TALLEST moment of a round its
//    beginning; retiring the panel at move 2 means the two never coexist at
//    full size.
//
// 4) TILE SIZE IS NOW HEIGHT-AWARE — it took the viewport WIDTH only, so a
//    short phone got the same tall stack as a tall one. Priority order
//    agreed with the user: keep the board as large as the width allows,
//    give up the instructions panel next, shrink tiles only as a last
//    resort. See VERSES_CHROME_PX.
//
// In-game header label bumps to "VERSES-SANDBOX v1.15". No gameplay,
// scoring, board-logic, or content change.
//
// =============================================================================
// MATCH-3 GAME v1.14 — PHONE VERSES SANDBOX (in-game header clipped at the top — fix)
//
// Lockstep port of the phone-verses v2.4 fix; see that file's v2.4 block for the
// full diagnosis. Summary: the top of the white in-game header card was
// intermittently cut off on iPhone, with the iOS status bar drawn over the
// title. Three causes compounded — (1) no CSS reset exists anywhere in the
// project, so <body> kept its default 8px margin and the document was always
// 100vh + 16px; (2) 100vh on iOS Safari is the toolbars-hidden height, adding
// another ~60–90px of scroll room whenever the toolbar shows; (3)
// touchAction:'none' on the wrapper (there so swipes reach the board) made that
// scroll unrecoverable, and nothing resets scrollTop; (4) viewport-fit=cover +
// black-translucent status bar run the page under the status bar with no
// env(safe-area-inset-top) compensation. Intermittency was just whether the page
// happened to sit at scroll 0.
//
// Fix — three changes:
//   a) `<style>html,body{margin:0;padding:0}</style>` added to
//      phone-verses-sandbox.html (scoped to this page).
//   b) Wrapper paddingTop: calc(env(safe-area-inset-top, 0px) + 8px), written as
//      longhands so the shorthand can't clobber it.
//   c) Wrapper minHeight 100vh → 100dvh.
// justifyContent: 'center' deliberately left alone (top-anchoring would move the
// game to the top of the screen on every device where it already fits).
//
// No gameplay, scoring, board, or content change. In-game header label bumps to
// "VERSES-SANDBOX v1.14".
//
// =============================================================================
// MATCH-3 GAME v1.13 — PHONE VERSES SANDBOX (title nudge tweak for medium titles)
//
// Small visual tweak mirroring phone-verses v2.3: the medium title tier (15–20
// chars) read too far right with the full 48px right-nudge; eased it back to
// 28px. The very-long tier (> 20 chars) keeps 48px. Font tiers unchanged.
// In-game header label bumps to "VERSES-SANDBOX v1.13".
//
// =============================================================================
// MATCH-3 GAME v1.12 — PHONE VERSES SANDBOX (port: bonus moves + free-replay arcade + see passage)
//
// Port of the three verses features to the sandbox, mirroring phone-verses
// v2.2 (structurally identical; arcade destination phone.html; phone
// single-column passage layout; header/see-passage treatment same as
// phone-verses):
//   1) USE BONUS MOVES IN VERSES. LOSS (0 moves, below target): arcade "Use
//      bonus moves / End and carry" banner drops down instead of ending. WIN:
//      "Use bonus moves" button on the passage screen returns to the board
//      (keeps the 1.5× victory round). Decision 4: loss-path bonus play stays
//      1× and its "Target reached — 1.5×!" toast is suppressed. Touch points:
//      end-of-round effect, 3 input-freeze guards, endLevelCarryBonus (VERSES
//      branch), resumeBonusFromPassage, passage-screen button matrix,
//      target→victory-round effect.
//   2) ARCADE ON FREE-REPLAY. "Arcade mode" button on free-replay end screens.
//   3) "SEE PASSAGE" BUTTON. Top-left during play (the in-game "← Back" button
//      was removed to make room, mirroring phone-verses); long titles get font
//      tiers (>20→16px, 15–20→18px) + a right-nudge; "Level N of M" → "N/M".
// No verses gameplay/scoring formula change. In-game header label bumps to
// "VERSES-SANDBOX v1.12".
//
// =============================================================================
// MATCH-3 GAME v1.11 — PHONE VERSES SANDBOX (bug fix: reveal + end freeze)
//
// Bug fix — two visible symptoms, one root cause (same fix as phone-verses v2.1
// / tablet-verses v2.1). On the start-of-round modal the "Begin game" handler
// (and the "Play again" restart path) reset the reveal tracker `prevMovesRef` to
// null, but the accompanying setMoves(versesLevel.moves) was a no-op whenever
// moves already equaled that value (the default 1× case — every game). With
// moves unchanged, the reveal effect never re-ran to restore the tracker, so the
// FIRST move's line-reveal was skipped, leaving every reveal one line behind for
// the whole round. The two symptoms:
...
// In-game header label bumps to "VERSES-SANDBOX v1.11".
//
// =============================================================================
// MATCH-3 GAME v1.10 — PHONE VERSES SANDBOX (play-through options)
//
// June-20 update item (b), ported from tablet-verses v1.17 / phone-verses
// v1.11: "Play N times" drill (1×/2×/3×/4× selector on the start modal,
// continuous loop, move budget N × chunks − 1, "Pass X of N" indicator,
// 60-move ceiling on offered counts) + "Repeat level" button on the
// multi-level non-final first-pass win screen. Tiered target multiplier
// (versesTargetForMoves) extends the ×300/×500 split upward, all play
// (Option A): <17 →×300, 17–24 →×500, 25–29 →×600, +×100 per +5 moves.
// In-game header label bumps to "VERSES-SANDBOX v1.10".
//
// =============================================================================
// MATCH-3 GAME v1.9 — PHONE VERSES SANDBOX (persistent bonus-move wallet)
//
// Mirrors phone-verses v1.10's persistent bonus-move wallet, but the
// sandbox pool is SELF-CONTAINED — its key is sandbox-scoped
// ('match3_phone_bonusMoves_sandbox'), which the real phone arcade does
// not read. So bonus moves persist across sandbox-verses games + browser
// sessions, but don't mix with the real game. Changes:
//   - Pool hydrates from the sandbox key at mount and persists in BOTH
//     modes (was: forced to 0 + non-persisted under VERSES_MODE).
//   - Removed the 4 artificial resets (handlePick, handleBackToPicker,
//     restartGame VERSES_MODE block, wrapper onRestartGame).
//   - "Arcade mode" handoff simplified to just navigate (carry-receipt
//     write retired).
//   - "Arcade mode" button added to single-level passages' end screen.
//   - Bonus-move cap 99 → 999, near-cap warning 90 → 900 (parity with
//     the player-facing platforms).
// In lockstep with tablet-verses v1.15, phone-verses v1.10, phone
// arcade v13.6, tablet arcade v11.17.
//
// In-game header label bumps to "VERSES-SANDBOX v1.9".
//
// =============================================================================
// MATCH-3 GAME v1.8 — PHONE VERSES SANDBOX (content add — Psalms 46, 97, 139)
//
...
// in lockstep with tablet-verses v1.14 + phone-verses v1.9.
//
// In-game header label bumps to "VERSES-SANDBOX v1.8".
//
// =============================================================================
...
//
//   #1 PHONE_BONUS_MOVES_KEY     'match3_phone_bonusMoves_sandbox'
//   #2 VERSES_PROGRESS_KEY_PREFIX 'm3_phone_verses_sandbox_'
//   #3 RUN_CURRENT_KEY            'match3_currentRun_sandbox'
//   #4 RUN_LONGEST_KEY            'match3_longestRun_sandbox'
//   #5 high-score                 'match3_highScore_sandbox'
//   #6 high-combo                 'match3_highCombo_sandbox'
//   #7 high-turn-score            'match3_highTurnScore_sandbox'
//   #8 STATS_KEY (imported)       — see note below
//   #9 carry-out write            'm3_arcade_carry_from_verses_phone_sandbox'
//
...
// v1.15: `safe center` tells the browser to centre the game vertically but
...
// v1.15: HEIGHT BUDGET (lockstep with phone-verses v2.5). Until now the
// tile size came from the viewport WIDTH only, so a narrow phone got
// smaller tiles but a SHORT phone got the same tall stack of header +
// text bar + board + instructions. On an iPhone SE-class screen (667pt
// tall) that stack does not fit, and because the page centres its
// content the overflow above the top is unreachable — the header would
// be clipped permanently rather than intermittently. So the tile size
// now takes whichever of the two budgets is smaller.
//
// VERSES_CHROME_PX is everything on screen that is NOT the board's tiles
// and inter-tile gaps, measured off a real device and rounded UP for
// narrower phones, where the same text wraps onto more lines:
//   wrapper padding (25 top + 25 bottom) 50 · header card ~100 ·
//   header → text bar gap 14 · text bar at its tallest ~135 ·
//   text bar → board gap 10 · board container padding + border 38
//   = ~347 → 350
...
// BEFORE shrinking the board, because the board is the game.
//
// This is the one place in the v1.15 work that uses an estimate rather
...
const PHONE_BONUS_MOVES_KEY = 'match3_phone_bonusMoves_sandbox';
...
// to a safe state.
// v1.10: Tiered target multiplier (item b). Extends the ×300 (<17) /
// ×500 (≥17) split upward; applies to ALL play (Option A).
//   < 17 →×300, 17–24 →×500, 25–29 →×600, +×100 per +5 moves.
...
function versesFlattenLevel(game, levelIndex = 0, plays = 1) {
  ...
  // v1.10: drill support (item b). `plays` (1–4) repeats the level
...
const VERSES_PROGRESS_KEY_PREFIX = 'm3_phone_verses_sandbox_';
...
// v1.9: cap 99 → 999, warn 90 → 900 — persistent wallet, parity with
// the player-facing platforms.
...
const RUN_CURRENT_KEY = 'match3_currentRun_sandbox';
const RUN_LONGEST_KEY = 'match3_longestRun_sandbox';
...
const VersesGame = ({
  ...
  // v1.10: selectedPlays — drill repetition count (1–4) from the
  // start-of-round modal. Resets to 1 on level change (wrapper remounts).
  ...
  const [highScore, setHighScore] = useState(() => {
    const stored = localStorage.getItem('match3_highScore_sandbox');
  ...
  // v1.12: showLevelPassage — on-demand peek at the CURRENT LEVEL's full
  ...
  // v1.15: pageRef + hideInstructions — the "does it actually fit?" check.
  //
  ...
  // v1.15: DOES THE LAYOUT ACTUALLY FIT? — the measurement behind
  ...
  // that caused the v1.14 header-clipping bug. visualViewport.height is
  ...
  // v1.15: re-run the fit check when the visible height changes on its own —
  ...
  const [allTimeHighCombo, setAllTimeHighCombo] = useState(() => {
    const stored = localStorage.getItem('match3_highCombo_sandbox');
    return stored ? parseInt(stored, 10) : 0;
  });
  const [allTimeHighTurnScore, setAllTimeHighTurnScore] = useState(() => {
    const stored = localStorage.getItem('match3_highTurnScore_sandbox');
    return stored ? parseInt(stored, 10) : 0;
  });
  ...
  useEffect(() => {
    localStorage.setItem('match3_highCombo_sandbox', allTimeHighCombo.toString());
  }, [allTimeHighCombo]);
  
  useEffect(() => {
    localStorage.setItem('match3_highTurnScore_sandbox', allTimeHighTurnScore.toString());
  }, [allTimeHighTurnScore]);
  
  useEffect(() => {
    localStorage.setItem('match3_highScore_sandbox', highScore.toString());
  }, [highScore]);
  ...
  useEffect(() => {
    ...
    // v1.12 (decision 4): the target-hit toast reads "✓ Target reached — 1.5×!"
  ...
  useEffect(() => {
    ...
    if (prevMovesRef.current !== null && moves < prevMovesRef.current) {
      setRevealedChunkIndex(prev => {
        ...
        // v1.10: in a drill (reps > 1) the passage loops — wrap to chunk
        // 0 instead of capping. reps === 1 keeps the cap.
  ...
  useEffect(() => {
    ...
    // v1.12: bonus-moves flow slots in here, before the passage screen.
  ...
  // v1.12: WIN-path resume. The passage screen carries a "Use bonus moves"
  ...
  const endLevelCarryBonus = () => {
    ...
    setScore(prev => prev + specialsBonus);
    // v1.12: In VERSES_MODE, ending bonus-move play opens the passage screen
  ...
  const handleCanvasClick = (e) => {
    ...
    if (VERSES_MODE && ((moves <= 0 && !(usingBonusMoves && bonusMoves > 0)) || showStartModal || showPassageModal)) return; // V-2: freeze input while a passage modal is open or during the 2.5s pre-modal hold. v1.12: but allow play while actively spending bonus moves (moves===0 but wallet remaining).
  ...
  const restartGame = () => {
    ...
    // v1.15: bring the instructions panel back for the new round. The
    ...
    // doesn't register as a decrement. v1.9: the bonus-moves pool is
    // NO LONGER zeroed here — it's a persistent wallet now, so "Play
    // again" keeps the player's accumulated moves.
    if (VERSES_MODE) {
      ...
      // v1.11: match prevMovesRef to the move count at play start (not null) so
  ...
  return (
    <div ref={pageRef} style={{
      ...
      alignItems: 'center',
      // v1.15: 'center' → 'safe center'. The `safe` keyword means "centre
      // it, but if the content is TALLER than the screen, fall back to
      // aligning from the top instead". Plain `center` splits any overflow
      // evenly above and below — and the half that goes above the top of
      // the screen cannot be scrolled to, so the header is simply lost.
      // That is the same failure v1.14 fixed for a different reason, and it
      // would return on any phone too short to fit the layout. With `safe`,
      // a short phone pushes its overflow to the BOTTOM, where the least
      // important content already sits. Browsers without support drop the
      // declaration and fall back to flex-start, the safe direction anyway.
      justifyContent: SAFE_CENTER,
      // v1.14: 100vh → 100dvh. On iOS Safari 100vh is the *large* viewport
      ...
      // once the page drifted, leaving the header's top cut off.
      minHeight: '100dvh',
      background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)',
      fontFamily: 'Arial, sans-serif',
      // v1.14: was `padding: '0 0 60px 0'`. phone-verses-sandbox.html sets
      // viewport-fit=cover + black-translucent status bar, which run the page
      // under the status bar/notch; nothing compensated, so the status bar sat
      // on the title. Longhands (not the shorthand) so paddingTop can't be
      // clobbered by property order.
      //
      // v1.15: TOP AND BOTTOM MADE SYMMETRIC (was 8px top / 60px bottom).
      ...
      // one is 0 in normal Safari browsing but non-zero when launched from
      // the Home Screen, and the bottom one keeps content off the home
      // indicator in that same case.
      ...
      <div style={{
        ...
        {/* v1.12: the in-game "← Back" button was REMOVED on phone (mirrors
            phone-verses v2.2) to free the top-left header spot for "See
            passage" — phone header is tight (Back · title · toggle left no
            room). Mid-game back is rarely needed; to leave, finish the level
            (moves are limited) and use the end screen's "Back to passage
            selections". `onBack` is still used by the start-of-round modal's
            Back button. */}
        {/* v1.12: "See passage" — top-left (the old Back button's spot).
            Opens the current level's full passage as an on-demand peek during
            play (memorization aid). Hidden while the start/end passage modals
            are up. Small pill; long titles get font tiers + a right-nudge (see
            the h1 below) so they clear this button. */}
        ...
        {/* V-3b: title + (optional) inline "Level N of M" indicator
            ...
            work. */}
        {/* v1.12/v1.13: on phone, long verses titles get a smaller font + a
            right-nudge so the centered title + "N/M" don't run under the
            top-left "See passage" button (free space on the right). Char-
            length tiers: > 20 → 16px + 48px nudge; 15–20 → 18px + 28px nudge
            (v1.13: eased back from 48px, mirrors phone-verses v2.3 — the full
            nudge read too far right for medium titles); ≤ 14 → 20px, no nudge.
            Mirrors phone-verses. */}
        <h1 style={{
          margin: '0',
          color: '#333',
          ...
          {VERSES_MODE && versesGame ? (
            <>
              ...
              {isMultiLevel && Array.isArray(versesGame.levels) && (
                <span style={{
                  ...
                  {/* v1.12: shortened "· Level N of M" → "N/M" (mirrors
                      phone-verses v2.2) to keep long titles clear of the
                      top-left "See passage" button. */}
                  {levelIndex + 1}/{versesGame.levels.length}
                </span>
              )}
              {/* v1.10: drill progress — shown for any level played 2–4×. */}
            ...
            <>VERSES-SANDBOX <span style={{ fontSize: '12px', color: '#ccc' }}>v1.16</span></>
          )}
        </h1>
      ...
      {/* v1.12: current-level passage peek — opened by the top-left "See
          passage" button during play. Same single-column Georgia layout as
          the start/end passage modal, rendering only the current level's
          chunks; a single Close returns to the board. The full-screen overlay
          (zIndex 2000) intercepts pointer events, so the board is frozen. */}
        ...
        <div style={{
          ...
          <div style={{
            ...
            {(() => {
              ...
              if (showStartModal) {
                // Start-of-round modal — drill selector (v1.10) + Back +
                // Begin (game / level).
                const beginLabel = isMultiLevel ? 'Begin level' : 'Begin game';
                const totalChunks = versesLevel.totalChunks;
                // v1.10: offer only rep counts whose drill total
                // (N × chunks − 1) stays ≤ the 60-move ceiling. If only
                // 1× fits, no selector shows.
                ...
                return (
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '14px' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'center', gap: '12px' }}>
                      ...
                      <button
                        onClick={() => {
                          // v1.10: seed the round from the (possibly
                          ...
                          if (VERSES_MODE && versesLevel) {
                            ...
                            setRevealedChunkIndex(0);
                            setTargetReached(false);
                            // v1.11: see restart path — match tracker to the move
              ...
              if (!isMultiLevel) {
                ...
                // Back to passage selections + Arcade mode (v1.9 — just
                // navigates; sandbox pool is self-contained).
                ...
                // v1.12: Arcade mode is offered on every free-replay screen (win
                // OR fail) — free-replay means the whole passage is already
                // fully completed, so an arcade handoff is always valid here,
                // matching single-level parity. Just navigate (sandbox pool is
                // self-contained; no transfer).
                buttons.push({
                  kind: 'secondary',
                  label: 'Arcade mode',
                  onClick: () => {
                ...
                buttons.push({
                  ...
                  label: 'Arcade mode',
                  onClick: () => {
                    // v1.9: just navigate. Sandbox pool is self-contained
                    // (sandbox-scoped key the real arcade doesn't read),
                    // so there's no transfer. Carry-receipt write retired.
                    const base = (typeof import.meta !== 'undefined' && import.meta.env && import.meta.env.BASE_URL) || '/';
                    window.location.href = `${base}phone.html`;
                  },
                });
                ...
                // Repeat level (v1.10) + Back to level selections.
                // Replays at the rep count just played; completion/stars/
                // best never regress; Next level stays available.
              ...
              // v1.12: WIN-path bonus-moves button (decision 1). Only when the
              // player won this round and has bonus moves to spend. Slotted
              // right after the primary action so it reads as an opt-in extra.
              // On a LOSS the offer already came via the banner before this
              // screen, so no button is added there.
              if (won && bonusMoves > 0) {
      ...
      {/* Instructions */}
      {/* v1.15: hidden once the layout has been measured as not fitting AND
...
const VersesPicker = ({ games, slugs, progress, onPick, secretUnlock, onSecretPressStart, onSecretPressEnd }) => {
  return (
    <div style={{
      ...
      flexDirection: 'column',
      alignItems: 'center',
      // v1.16: 100vh → 100dvh, matching the game screen. On iOS Safari 100vh
      ...
      background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)',
      fontFamily: 'Arial, sans-serif',
      // v1.16: top padding gains the safe-area inset. This page is set to use
      ...
      // put the clock on top of the in-game title before v1.14. Reserving the
      // inset keeps this screen's heading clear of that strip. Unlike
      // phone-verses, this screen carries no top-left control — the Tutorial
      // pill was never ported to the sandbox — so there is nothing else here
      // needing the inset.
...
const VersesLevelSelect = ({ game, gameProgress, completedLevels, fullCompleted, onPickLevel, onBack, onShowFullPassage, secretUnlock, onSecretPressStart, onSecretPressEnd }) => {
  ...
  return (
    <div style={{
      ...
      flexDirection: 'column',
      alignItems: 'center',
      // v1.16: 100vh → 100dvh, matching the game screen (see the passage-select
      // screen above for why).
      minHeight: '100dvh',
      background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)',
      fontFamily: 'Arial, sans-serif',
      // v1.16: TOP PADDING RAISED 40px → 68px, PLUS the safe-area inset.
      //
      // The bug (player-reported 2026-08-09 on phone-verses, screenshot of
      // Isaiah 52:13–53:12; this file's block was character-for-character
      // identical): the "← Back to passage selections" button overlapped the
      // passage title. Cause is a static layout collision, not a scroll or
      // status-bar problem — the button is absolutely positioned at top:20px
      // and is 32px tall (14px text + 12px vertical padding + 4px border),
      // so it occupies the band 20–52px, while the title is a normal in-flow
      // element starting at this container's 40px top padding — 12px inside
      // the button's band. The padding was never raised to account for a
      // button taken out of flow.
      //
      ...
      //
      // 68px = the button's 52px lower edge + ~16px of breathing room.
      ...
      <button
        ...
        style={{
          position: 'absolute',
          // v1.16: + the safe-area inset. Absolute positioning is measured from
...
const Match3Verses = () => {
  ...
  // and skip PHONE_BONUS_MOVES_KEY persistence — memorize-mode
  // bonus moves are session-scoped (zero on return to picker). In
  // !VERSES_MODE (arcade fallback), hydrate from
  // PHONE_BONUS_MOVES_KEY at mount and persist via the effect
  // below. (!VERSES_MODE branch is dead in this file but preserved
  // for symmetry; VERSES_MODE = true at module load.)
  // v1.9: persistent wallet — hydrate from the sandbox key at mount and
  // persist on every change in BOTH modes (self-contained; the real
  // phone arcade does not read this sandbox-scoped key).
  ...
  const handlePick = (slug) => {
    ...
    // v1.9: bonusMoves NOT reset — persistent wallet.
    ...
    <>
      <VersesGame
        ...
        onRestartGame={() => {
          ...
          // completedLevels (player stays in free-replay). v1.9: the
          // bonus-moves pool persists (wallet) — not zeroed.
