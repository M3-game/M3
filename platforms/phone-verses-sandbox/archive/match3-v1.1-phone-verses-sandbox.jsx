...
const TARGET_PER_MOVE_DEFAULT = 1000;  // VS-1: sandbox bumps to moves × 1000 (was 250 in main phone-verses) to keep difficulty meaningful given Mechanic A + B raise scoring volume
...
// processMatches.
const NEIGHBOR_BIAS_PCT = 14; // v1.1 (2026-05-07): 10 → 14 after ~20-game v1.0 playtest.
...
const VersesGame = ({
  ...
  const fillEmptySpaces = (currentGrid, generation) => {
    ...
    // VS-1 Mechanic B: consume a pending big-turn special drop. One randomly-
    // chosen refilled cell becomes the supernova/hypernova tile.
    if (_pendingSpecialDrop.kind && refilledCells.length > 0) {
      ...
      console.log('[VS-1 big_turn_drop]', { kind, specialType, at: [sr, sc] });
  ...
  const restartGame = () => {
    ...
    if (VERSES_MODE) {
      ...
      setBonusMoves(0);
      try { localStorage.setItem(PHONE_BONUS_MOVES_KEY, '0'); } catch {}
    }
      ...
      <div style={{
        ...
        <h1 style={{ margin: '0', color: '#333', fontSize: '20px', textShadow: '2px 2px 4px rgba(0,0,0,0.1)' }}>
          {VERSES_MODE && versesGame ? (
            ...
            <>VERSES-SANDBOX <span style={{ fontSize: '12px', color: '#ccc' }}>v1.1</span></>
          )}
        </h1>
              ...
              if (!isMultiLevel) {
                ...
                buttons.push({
                  ...
                  onClick: () => {
                    ...
                    // phone arcade's PHONE_BONUS_MOVES_KEY (the
                    // same key the arcade reads at mount) AND write a
                    // separate carry-receipt under
                    // m3_arcade_carry_from_verses_phone (T-3b 2026-05-03;
                    // was m3_arcade_carry_from_verses_phone418) so phone
                    // arcade v13.5 can show a transfer banner on entry.
                    ...
                    try {
                      ...
                      const existing = parseInt(localStorage.getItem(PHONE_BONUS_MOVES_KEY) || '0', 10);
                      localStorage.setItem(PHONE_BONUS_MOVES_KEY, (existing + carryAmount).toString());
                      // VS-1: write to sandbox-scoped carry key so main phone arcade
                      // doesn't pick up sandbox-tuned bonus pools. Future phone-arcade-
                      // sandbox (or enhanced phone arcade) will read this key.
                      localStorage.setItem('m3_arcade_carry_from_verses_phone_sandbox', JSON.stringify({
                        amount: carryAmount,
...
const Match3Verses = () => {
  ...
  const [bonusMoves, setBonusMoves] = useState(() => (
    VERSES_MODE ? 0 : parseInt(localStorage.getItem(PHONE_BONUS_MOVES_KEY) || '0', 10)
  ));
  useEffect(() => {
    if (VERSES_MODE) return;
