...
// migration at module load. Desktop doesn't otherwise import core, so
// this explicit call ensures the migration runs on first visit to
// desktop after T-3b deploys (idempotent — no-op on subsequent loads).
migrateBonusMovesKeys();

// =============================================================================
// MATCH-3 GAME v12.3 - DESKTOP (T-3b storage-value migration)
//
// Session T-3b (2026-05-03): Storage-string VALUE migration (data half,
// B part of T-3 split). Desktop's local `BONUS_MOVES_KEY` value flips
// from 'match3_desktop_bankedMoves' to 'match3_desktop_bonusMoves'.
// Existing player data migrates automatically on first load via core's
// `migrateBonusMovesKeys()` (imported above; runs at module load,
...
// `bonusMoves` (the carry pool of earned +1 moves). Mirrors T-1's pattern
// applied to tablet/tablet-verses/phone-verses; T-2 completed the sweep
// for desktop, time-attack, phone arcade, and campaign.
//
// Desktop-specific changes (T-2):
//   - All identifier renames per the standard inventory (state vars, refs,
//     setters, functions, the local `BANKED_MOVES_KEY` constant).
//   - User-visible strings: "BONUS ROUND" / "Bonus Round" / "bonus round"
//     → "VICTORY ROUND" / "Victory Round" / "victory round"; "Banked moves"
//     / "banked moves" → "Bonus moves" / "bonus moves".
//   - Comments + version-history headers updated for coherence.
//
// Stats-data layer: desktop doesn't currently call recordGameResult
// (no AdminPanel integration — pre-existing gap). N/A here.
//
// No behavior changes. Pure data-key migration.
//
// =============================================================================
// MATCH-3 GAME v11 - DESKTOP
// Performance-optimized with HTML5 Canvas rendering
// 10x12 grid, R1 Sunflower, Blue Jewel, Green Clover (board v7 designs)
// v10-desktop: Bugfix release (all three platforms synced to v10)
//   Fix 1–11: (see original v10 notes)
...
// v10.4: Bonus move system (BONUS_MOVE_INTERVAL=10000, refs, useEffect)
// v10.5: Bonus move timing fix, startBonusRound/endLevelEarly flush ref
// v10.5.1–v10.5.3: Bonus move burst animation at Moves counter
// v10.5.4: Fix tile fall animation — applyGravity directly seeds animStateRef
// v10.6: Fix K2 dragStart useState→useRef, Fix K1 swapFiredRef phantom click guard,
...
const BONUS_ROUND_MULTIPLIER = 1.5;   // v8.10: Points multiplier during victory round
...
// T-3b (2026-05-03): VALUE flipped from 'match3_desktop_bankedMoves'.
// Migration in core/AdminPanel.jsx runs at module load via the import
// at top of this file.
const BONUS_MOVES_KEY = 'match3_desktop_bonusMoves';
...
const drawSpecialIcon = (ctx, x, y, size, specialType) => {
  ...
  const icon = icons[specialType] || '✨';
  
  ctx.save();
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  
  if (specialType === 'line' || specialType === 'cross') {
    // Corner position for line and cross (21px)
    const iconX = x + size - 14;
    const iconY = y + size - 10;
    ...
    // Centered for bomb, supernova, hypernova (14px)
    ctx.font = '14px Arial';
    ctx.shadowColor = 'rgba(0,0,0,0.8)';
    ctx.shadowBlur = 4;
...
const Match3Game = () => {
  ...
  // v10 Fix 9: Set-based O(1) lookups for tile state checks in rAF loop (backport from phone/tablet)
  ...
  // v10.2: animStateRef — per-tile animation positions, avoids mutating tile objects
  ...
  useEffect(() => {
    ...
    if (threshold > 0 && threshold > bonusMoveThresholdRef.current) {
      ...
      setBonusMoves(prev => prev + newMoves);
      bonusMovesRef.current += newMoves;
      if (showBonusPrompt) {
  ...
  // v10 Fix 10: Backport v9.8 fix — pre-count unused specials toward target before deciding win/loss
  ...
  useEffect(() => {
    ...
    const checkTimer = setTimeout(() => {
      ...
      // v10.5 Fix A: Defer game-end if a bonus move threshold is still pending
      ...
      // v10 Fix 10: Calculate specials bonus FIRST, then check if we'd reach target with it
      ...
      // v8.10: If in victory round and no moves left, end game (no move bonus)
      ...
      // Target reached (including with specials bonus) with no moves = WIN
      ...
      // v12.0: Out of moves — offer bonus moves before game over
      if (moves <= 0 && !hasReachedTarget && !usingBonusMoves) {
        if (bonusMoves > 0) {
      ...
      // v12.0: Using bonus moves — each swap consumes 1; when bonusMoves hits 0, true game over
      if (moves <= 0 && !hasReachedTarget && usingBonusMoves) {
        if (bonusMoves > 0) return; // still have bonus moves — game continues
        setScore(prev => prev + specialsBonus);
        if (scoreWithBonus > highScore) setHighScore(scoreWithBonus);
        setDifficultyBonus(0);
        setGameState('gameover');
  ...
  const startBonusRound = () => {
    ...
    // v10.5: Flush any pending bonus move flash now that prompt is dismissed
  ...
  // v12.0: Player chose "End and carry moves forward" — game ends, bonusMoves persist naturally
  ...
  const endLevelEarly = () => {
    setShowBonusPrompt(false);
    // v10.5: Discard any pending bonus move flash — level is ending early
    bonusMoveFlashPendingRef.current = 0;
  ...
  // Score popup cleanup - v8.2: handles variable delays and durations
  useEffect(() => {
  ...
  const renderCanvas = useCallback(() => {
    ...
    grid.forEach((row, rowIndex) => {
      row.forEach((tile, colIndex) => {
        ...
        // v10 Fix 9: O(1) Set lookups instead of O(n) Array.some on every frame
        const isMatched = matchedTileSet.has(`${rowIndex}-${colIndex}`);
        const isPending = pendingSpecialSet.has(`${rowIndex}-${colIndex}`);
        ...
        // v10 Fix 9: O(1) Set lookups for glow/flash state
        const isGlowing = glowingTileSet.has(`${rowIndex}-${colIndex}`);
        const isFlashing = flashingTileSet.has(`${rowIndex}-${colIndex}`);
  ...
  const handleCanvasClick = (e) => {
    if (swapFiredRef.current || e.detail === 0) return; // v10.2: Mobile double-tap guard; Fix 2: phantom click after drag
  ...
  const removeMatches = (currentGrid, matchGroups, lShapeMatches, generation, connectedGroups = [], swapPosition = null) => {
      ...
      sortedConnected.forEach(group => {
        if (group.totalUniqueTiles >= 4) {
          ...
          if (!bestTile) {
            ...
            // v10 Fix 2b: If all center tiles are claimed, find ANY unclaimed tile in the group
            if (!bestTile) {
              bestTile = group.tiles.find(t => !claimedPositions.has(`${t.row}-${t.col}`));
            }
          }
          
          // v10 Fix 2b: Only proceed if tile is unclaimed AND will actually be empty
          if (bestTile) {
            const posKey = `${bestTile.row}-${bestTile.col}`;
    ...
    specialsToCreate.forEach(({ row, col, type, tileColor }) => {
      newGrid[row][col] = {
        ...
        special: type,
        isNew: false,
        // v10.2: Use _initAnim fields; renderCanvas reads on first encounter
      ...
      for (let row = ROWS - 1; row >= 0; row--) {
        if (newGrid[row][col] !== null) {
          if (row !== emptyRow) {
            ...
            // v10.6.1: Directly seed animStateRef from old visual position.
    ...
    <div style={{
      ...
      <div style={{
        ...
        <h1 style={{ margin: '0', color: '#333', fontSize: '20px', textShadow: '2px 2px 4px rgba(0,0,0,0.1)' }}>
          🎮 Match-3 <span style={{ fontSize: '12px', color: '#888' }}>v12.3</span>
        </h1>
        <div style={{
          display: 'flex',
          ...
          <div style={{ position: 'relative' }}>
            ...
            {bonusMoves > 0 && (
              <span style={{ marginLeft: '8px', color: usingBonusMoves ? '#e65100' : '#667eea' }}>
                🏦{bonusMoves}
              </span>
            )}
            {bonusMoveFlash > 0 && (
              <span
                ...
                style={{
                  ...
                  fontSize: '22px', fontWeight: '900', color: '#00C853',
        ...
        {bonusRoundActive && (
          <div style={{
            ...
            🌟 VICTORY ROUND - {BONUS_ROUND_MULTIPLIER}x ALL POINTS! 🌟
          </div>
        )}

        {/* v12.0: Persistent "End and carry" button while using bonus moves */}
        {usingBonusMoves && (
          <button
          ...
          >
            End and carry 🏦{bonusMoves} moves forward
          </button>
        )}
      ...
      {/* v12.0: Bonus Moves Prompt — shown when regular moves hit 0 and 🏦 > 0 */}
      {showBonusMovesPrompt && (
        <div style={{
          ...
          gap: '20px', zIndex: 1000, boxShadow: '0 4px 20px rgba(0,0,0,0.4)',
          ...
          <div style={{ textAlign: 'center', color: 'white', minWidth: '160px' }}>
            ...
            <div style={{ fontSize: '15px', color: 'rgba(255,255,255,0.85)' }}>
              You have {bonusMoves} banked move{bonusMoves !== 1 ? 's' : ''}
            </div>
          </div>
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', justifyContent: 'center' }}>
            <button
              ...
              style={{
                padding: '12px 24px', fontSize: '16px', background: '#00C853',
            ...
            <button
              onClick={endGameCarryBanked}
              style={{
                padding: '12px 24px', fontSize: '16px', background: 'white',
      ...
      {showBonusPrompt && (() => {
        // v10 Fix 11: Calculate specials bonus to show accurate "potential score" (backport from phone)
        ...
        return (
          <div style={{
            ...
            <div style={{ 
              ...
              {bonusRoundScore > 0 && (
                <div>🌟 Victory Round: +{bonusRoundScore}</div>
              )}
