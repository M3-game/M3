...
const Match3Game = () => {
  ...
  const activateSpecialTile = (row, col, currentGrid, alreadyCleared = new Set()) => {
    ...
    if (tile.special === 'line') {
      ...
      // v10.1: 5×5 + row + col, then random 50% of remaining regular tiles. Specials immune. Min 30 tiles.
      const addedKeys = new Set();
      ...
      remaining.slice(0, halfCount).forEach(t => { tilesToClear.push(t); addedKeys.add(`${t.row}-${t.col}`); });
      const minTiles = 30;
      if (tilesToClear.length < minTiles) {
        const extra = remaining.slice(halfCount);
        for (let i = 0; i < extra.length && tilesToClear.length < minTiles; i++) tilesToClear.push(extra[i]);
      }
  ...
  // Activate enhanced effects when two special tiles are swapped together (v10.1 + v10.5 Fix B)
  const activateSpecialCombination = (row1, col1, row2, col2, type1, type2, currentGrid, additionalGroups = [], swapPosition = null) => {
    ...
    const getCascadeDelay = (depth) => depth <= 1 ? 0 : (depth - 1) * 400;
      ...
      >
        ...
        >
          ...
          <h1 style={{ margin: '0', color: '#333', fontSize: '15px', fontWeight: 'bold' }}>
            M3 <span style={{ fontSize: '9px', color: '#999', fontWeight: 'normal' }}>v13.0-418px</span>
          </h1>
