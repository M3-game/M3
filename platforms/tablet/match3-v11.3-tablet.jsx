...
const Match3Game = () => {
  ...
  // v11.3: In-game bonus moves pool — earned per 10k points, separate from regular moves
  const [bonusMovePool, setBonusMovePool] = useState(0);
  // v11.3: True after player chooses "Use extra moves" — enables "End level" button in header
  const [usingExtraMoves, setUsingExtraMoves] = useState(false);
  // v11.3: True when regular moves=0 and pool>0, shows the two-button decision prompt
  ...
  useEffect(() => {
    ...
    if (showExtraMovesPrompt) return; // v11.3: Don't check while extra moves prompt is showing
    ...
    const checkTimer = setTimeout(() => {
      ...
      // v11.3: Out of regular moves — check bonus pool before declaring game over
      ...
      // v11.3: Extra moves exhausted — end game
      if (moves <= 0 && !hasReachedTarget && usingExtraMoves) {
        setScore(prev => prev + specialsBonus);
        if (scoreWithBonus > highScore) setHighScore(scoreWithBonus);
        setDifficultyBonus(0);
  ...
  // v11.2: Add moves to the banked pool, respecting the cap
  const addBankedMoves = (n) => {
    setBankedMoves(prev => Math.min(prev + n, BANKED_MOVES_CAP));
  };

  // v11.3: Player chose "Use extra moves" — transfer pool to regular moves counter
  const useExtraMoves = () => {
  ...
  // v11.3: Player chose "Save moves / End level" — bank pool, end game
  const saveMovePool = () => {
    addBankedMoves(bonusMovePool);
    setBonusMovePool(0);
    setShowExtraMovesPrompt(false);
    recordGameResult({ endType: 'savedMoves', finalScore: score, won: false });
    setGameState('gameover');
  };

  // v11.3: "End level" button pressed mid-extra-moves — bank remaining moves, end game
  const endLevelFromExtra = () => {
    addBankedMoves(moves);
    setMoves(0);
    recordGameResult({ endType: 'savedMoves', finalScore: score, won: false });
    setGameState('gameover');
  };
  ...
  const restartGame = () => {
    ...
    bonusMoveFlashPendingRef.current = 0;
    setBonusMoveFlash(0);
    // v11.3: Reset bonus pool and extra-moves flow
    setBonusMovePool(0);
    setUsingExtraMoves(false);
    setShowExtraMovesPrompt(false);
  };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          {/* v11.3: Bonus move pool — in-game only, shown when non-zero */}
          {bonusMovePool > 0 && (
          ...
          {/* v11.2: Banked moves — separate persistent counter */}
          <div title="Saved moves carry forward to future games">
            🏦 <span style={{
              color: bankedMoves >= BANKED_MOVES_CAP  ? '#e53935' :
      ...
      {/* v11.3: Extra Moves Prompt — shown when regular moves hit 0 and pool is non-empty */}
