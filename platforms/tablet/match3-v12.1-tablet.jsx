...
// =============================================================================
// MATCH-3 GAME v12.1 - TABLET OPTIMIZED (escalating target survives navigation)
//
// Player-reported: going from tablet-verses to tablet arcade reset the raised
// target back to base. Three changes.
//
//   1) BUG FIX — the escalating target is now persisted.
//      `difficultyBonus` lived only in React state, so it was discarded by any
//      page load. Both cross-mode buttons ("Go to Verses" here, "Arcade mode"
//      in tablet-verses) are full navigations, so the round trip wiped it —
//      but so did an ordinary refresh. The result was a contradiction, because
//      `match3_currentRun` IS persisted: the header could report a six-win run
//      next to a beginner target. Now stored under `match3_difficultyBonus`.
//      NOTE: the restore has to feed BOTH the initial `levelTarget` and
//      restartGame. Only doing restartGame would leave the first round after
//      every page load at the base target, with the ramp appearing on round 2.
//
//   2) TIERED INCREMENTS. The flat 200–500 per win now tapers by run length:
//         wins  1–11 → 200–500      wins 24–35 → 100–300
//         wins 12–23 → 200–400      wins 36+   → 100–200
//      Tier boundaries at 12 / 24 / 36 rather than round numbers: planned
//      reward levels arrive every 5 wins, and because 5 and 12 share no
//      factors a boundary and a reward level cannot coincide until win 60.
//      The four win paths now share `difficultyIncrementFor()` instead of
//      each carrying its own copy of the numbers.
//
//   3) NEW STAT — "Highest target reached". The hardest arcade round ever
//      beaten, on the end-game screen with the run stats. Deliberately NOT on
//      the in-game header: high score is always >= highest target (winning
//      requires scoring at least the target), so side by side the new number
//      would be permanently smaller with no visible reason why, reading as a
//      weaker duplicate. In-game it appears only as a 5-second "Highest target
//      yet" marker beside the target at the start of a qualifying round.
// In-game header label bumps to "v12.1".
//
// =============================================================================
// MATCH-3 GAME v12.0 - TABLET OPTIMIZED ("Go to Verses" + tutorial graduation)
//
// Major bump v11.26 → v12.0. Two things:
//   1) MAJOR-BUMP GRADUATION. The in-app Tutorial (a significant new screen)
//      arrived incrementally across v11.19–v11.26 as patch bumps. Per user
//      direction (2026-07-04), the next tablet-arcade change graduates the
//      version to v12.0 to mark the tutorial as part of the game — rather than
//      bumping retroactively. This is that next change, so it lands the
//      graduation.
//   2) NEW FEATURE — "Go to Verses" button. Adds a "📖 Go to Verses" button to
//      the end-game screen (win AND loss), the reverse of the "Arcade mode"
//      handoff verses already has. Always shown — serves both the player who
//      arrived from verses and the arcade-first player who wants to try it.
//      Navigates to verses.html. Bonus moves carry automatically: arcade and
//      verses share one persistent wallet (core BONUS_MOVES_KEY, read fresh at
//      mount on both sides), so nothing is transferred — it's already shared.
//      Same button styling as "Play Again". No gameplay/scoring change.
// In-game header label bumps to "v12.0".
//
// =============================================================================
// MATCH-3 GAME v11.26 - TABLET OPTIMIZED (tutorial: panel 8 fusion final — all 8)
//
// Finalizes panel 8 (fusion) after review — the v11.25 draft, now reviewed &
// approved. **All 8 shared tutorial panels are now complete** (basic match,
// line special, bomb, cross, supernova, hypernova, multipliers, fusion). The
// panel lives in core/Tutorial.jsx (edit-in-place, inherited); this file
// carries only the `sections` list + version label. Everything else identical
// to v11.25. In-game header label bumps to "v11.26".
//
// =============================================================================
...
// v12.1: Escalating target — tiered increments (was a flat 200–500 per win).
// Each won round raises the NEXT round's target by a random amount drawn from
// the band for the tier the run has reached, so difficulty stops accelerating
// as a run gets long. A loss resets the accumulated bonus to 0.
//
// Boundaries sit at 12 / 24 / 36 wins rather than 10 / 20 / 40 for a specific
// reason: reward levels (planned) arrive every 5 wins, and 5 and 12 share no
// common factor, so a tier step and a reward level cannot land on the same
// round until win 60. On 10/20/40 every boundary collided.
//
// The 100 floor on the later bands is also deliberate. The final target is
// rounded to the nearest 100, so an increment below ~50 would frequently be
// invisible to the player — a win that appears to change nothing.
...
// v12.1: Single source of truth for the increment. Four separate code paths
// can end a round in a win (victory-round end, moves exhausted, end-early, and
// ending while on bonus moves) and every one of them raises the target. Before
// v12.1 each carried its own copy of the min/max; with a four-tier table that
// would be four copies to keep in step, so they all call this instead.
//
// `winNumber` is the 1-based position of the win being completed — i.e.
// currentRun + 1, because the run-tracking effect only increments currentRun
// afterwards, in response to the gameState change.
...
// v12.1: The escalating target now survives page loads. Before v12.1 it was
// React state only, so the "Go to Verses" / "Arcade mode" round trip between
// tablet.html and verses.html — and any ordinary refresh — silently reset it
// to base while RUN_CURRENT_KEY above kept showing the run.
const DIFFICULTY_BONUS_KEY = 'match3_difficultyBonus';

// v12.1: Highest target ever beaten (not merely faced — the round that ends a
// run does not count). Only arcade rounds move it; reward levels, when they
// arrive, draw targets from their own table and keep their own stats.
const HIGHEST_TARGET_KEY = 'match3_highestTarget';

// v12.1: Shared reader for the two keys above. Anything missing, unparseable,
// or negative reads as 0, which is exactly the pre-v12.1 behavior — so an
// existing player with nothing stored sees no change on first load and no
// migration is needed.
...
const Match3Game = () => {
  ...
  // v12.1: Restored from storage rather than starting at 0. A loss still
  ...
  // v12.1: The first round's target must include the restored bonus too.
  // restartGame has always added difficultyBonus, but this initializer had not
  // — harmless while the bonus was always 0 at mount, and a real bug once it
  // persists: the first round after any page load would drop to base and the
  // ramp would only reappear on round 2. Reads storage directly rather than
  // the state above, because both initializers run in the same render pass.
  ...
  // v12.1: Highest target ever beaten. The ref mirrors the state so the win
  ...
  // v12.1: True when the round that just ended set a new highest target, so
  // the end screen can mark it. Cleared on restart.
  ...
  // v12.1: Persist the escalating target. This is the actual fix for the
  // verses → arcade reset: with the value written here and read back at mount,
  // navigating away and returning no longer discards a run's accumulated
  // difficulty. Writes 0 on a loss too, which is correct — a loss clears it.
  useEffect(() => {
    try { localStorage.setItem(DIFFICULTY_BONUS_KEY, difficultyBonus.toString()); } catch {}
  }, [difficultyBonus]);

  // v12.1: Persist the highest target reached, and keep the ref in step.
  useEffect(() => {
    highestTargetRef.current = highestTarget;
    try { localStorage.setItem(HIGHEST_TARGET_KEY, highestTarget.toString()); } catch {}
  }, [highestTarget]);

  // v12.1: Called from each of the four win paths with the target of the round
  // just won. Compares against the ref rather than the state value because
  // several win paths run inside effects or handlers that would otherwise see
  // a stale closure. Only ever moves upward.
  ...
  // v12.1: "Highest target yet" marker — shown beside the target for 5 seconds
  // at the start of a round whose target beats the record, then cleared.
  // Timed rather than persistent on purpose: the target is fixed for the whole
  // round, and once a run passes its own best EVERY later round qualifies, so
  // a permanent badge would sit there for the rest of the run and stop meaning
  // anything. Guarded on a record existing at all, so a player who has never
  // won a round doesn't see it on their first game.
  ...
  useEffect(() => {
    ...
    const checkTimer = setTimeout(() => {
      ...
      if (victoryRoundActive && moves <= 0) {
        ...
        // v12.1: tiered increment, shared helper. currentRun + 1 because the
      ...
      if (moves <= 0 && !victoryRoundActive) {
        ...
        if (hasReachedTarget) {
          if (!targetReached) setTargetReached(true);
          // v12.1: tiered increment, shared helper.
          setDifficultyBonus(prev => prev + difficultyIncrementFor(currentRun + 1));
          recordTargetReached(levelTarget);
    ...
    // v12.1: currentRun added — the two win paths in this effect now read it
    // to pick the difficulty tier for the win being completed.
  }, [moves, gameState, levelTarget, highScore, isAnimating, combo, targetReached, pendingSpecials.length, grid, turnComplete, victoryRoundActive, showVictoryPrompt, showBonusMovesPrompt, bonusMoves, usingBonusMoves, currentRun]);
  ...
  const restartGame = () => {
    ...
    setShowEndConfirm(false);
    // v12.1: Clear the "new best target" flag from the round just finished.
    ...
    <div style={{
      ...
      <div style={{
        ...
        <h1 style={{ margin: '0', color: '#333', fontSize: '20px', textShadow: '2px 2px 4px rgba(0,0,0,0.1)' }}>
          🎮 Match-3 <span style={{ fontSize: '12px', color: '#ccc' }}>v12.1</span>
        </h1>
        <div style={{
          display: 'flex',
          ...
          {/* v12.1: Target, plus the 5-second "Highest target yet" marker when
              this round's target beats the stored record. Amber rather than the
              #FFD700 gold used on the end screen, because this sits on the light
              header card where gold is close to unreadable. */}
          <div>
        ...
        return (
          <div style={{
            ...
            <div style={{ textAlign: 'center', color: 'white', minWidth: '200px' }}>
              ...
              {/* v12.1: Highest target reached — the hardest round ever beaten
            ...
            {/* v12.0: "Go to Verses" — the reverse of the "Arcade mode"
                handoff verses already has. Always shown on the end-game
                screen (win or loss) — serves both the player who arrived
                from verses and the arcade-first player who wants to try
                verses. Bonus moves carry automatically (shared wallet —
                verses reads the same BONUS_MOVES_KEY at mount). Same button
                styling as Play Again. */}
            <button
              onClick={() => {
                const base = (typeof import.meta !== 'undefined' && import.meta.env && import.meta.env.BASE_URL) || '/';
                window.location.href = `${base}verses.html`;
              }}
              style={{
                padding: '10px 25px',
                fontSize: '16px',
      ...
      <style>{`
        ...
        /* v12.1: "Highest target yet" marker — holds for most of its 5s life,
           then fades over the last half second so it leaves quietly rather
           than vanishing mid-glance. The React timer clears it at 5s. */
