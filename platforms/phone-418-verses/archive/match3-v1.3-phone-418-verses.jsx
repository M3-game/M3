...
// N-2: phone-418-verses no longer imports BANKED_KEY from AdminPanel —
// uses a phone-scoped local constant `PHONE418_BANKED_MOVES_KEY`
...
// =============================================================================
// MATCH-3 GAME v1.3 - PHONE 418PX VERSES (MEMORIZE MODE)
//
...
// needed.
//
// Scope: phone-418-verses only. Phone-418 arcade has the same chrome
...
//
// =============================================================================
// MATCH-3 GAME v1.2 - PHONE 418PX VERSES (MEMORIZE MODE)
//
...
// v11.13: onClick switched from `endLevelCarryBanked` to
...
//     routes through `requestEndLevelCarryBanked` (Option B fix part 1).
...
//
// =============================================================================
// MATCH-3 GAME v1.1 - PHONE 418PX VERSES (MEMORIZE MODE)
//
...
//   - Row 2 (only when `bankedMoves > 0` OR `usingBankedMoves`):
...
//
// =============================================================================
// MATCH-3 GAME v1.0 - PHONE 418PX VERSES (MEMORIZE MODE)
//
...
//
// TILE / BOARD SIZING (matches phone-418 arcade):
...
//   - Banked-moves carry into phone-418 arcade reads/writes
//     'match3_phone418_bankedMoves' (phone-418 arcade's pool key) via
//     a new local PHONE418_BANKED_MOVES_KEY constant — replaces the
//     tablet `BANKED_KEY` import for these runtime writes.
//   - Verses → arcade carry-receipt key 'm3_arcade_carry_from_verses'
//     → 'm3_arcade_carry_from_verses_phone418'. Phone-418 arcade
...
// "🌟 BONUS ROUND - 1.5x ALL POINTS! 🌟" pulsing banner in
// VERSES_MODE. The banner was an arcade-side render gated only on
// `bonusRoundActive`; V-4 flipped that flag in VERSES_MODE to drive
...
// memorize-mode read. One-line guard: `!VERSES_MODE && bonusRoundActive`
...
//     return to picker — bankedMoves does NOT persist.
//
...
//     `bonusRoundActive` flag (already wired into the scoring
//     branch with `BONUS_ROUND_MULTIPLIER = 1.5`) auto-flips true.
...
//     caption removed.
//   - onClick: writes `bankedMoves` to `BANKED_KEY` (the same key
...
//     spec'd as level-to-level-but-not-game-to-game (`bankedMoves`).
//   - `bankedMoves` lifted out of `VersesGame` — the v1.4 useState +
//     persist effect are gone from the game body. `VersesGame` now
//     receives `bankedMoves` as a prop and reports changes via an
//     `onBankedMovesChange` callback. Reset/zero behavior moves to
//     the wrapper.
...
//     owned state (bankedMoves, completedLevels, fullCompleted)
...
//     `completedLevels`, `fullCompleted`, and zeros `bankedMoves`.
...
//     in free-replay) and zeroing `bankedMoves` (full-game restart
...
// N-2 (phone-418-verses v1.0): TILE_SIZE 50 → 40, TILE_GAP 4 → 2 to
// match phone-418 arcade and fit the 418px viewport.
// N-6 (phone-418-verses v1.3, 2026-05-01): COLS 10 → 9 + TILE_SIZE
...
// N-2 (phone-418-verses v1.0): phone-scoped banked-moves storage key.
// Replaces the imported tablet `BANKED_KEY` for runtime reads/writes —
// matches phone-418 arcade's existing key (defined in
// platforms/phone-418/match3-v13.x-418px-phone.jsx). Keeps phone runs
// distinct from tablet runs per the 2026-03-23 design principle.
const PHONE418_BANKED_MOVES_KEY = 'match3_phone418_bankedMoves';
...
const TARGET_PER_MOVE_DEFAULT = 250;  // N-2: phone uses moves × 250 (tablet stays at × 300)
...
// N-2 (phone-418-verses): phone-scoped progress prefix to keep iPhone
// memorize progress distinct from iPad's per the 2026-03-23 principle.
const VERSES_PROGRESS_KEY_PREFIX = 'm3_phone418_verses_';
...
//
// V-3b: bonus moves (`bankedMoves`) lifted to the wrapper so they
// survive level-to-level remounts. `VersesGame` receives the current
// value as a prop and reports changes via `onBankedMovesChange`
...
const VersesGame = ({
  ...
  // VersesGame receives `bankedMoves` as a prop and updates it via
  // `onBankedMovesChange`. In !VERSES_MODE (arcade fallback path),
  ...
  const setBankedMoves = onBankedMovesChange;
  ...
  // along with the bankedMoves state itself. The wrapper handles the
  ...
  // V-4: also flips bonusRoundActive so the existing scoring branch
  ...
  useEffect(() => {
    ...
    const checkTimer = setTimeout(() => {
      ...
      // prompt, banked-moves prompt, gameState win/lose, recordGameResult)
  ...
  // (in showBankedMovesPrompt) now also routes through this wrapper.
  ...
  const restartGame = () => {
    ...
    // doesn't register as a decrement. Bonus-moves pool (bankedMoves)
    ...
    if (VERSES_MODE) {
      ...
      setBankedMoves(0);
      try { localStorage.setItem(PHONE418_BANKED_MOVES_KEY, '0'); } catch {}
    }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* v1.1: Two-row score zone for phone-418-verses.
        ...
        <div style={{
          ...
          <div style={{
            ...
            {/* V-2/V-4: In VERSES_MODE, target display flips gold + ✓
                ...
                end-of-round. V-4 appends "· 1.5×" while bonusRoundActive
          ...
          <div style={{
            ...
            {(() => {
              ...
              if (!isMultiLevel) {
                ...
                // — writes bankedMoves to BANKED_KEY + carry-receipt
                ...
                buttons.push({
                  ...
                  label: 'Arcade mode',
                  onClick: () => {
                    // N-2: arcade-mode handoff. Roll bankedMoves into
                    // phone-418 arcade's PHONE418_BANKED_MOVES_KEY (the
                    // same key the arcade reads at mount) AND write a
                    // separate carry-receipt under
                    // m3_arcade_carry_from_verses_phone418 so phone-418
                    // arcade v13.2 can show a transfer banner on entry.
                    // Phone-scoped per the 2026-03-23 phone/tablet-
                    // distinct-runs principle. Then navigate.
                    try {
                      const carryAmount = bankedMoves || 0;
                      // Add to existing arcade banked-moves (don't
                      // overwrite — the player may have prior arcade
                      // bonus moves already stored).
                      const existing = parseInt(localStorage.getItem(PHONE418_BANKED_MOVES_KEY) || '0', 10);
                      localStorage.setItem(PHONE418_BANKED_MOVES_KEY, (existing + carryAmount).toString());
                      localStorage.setItem('m3_arcade_carry_from_verses_phone418', JSON.stringify({
                        amount: carryAmount,
                    ...
                    window.location.href = `${base}phone418.html`;
                  },
                });
...
// resets via the remount; cross-level state (bankedMoves,
...
const Match3Verses = () => {
  ...
  // and skip PHONE418_BANKED_MOVES_KEY persistence — memorize-mode
  // bonus moves are session-scoped (zero on return to picker). In
  // !VERSES_MODE (arcade fallback), hydrate from
  // PHONE418_BANKED_MOVES_KEY at mount and persist via the effect
  ...
  const [bankedMoves, setBankedMoves] = useState(() => (
    VERSES_MODE ? 0 : parseInt(localStorage.getItem(PHONE418_BANKED_MOVES_KEY) || '0', 10)
  ));
  useEffect(() => {
    if (VERSES_MODE) return;
    localStorage.setItem(PHONE418_BANKED_MOVES_KEY, bankedMoves.toString());
  }, [bankedMoves]);
  ...
  // game-scoped state (completedLevels, fullCompleted, bankedMoves)
  ...
  return (
    <>
      <VersesGame
        ...
        bankedMoves={bankedMoves}
        onBankedMovesChange={setBankedMoves}
        completedLevels={completedLevels}
        fullCompleted={fullCompleted}
        ...
        onRestartGame={() => {
          ...
          // bankedMoves (full-game restart is a soft session boundary).
