...
// migration at module load. Time-attack doesn't otherwise import core,
// so this explicit call ensures the migration runs on first visit to
// time-attack after T-3b deploys (idempotent — no-op on subsequent
// loads).
migrateBonusMovesKeys();

// =============================================================================
// MATCH-3 GAME v12.4 - TIME ATTACK MODE (DESKTOP) (Session F-1: grace window)
//
// Session F-1 (2026-05-03): Port the 1.5s grace window from campaign
// tablet v1.25 to time-attack. Closes the long-standing "score
// undercounted if timer fires mid-cascade" bug — when the timer hits
// 0 mid-cascade, late `addTimeExtension` calls (specials, combo 5+,
// score milestones) firing during the cascade can now re-arm the
// timer and let play continue. Without grace, the cascade's
// extensions were silently ignored and the game ended early.
//
// Implementation (mirrors campaign v1.28's pattern):
//   - New `TIME_ATTACK_GRACE_MS = 1500` constant.
//   - New `graceActive` useState.
//   - Timer effect: pauses if `graceActive` (early return). When
//     timer hits 0 with no bonus-moves prompt path, sets graceActive
//     instead of the previous 100ms setTimeout to setGameState.
//   - New grace-watcher useEffect: if `timeRemaining > 0` (an
//     extension landed mid-cascade), cancels grace; otherwise after
//     TIME_ATTACK_GRACE_MS, fires the win/gameover resolution.
//
// Edge cases (matches campaign behavior):
//   - Bonus move earned during grace (cascade crosses BONUS_MOVE
//     threshold mid-grace): bonus move banks to pool for next game;
//     this game still ends after grace. Don't switch tracks.
//   - Bonus-moves prompt path unchanged: when bonusMoves > 0 and
//     below target at timer=0, prompt path is taken instead of grace.
//   - useBonusMove (player chose +5s from prompt): unchanged. Sets
//     timeRemaining to 5; timer effect resumes naturally.
//
// No other behavior changes.
//
// =============================================================================
// MATCH-3 GAME v12.3 - TIME ATTACK MODE (DESKTOP) (T-3b storage migration)
//
// Session T-3b (2026-05-03): Storage-string VALUE migration (data half,
// B part of T-3 split). Time-attack's local `BONUS_MOVES_KEY` value
// flips from 'match3_timeattack_bankedMoves' to 'match3_timeattack_bonusMoves'.
// Existing player data migrates automatically on first load via core's
// `migrateBonusMovesKeys()` (imported above; idempotent via stricter
// rule — always removes the old key).
//
// Session T-2 (2026-05-02): Bundled terminology rename. "Bonus Round" →
// "Victory Round" and `bankedMoves` → `bonusMoves`. Mirrors T-1's
// pattern; T-2 completed the sweep across desktop, time-attack, phone
// arcade, and campaign.
//
// Time-attack-specific changes (T-2):
...
// v10.4: Bonus move refs added (no victory round in time attack)
...
//      Pool persists across games (localStorage key — T-3b: 'match3_timeattack_bonusMoves').
...
// F-1 (2026-05-03): grace window after timer hits 0 — gives any in-flight
// cascade time to land its addTimeExtension calls and re-arm the timer.
// Mirrors campaign v1.25's TIME_ATTACK_GRACE_MS.
const TIME_ATTACK_GRACE_MS = 1500;
...
// T-3b (2026-05-03): VALUE flipped from 'match3_timeattack_bankedMoves'.
// Migration in core/AdminPanel.jsx runs at module load via the import
// at top of this file.
const BONUS_MOVES_KEY = 'match3_timeattack_bonusMoves';
...
const Match3Game = () => {
  ...
  // F-1 (2026-05-03): grace window after timer hits 0. Pauses the timer
  // for TIME_ATTACK_GRACE_MS so a late addTimeExtension from an in-flight
  // cascade can re-arm timeRemaining and resume play.
  ...
  // F-1 (2026-05-03): Also pauses during the grace window (graceActive). A
  //      late addTimeExtension during grace can re-arm the timer by setting
  //      timeRemaining > 0, at which point the grace-watcher effect clears
  //      graceActive and this effect re-runs to re-create the interval.
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      setTimeRemaining(prev => {
        if (prev <= 1) {
          ...
          if (scoreRef.current < TIME_ATTACK_TARGET && bonusMovesRef.current > 0) {
            setShowTimeUpPrompt(true);
            return 0;
          }
          // F-1: start grace window. Resolution happens in the grace-watcher
          // effect below — either canceled by an addTimeExtension landing
          // mid-cascade (timeRemaining > 0), or fired after TIME_ATTACK_GRACE_MS.
          setGraceActive(true);
          return 0;
        }
        return prev - 1;
  ...
  }, [gameState, score, highScore, showTimeUpPrompt, graceActive]);

  // F-1 (2026-05-03): Grace window watcher. When the timer hits 0 and the
  // game is about to resolve, we wait TIME_ATTACK_GRACE_MS. If an
  // addTimeExtension (from a cascade's special / combo / score milestone)
  // fires during that window, it sets timeRemaining > 0 — we cancel grace
  // and the main timer effect resumes the interval. If nothing lands by
  // the end of the window, fire the win/gameover resolution that the
  // previous 100ms setTimeout did.
  useEffect(() => {
    if (!graceActive) return;
    if (timeRemaining > 0) {
  ...
  }, [graceActive, timeRemaining, highScore]);
  ...
  const activateSpecialCombination = (row1, col1, row2, col2, type1, type2, currentGrid, additionalGroups = [], swapPosition = null) => {
    ...
    // Time attack: no victory round multiplier
  ...
  const removeMatches = (currentGrid, matchGroups, lShapeMatches, generation, connectedGroups = [], swapPosition = null) => {
    ...
    // v9.9 TIME ATTACK: Add special points to score (no victory round multiplier)
  ...
  const restartGame = () => {
    ...
    setLastMilestoneShown(0);
    setShowTimeUpPrompt(false);
    setGraceActive(false);  // F-1: clear any in-flight grace window on restart
    ...
    setBonusMoveFlash(0);
    // v12: Reset prompt state (bonusMoves intentionally NOT reset — pool persists)
    setShowTimeUpPrompt(false);
  };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <h1 style={{ margin: '0', color: '#333', fontSize: '20px', textShadow: '2px 2px 4px rgba(0,0,0,0.1)' }}>
          ⏱️ TIME ATTACK <span style={{ fontSize: '12px', color: '#888' }}>v12.4</span>
        </h1>
        ...
        <div style={{
          ...
          {bonusMoves > 0 && (
            <div title="Bonus moves — carry forward between games">
              🎯 <span style={{ color: '#667eea', fontWeight: 'bold' }}>{bonusMoves}</span>
            </div>
          )}
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ fontSize: '14px', color: 'rgba(255,255,255,0.85)', marginBottom: '24px' }}>
              You have <strong>{bonusMoves}</strong> bonus move{bonusMoves !== 1 ? 's' : ''}.
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
              ...
              >
                End game — save {bonusMoves} bonus move{bonusMoves !== 1 ? 's' : ''}
              </button>
            </div>
          </div>
        </div>
