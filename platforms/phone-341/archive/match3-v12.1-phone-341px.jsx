...
// =============================================================================
// MATCH-3 GAME v10.5.4 - PHONE OPTIMIZED (341px)
// Performance-optimized with HTML5 Canvas rendering
// 8x10 grid for phone screens, portrait orientation
// v10-phone: Bugfix release (all three platforms synced to v10)
//   Fix 1–7: (see original v10 notes)
...
// v10.5: Bonus move timing fix (pending threshold check), startBonusRound/
//        endLevelEarly flush bonusMoveFlashPendingRef
...
// v11-phone-341: Input + animation fixes
//   Fix 1: dragStart converted from useState to useRef — synchronous clear
//           prevents stale-closure double-swap (the 2→0 move skip bug)
//   Fix 2: swapFiredRef blocks phantom click that fires after a touch drag-swap
//           (modern browsers emit detail=1, making the old detail===0 guard useless)
//   Fix 3: Special tiles created from 4+ matches now start _initAnimY one
//           tile-height above final position — prevents instant pop-in
//   Fix 4: Removed duplicate processMatches definition (dead code, shadowed copy)
//   Fix 5: Added missing closing brace in restartGame (syntax error)
...
//   are never reset on restart — they persist across sessions. Only after both
//   regular and banked moves are exhausted does true game over trigger.
...
// Game Constants - Phone optimized
const ROWS = 10;
const COLS = 8;
const TILE_SIZE = 40;
const TILE_GAP = 3;
...
// Difficulty Constants (adjusted for 80 tiles, ~46 pts/tile)
const MIN_MOVES = 18;
const MAX_MOVES = 24;
const BASE_TARGET = 3700;
const TARGET_VARIANCE = 550;

// Scoring Constants
const WIN_BONUS_PER_MOVE = 100;
const EARLY_END_BONUS_PER_MOVE = 200;
const BONUS_ROUND_MULTIPLIER = 1.5;
const DIFFICULTY_INCREMENT_MIN = 150;
const DIFFICULTY_INCREMENT_MAX = 350;
...
const BANKED_MOVES_KEY = 'match3_phone341_bankedMoves';
...
const Match3Game = () => {
  ...
  const dragStart = useRef(null); // v11 Fix 1: useRef for synchronous clear (prevents stale-closure double-swap)
  ...
  // v12.0: Award bonus moves every BONUS_MOVE_INTERVAL points — go into banked pool
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const checkTimer = setTimeout(() => {
      ...
      // v12.0: Out of moves — offer banked moves before game over
      if (moves <= 0 && !hasReachedTarget && !usingBankedMoves) {
        if (bankedMoves > 0) {
      ...
      // v12.0: Using banked moves — each swap consumes 1; when bankedMoves hits 0, true game over
      if (moves <= 0 && !hasReachedTarget && usingBankedMoves) {
        if (bankedMoves > 0) return; // still have banked moves — game continues
        setScore(prev => prev + specialsBonus);
        if (scoreWithBonus > highScore) setHighScore(scoreWithBonus);
        setDifficultyBonus(0);
        setGameState('gameover');
  ...
  // v12.0: Player chose "End and carry moves forward" — game ends, bankedMoves persist naturally
  ...
  const handleDragStart = (e) => {
    ...
    if (row >= 0 && row < ROWS && col >= 0 && col < COLS) {
      dragStart.current = { row, col, x, y }; // v11 Fix 1
      setSelectedTile({ row, col });
    }
  };
  
  const handleDragMove = (e) => {
    if (!dragStart.current || isAnimating || gameState !== 'playing') return; // v11 Fix 1
    e.preventDefault();
    ...
    if ((targetRow !== dragStart.current.row || targetCol !== dragStart.current.col) &&
      ...
      const { row: startRow, col: startCol } = dragStart.current; // v11 Fix 1: capture before clearing
      dragStart.current = null; // synchronous clear prevents re-entry
      swapFiredRef.current = true; // v11 Fix 2: block phantom click
  ...
  const handleDragEnd = () => {
    ...
    dragStart.current = null; // v11 Fix 1
  };
  ...
  // v11 Fix 4: Removed duplicate definition that appeared above this line (dead code)
    ...
    if (tile.special === 'line') {
      ...
      for (let r = 0; r < ROWS; r++) {
        for (let c = 0; c < COLS; c++) {
          if (currentGrid[r][c]) {
            tilesToClear.push({ row: r, col: c });
            // v8.3: Fixed - Hypernova now chains to other specials
            if (currentGrid[r][c].special && !(r === row && c === col) && !alreadyCleared.has(`${r}-${c}`)) {
              chainedSpecials.push({ row: r, col: c, type: currentGrid[r][c].special });
            }
          }
  ...
  const activateSpecialCombination = (row1, col1, row2, col2, type1, type2, currentGrid, additionalGroups = [], swapPosition = null) => {
    ...
    if (combo === 'line+line') {
      ...
      // Clear 5×5 area (upgraded from 3×3)
      for (let r = Math.max(0, effectRow - 2); r <= Math.min(ROWS - 1, effectRow + 2); r++) {
        for (let c = Math.max(0, effectCol - 2); c <= Math.min(COLS - 1, effectCol + 2); c++) {
          if (currentGrid[r][c]) tilesToRemove.push({ row: r, col: c });
        }
      }
      points = 1000;
      message = '💣💣 MEGA BLAST! +1000';
      ...
      points = 900;
      message = '💣✨ CROSS BOMB! +900';
    } else if (combo === 'supernova+supernova') {
      // Clear nearly entire board (10×10 equivalent)
      for (let r = Math.max(0, effectRow - 5); r <= Math.min(ROWS - 1, effectRow + 5); r++) {
        for (let c = Math.max(0, effectCol - 5); c <= Math.min(COLS - 1, effectCol + 5); c++) {
          if (currentGrid[r][c]) tilesToRemove.push({ row: r, col: c });
        }
      }
      points = 5000;
      message = '🌌🌌 DUAL SUPERNOVA! +5000';
      ...
      // v9.8: Fixed 'line+hypernova' to 'hypernova+line' (alphabetical sort order)
      // Clear entire board
      for (let r = 0; r < ROWS; r++) {
        for (let c = 0; c < COLS; c++) {
          if (currentGrid[r][c]) tilesToRemove.push({ row: r, col: c });
  ...
  const removeMatches = (currentGrid, matchGroups, lShapeMatches, generation, connectedGroups = [], swapPosition = null) => {
    ...
    // Infinite loop protection comes from hasValidMoves + gravity settling, not this cap.
    ...
    specialsToCreate.forEach(({ row, col, type, tileColor }) => {
      newGrid[row][col] = {
        ...
        // v11 Fix 3: start one tile-height above final position so special drops in
  ...
  const fillEmptySpaces = (currentGrid, generation) => {
    ...
    setTimeout(() => {
      ...
      if (matches.length > 0) {
        ...
        setLastCombo(comboRef.current); // v10.3: use ref to avoid stale closure
        setCombo(0);
        setTimeout(() => {
          setIsAnimating(false);
          setTurnComplete(true);
  ...
  // Core new board logic
  const performNewBoard = () => {
    setIsAnimating(true);
    ...
    for (let row = 0; row < ROWS; row++) {
      ...
      for (let col = 0; col < COLS; col++) {
        ...
        newGrid[row][col] = {
          ...
          special: null,
          isNew: true,
          // v10.2: Use _initAnimY so renderCanvas starts drop from above
  ...
  const restartGame = () => {
    ...
    setGlowingTiles([]);
    setChainTexts([]);
    // v10.2: Reset animation position cache so new tiles don't inherit stale positions
    ...
    usingBankedMovesRef.current = false;
    setShowBankedMovesPrompt(false);
  }; // v11 Fix 5: Added missing closing brace (was a syntax error)
  ...
  return (
    <div style={{
      ...
      {/* v9.10.3: Redesigned Compact Header */}
      <div style={{
        ...
        {/* Top row: Combo left, Title center, Dark mode right */}
        <div style={{ 
          display: 'flex', 
          ...
          {/* Combo badge - fixed width to prevent layout shift */}
          <div style={{ 
            width: '95px',
            fontSize: '12px',
            fontWeight: 'bold',
            ...
            opacity: (combo > 0 || lastCombo > 0) ? (combo > 0 ? 1 : (gameState === 'playing' ? 0.6 : 0)) : 0, // v10.3
            transition: 'opacity 0.3s'
          }}>
            {(combo > 0 || lastCombo > 0) && `🔥Combo x${(combo > 0 ? combo : lastCombo) + 1}`}
          </div>
          
          {/* Title */}
          <h1 style={{ margin: '0', color: '#333', fontSize: '15px', fontWeight: 'bold' }}>
            M3 <span style={{ fontSize: '9px', color: '#999', fontWeight: 'normal' }}>v12</span>
          </h1>
        ...
        <div style={{
          ...
          <div style={{ position: 'relative' }}>
            ...
            {bankedMoves > 0 && (
              <span style={{ marginLeft: '6px', color: usingBankedMoves ? '#e65100' : '#667eea' }}>
                🏦{bankedMoves}
              </span>
            )}
            {bonusMoveFlash > 0 && (
              <span
                ...
                style={{
                  ...
                  fontSize: '20px', fontWeight: '900', color: '#00C853',
        ...
        {/* v12.0: Persistent "End and carry" button while using banked moves */}
        {usingBankedMoves && (
          <button
          ...
          >
            End and carry 🏦{bankedMoves} moves forward
          </button>
        )}
      </div>
      ...
      {/* v12.0: Banked Moves Prompt — shown when regular moves hit 0 and 🏦 > 0 */}
        ...
        <div style={{
          ...
          <div style={{ textAlign: 'center', color: 'white', minWidth: '160px' }}>
            ...
            <div style={{ fontSize: '14px', color: 'rgba(255,255,255,0.85)' }}>
              You have {bankedMoves} banked move{bankedMoves !== 1 ? 's' : ''}
            </div>
          </div>
