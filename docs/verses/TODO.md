...
| AB | **Arcade button on free-replay end screens** — "Arcade mode" now also shows on the free-replay end screen (any level replay once a multi-level passage is fully completed, win or fail). Was previously missing there, so arcade vanished on all replays after 100%-ing a passage. Scope: final-level/free-replay only — NOT on first-pass non-final wins or first-pass fails (user direction). **Tablet-verses shipped v2.3 (Session 8, 2026-07-04). Phone-verses + sandbox port rides with the BM port** (one `buttons.push` per file). | 🚧 (tablet shipped; 2 ports pending) | S (per port) |
| SP | **"See passage" button** — in-game button (top-left, right of Back) that opens the CURRENT LEVEL's full passage during play (memorization peek, no round end). Current level only — whole-game text already reachable via "See entire <title>" + end screen. **Tablet-verses shipped v2.4 (Session 9, 2026-07-04). Phone-verses + sandbox port rides with the BM port.** | 🚧 (tablet shipped; 2 ports pending) | S (per port) |
